...
\node[align=center] at (5,0.5) {\textbf{Unit tests}\\Prompts, schemas, deterministic code (cheap, many)};
% Integration
\draw (1,1) rectangle (9,2);
\node[align=center] at (5,1.5) {\textbf{Integration tests}\\RAG chains, tool calls, agents (moderate)};
% E2E
\draw (2,2) rectangle (8,3);
\node[align=center] at (5,2.5) {\textbf{End-to-end tests}\\User workflows, multi-turn (expensive)};
% Adversarial
\draw (3,3) rectangle (7,4);
\node[align=center] at (5,3.5) {\textbf{Adversarial / red-team}\\Injection, jailbreaks, abuse (targeted)};
\end{tikzpicture}
\end{llmfigbox}
...
% Axes labels
\node[anchor=east] at (0,3) {Linguistic Capability};
\node[anchor=north, rotate=90] at (-0.5,0) {Test Type};

% Grid cells
\foreach \x in {0,1,2} {
  \foreach \y in {0,1,2} {
    \draw[fill=gray!20, draw=black!40] (\x*2.5,\y*1.5) rectangle (\x*2.5+2.2,\y*1.5+1.2);
  }
}

% Capability labels
\node[anchor=center] at (1.1,3.3) {Vocabulary};
\node[anchor=center] at (3.6,3.3) {Negation};
\node[anchor=center] at (6.1,3.3) {Coreference};

% Test type labels
\node[anchor=east] at (-0.2,2.1) {Invariance};
\node[anchor=east] at (-0.2,0.6) {Directional};
\node[anchor=east] at (-0.2,-0.9) {Minimum};

% Coverage markers (example coverage pattern)
\filldraw[fill=blue!60, draw=black!70] (0.3,1.8) rectangle (1.9,2.7);
\filldraw[fill=blue!60, draw=black!70] (2.8,1.8) rectangle (4.4,2.7);
\filldraw[fill=blue!60, draw=black!70] (5.3,0.3) rectangle (6.9,1.2);
\filldraw[fill=blue!60, draw=black!70] (0.3,-0.6) rectangle (1.9,0.3);

% Legend
\node[anchor=west] at (7.5,2.5) {\textbf{Coverage:}};
\draw[fill=blue!60, draw=black!70] (7.5,1.8) rectangle (7.9,2.2);
\node[anchor=west] at (8.1,2.0) {Tested};
\draw[fill=gray!20, draw=black!40] (7.5,1.2) rectangle (7.9,1.6);
\node[anchor=west] at (8.1,1.4) {Not tested};
\end{tikzpicture}
\end{llmfigbox}
...
\begin{lstlisting}[language=yaml, style=springer]
...
Exact match, F1, execution accuracy &
...
\begin{tikzpicture}[font=\small]
% Dashboard layout
\draw[fill=gray!5, draw=black!30] (0,0) rectangle (10,6);
\node[anchor=north west] at (0.2,5.8) {\textbf{Evaluation Metrics Dashboard}};

% Time series plots (simplified)
\draw[fill=white, draw=black!40] (0.5,4.5) rectangle (4.5,5.5);
\node[anchor=center] at (2.5,5.0) {Correctness Trend};

\draw[fill=white, draw=black!40] (5.5,4.5) rectangle (9.5,5.5);
\node[anchor=center] at (7.5,5.0) {Faithfulness Trend};

\draw[fill=white, draw=black!40] (0.5,2.5) rectangle (4.5,3.5);
\node[anchor=center] at (2.5,3.0) {Latency Trend};

\draw[fill=white, draw=black!40] (5.5,2.5) rectangle (9.5,3.5);
\node[anchor=center] at (7.5,3.0) {Cost Trend};

% Trend indicators
\node[green!70!black] at (4.2,5.0) {$\uparrow$};
\node[red!70!black] at (9.2,5.0) {$\downarrow$};
\node[green!70!black] at (4.2,3.0) {$\uparrow$};
\node[green!70!black] at (9.2,3.0) {$\downarrow$};
\end{tikzpicture}
\end{llmfigbox}
...
\node[box, minimum width=2.5cm] (input) {Input Query};
\node[box, right=of input, minimum width=2.5cm] (golden) {Golden Dataset\\Check};
\node[box, right=of golden, minimum width=2.5cm] (judge) {LLM-as-Judge};
\node[box, right=of judge, minimum width=2.5cm] (semantic) {Semantic\\Similarity};
\node[box, below=of judge, minimum width=2.5cm] (aggregate) {Aggregated\\Score};

\draw[arrow] (input) -- (golden);
\draw[arrow] (golden) -- node[above] {reference match} (judge);
\draw[arrow] (judge) -- (semantic);
\draw[arrow] (semantic) -- (aggregate);
\draw[arrow] (golden) .. controls +(0,-8mm) and +(-8mm,0) .. (aggregate);
\end{tikzpicture}
\end{llmfigbox}
...
\addplot[only marks, mark=*, mark size=2pt, blue!60] coordinates {
...
\addplot[thick, red!70, domain=1:5] {0.7*x + 0.9};
% Perfect calibration line (reference)
\addplot[dashed, gray!60, domain=1:5] {x};
\end{axis}
\end{tikzpicture}
\end{llmfigbox}
...
\begin{lstlisting}[language=json, style=springer]
{
...
\begin{lstlisting}[language=JavaScript, style=springer]
...
\node[box, minimum width=2.5cm] (change) {Code Change};
\node[box, below=of change, minimum width=2.5cm] (tests) {Run Tests};
\node[box, below=of tests, minimum width=2.5cm] (compare) {Compare\\Metrics};
\node[box, below=of compare, minimum width=2.5cm] (gate) {Gate\\Decision};
\node[box, left=of gate, minimum width=2cm] (block) {Block};
\node[box, right=of gate, minimum width=2cm] (deploy) {Deploy};

\draw[arrow] (change) -- (tests);
\draw[arrow] (tests) -- (compare);
\draw[arrow] (compare) -- (gate);
\draw[arrow] (gate) -- node[above] {fail} (block);
\draw[arrow] (gate) -- node[above] {pass} (deploy);
\draw[arrow] (block) .. controls +(-5mm,0) and +(-5mm,0) .. (change);
\end{tikzpicture}
\end{llmfigbox}
...
\begin{llmfigbox}
\begin{tikzpicture}[font=\small, node distance=10mm]
\node[draw, rounded corners, align=center, minimum width=3.2cm, minimum height=8mm] (pr) {Pull Request\\(fast gate)};
\node[draw, rounded corners, align=center, minimum width=3.2cm, minimum height=8mm, right=of pr] (merge) {Merge to main\\(nightly gate)};
\node[draw, rounded corners, align=center, minimum width=3.2cm, minimum height=8mm, right=of merge] (release) {Pre-release\\(full gate)};
\node[draw, rounded corners, align=center, minimum width=3.2cm, minimum height=8mm, right=of release] (canary) {Canary + monitor\\(online gate)};

\draw[->] (pr) -- node[above]{smoke eval} (merge);
\draw[->] (merge) -- node[above]{full eval suite} (release);
\draw[->] (release) -- node[above]{deploy} (canary);
\end{tikzpicture}
\end{llmfigbox}
...
\begin{tikzpicture}[font=\small, node distance=10mm]
% Normal path
\node[lane, minimum width=0.95\linewidth] (normal) {Normal Path: Primary LLM → Response};
\node[lane, below=of normal, minimum width=0.95\linewidth] (failure) {Failure Path: Timeout/Circuit Breaker → Fallback Model → Graceful Degradation};

% Decision points
\node[box, right=8mm of normal.east, anchor=west] (timeout) {Timeout?};
\node[box, below=of timeout] (circuit) {Circuit\\Open?};
\node[box, below=of circuit] (fallback) {Fallback\\Model};
\node[box, below=of fallback] (degrade) {Degraded\\Response};

\draw[arrow] (normal.east) -- (timeout);
\draw[arrow] (timeout) -- node[right] {yes} (circuit);
\draw[arrow] (circuit) -- node[right] {yes} (fallback);
\draw[arrow] (fallback) -- (degrade);
\end{tikzpicture}
\end{llmfigbox}
...
\begin{llmfigbox}
\begin{tikzpicture}[font=\small, node distance=10mm]
\node[draw, rounded corners, align=center, minimum width=3.5cm, minimum height=8mm] (logs) {Production logs\\(sampled, labeled)};
\node[draw, rounded corners, align=center, minimum width=3.5cm, minimum height=8mm, right=of logs] (datasets) {Eval datasets\\golden + incidents + slices};
\node[draw, rounded corners, align=center, minimum width=3.5cm, minimum height=8mm, right=of datasets] (runner) {Eval runner\\(offline)};
\node[draw, rounded corners, align=center, minimum width=3.5cm, minimum height=8mm, right=of runner] (report) {Report + gates\\CI/CD decision};

\draw[->] (logs) -- node[above]{curate} (datasets);
\draw[->] (datasets) -- node[above]{execute} (runner);
\draw[->] (runner) -- node[above]{aggregate} (report);
\end{tikzpicture}
\end{llmfigbox}
