...
\begin{tikzpicture}[
  ...
  hot/.style={draw=none, rounded corners=4pt, align=center, minimum width=32mm, minimum height=10mm, fill=red!20, inner sep=4pt},
...
\node[hot, below=12mm of route] (hotidx) {Hot Tier Index\\(in-memory / GPU)};
...
% Hot tier lookup
\draw[arr] (route) -- node[pos=0.5,right,sloped,font=\footnotesize]{first lookup} (hotidx);

% Hit path from hot to merge (green for success)
\draw[arrhit] (hotidx) -- node[pos=0.5,left,font=\footnotesize\bfseries]{hit} (merge);
...
% Legend box - moved to bottom right to avoid overlap
\node[draw=black!40, rounded corners=4pt, thick, align=left, anchor=south east, fill=white!95, inner sep=4pt, text width=7cm] at ($(gen.south east)+(2mm,-2mm)$) (leg) {%
