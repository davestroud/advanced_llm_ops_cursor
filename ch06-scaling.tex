...
% Regions (left-to-right placement) - with colors
\node[region, fill=blue!15, anchor=west] (na) at ($(frame.west)+(0.6,0)$) {North America\\(NA cluster)};
\node[region, fill=green!15] (eu) at ($(frame.center)+(0,1.0)$) {Europe\\(EU cluster)};
\node[region, fill=orange!15, anchor=east] (ap) at ($(frame.east)+(-0.6,0)$) {Asia-Pacific\\(APAC cluster)};
...
% Compliance / geofencing note - moved to lower left, separated from main content
\node[draw=black!40, rounded corners=4pt, align=left, anchor=north west, fill=white!95, inner sep=4mm]
at ($(frame.south west)+(0.2,-0.5)$) {\small
\textbf{Notes}\\[2pt]
