...
% Hot Tier Index label above the box
\node[above=4mm of hotidx, font=\footnotesize, text=black!70] (hotlabel) {(in-memory / GPU)};
...
% "first lookup" label positioned just below the (in-memory / GPU) label
\node[below=2mm of hotlabel, font=\footnotesize, text=black!70] {first lookup};

% Hit path from hot to merge (green for success) - label moved below Shard 2
\draw[arrhit] (hotidx) -- (merge);
\node[below=2mm of sh2, font=\footnotesize\bfseries, text=green!70!black] {hit};
