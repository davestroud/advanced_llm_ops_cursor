...
% Regions (left-to-right placement) - with colors, increased spacing
\node[region, fill=blue!15, anchor=west] (na) at ($(frame.west)+(0.4,0)$) {North America\\(NA cluster)};
\node[region, fill=green!15] (eu) at ($(frame.center)+(0,1.0)$) {Europe\\(EU cluster)};
\node[region, fill=orange!15, anchor=east] (ap) at ($(frame.east)+(-0.4,0)$) {Asia-Pacific\\(APAC cluster)};
...
% Failover paths (between regional clusters) - increased spacing for visibility
\draw[fail] (na.east) .. controls +(1.5,0.5) and +(-1.5,0.5) .. (eu.west) node[pos=0.52,above,sloped] {\small failover};
\draw[fail] (eu.east) .. controls +(1.5,-0.5) and +(-1.5,-0.5) .. (ap.west) node[pos=0.52,below,sloped] {\small failover};
