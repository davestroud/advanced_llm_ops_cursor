...
\begin{tikzpicture}[
  ...
  stage/.style={rounded corners=4pt, draw=black!70, thick, align=center, 
                minimum width=24mm, minimum height=18mm, inner sep=4pt, font=\small},
  note/.style={align=left, font=\footnotesize},
  >=Latex
]

% Color-coded stages - more compact spacing
...
% Feedback loop - more compact
\coordinate (looptop) at ($(canary.north)+(0,14mm)$);

\draw[->, thick, draw=teal!70!black, dashed]
  (canary.east) -- ++(6mm,0)
  |- node[pos=0.25, note, xshift=10mm, yshift=-5mm, text width=30mm] {%
    \textbf{Feedback:}\\
...
\node[draw=cyan!70!black, fill=cyan!10, rounded corners=4pt, fit=(pre)(post)(night), 
      inner sep=5pt, label={[note, below=3mm]south:\textbf{Artifacts:} versioned datasets, judges \& prompts; metric code; thresholds; HTML/JSON reports}, thick] (bundle) {};

% Small callouts - positioned with adequate space and colored backgrounds
\node[note, above=4mm of pre, fill=yellow!15, rounded corners=3pt, inner sep=2pt, draw=yellow!50, 
      text width=26mm, align=center] {regex PII / inj.; schema};
\node[note, above=4mm of post, fill=yellow!15, rounded corners=3pt, inner sep=2pt, draw=yellow!50,
      text width=28mm, align=center] {helpfulness, groundedness; McNemar, bootstrap};
\node[note, above=4mm of night, fill=yellow!15, rounded corners=3pt, inner sep=2pt, draw=yellow!50,
      text width=30mm, align=center] {red-team, NLI contradictions, fairness slices};
\node[note, above=4mm of canary, fill=yellow!15, rounded corners=3pt, inner sep=2pt, draw=yellow!50,
      text width=28mm, align=center] {online evals; SLO monitors; rollback};

% Legend - positioned below bundle
\node[stage, fill=gray!15, draw=gray!70!black, below=16mm of bundle, minimum width=45mm, minimum height=16mm] (legend) {\textbf{Legend}};
\node[note, below=2mm of legend, text width=45mm] {
