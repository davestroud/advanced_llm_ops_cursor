...
\begin{tikzpicture}[
  ...
  stage/.style={rounded corners=6pt, draw=black!30, line width=0.8pt, align=center, 
                minimum width=52mm, minimum height=36mm, inner sep=10pt, font=\small},
...
\node[stage, fill=blue!20] (commit) {\small\bfseries{/commit}\\\small code, prompts, data diffs};
\node[stage, fill=green!20, right=of commit] (pre) {\small\bfseries{Pre-merge}\\\small \(\sim\)50--200 det. cases\\guards \& format checks};
\node[stage, fill=orange!20, right=of pre] (post) {\small\bfseries{Post-merge}\\\small 1--5k cases\\stat. gating vs. baseline};
\node[stage, fill=purple!20, right=of post] (night) {\small\bfseries{Nightly/Batch}\\\small 5k--20k cases\\adv., fairness, long ctx};
\node[stage, fill=red!20, right=of night] (canary) {\small\bfseries{Canary/Shadow}\\\small live slice\\drift, safety, SLOs};
...
\node[note, right=10mm of canary, fill=yellow!18, callout, 
      text width=42mm, align=left, anchor=west, font=\small] (feedback) {%
    \textbf{Feedback:}\\
...
% Baseline/bundle box under pre/post/night - clearer and more visible
\node[draw=cyan!60, dashed, line width=1pt, fill=cyan!8, rounded corners=6pt, fit=(pre)(post)(night), 
      inner sep=14pt, label={[note, below=8mm, font=\small]south:\textbf{Artifacts:} versioned datasets, judges \& prompts; metric code; thresholds; HTML/JSON reports}] (bundle) {};

% Callouts - larger text, better spacing, and clearer styling
\node[note, above=10mm of pre, fill=yellow!18, callout, 
      text width=48mm, align=center, font=\small] {regex PII / inj.; schema};
\node[note, above=10mm of post, fill=yellow!18, callout,
      text width=52mm, align=center, font=\small] {helpfulness, groundedness; McNemar, bootstrap};
\node[note, above=10mm of night, fill=yellow!18, callout,
      text width=56mm, align=center, font=\small] {red-team, NLI contradictions, fairness slices};
\node[note, above=10mm of canary, fill=yellow!18, callout,
      text width=50mm, align=center, font=\small] {online evals; SLO monitors; rollback};

% Legend - larger and better spaced with clearer styling
\node[stage, fill=gray!18, below=28mm of bundle, minimum width=70mm, minimum height=24mm, draw=black!30, line width=0.8pt] (legend) {\textbf{Legend}};
\node[note, below=6mm of legend, text width=70mm, font=\small, align=left] {
