...
\begin{tikzpicture}[
  ...
  stage/.style={rounded corners=5pt, draw=none, align=center, 
                minimum width=34mm, minimum height=24mm, inner sep=6pt, font=\small},
  note/.style={align=left, font=\small},
  >=Latex
]

% Color-coded stages - much larger boxes with bigger text
\node[stage, fill=blue!20] (commit) {/commit\\\small code, prompts, data diffs};
\node[stage, fill=green!20, right=of commit] (pre) {Pre-merge\\\small \(\sim\)50--200 det. cases\\guards \& format checks};
\node[stage, fill=orange!20, right=of pre] (post) {Post-merge\\\small 1--5k cases\\stat. gating vs. baseline};
\node[stage, fill=purple!20, right=of post] (night) {Nightly/Batch\\\small 5k--20k cases\\adv., fairness, long ctx};
\node[stage, fill=red!20, right=of night] (canary) {Canary/Shadow\\\small live slice\\drift, safety, SLOs};

% Arrows forward - thicker
\draw[->, very thick, line width=1.5pt, draw=black!70] (commit) -- (pre);
\draw[->, very thick, line width=1.5pt, draw=black!70] (pre) -- (post);
\draw[->, very thick, line width=1.5pt, draw=black!70] (post) -- (night);
\draw[->, very thick, line width=1.5pt, draw=black!70] (night) -- (canary);

% Feedback loop - much more space
\coordinate (looptop) at ($(canary.north)+(0,26mm)$);

\draw[->, very thick, line width=1.5pt, draw=teal!70!black, dashed]
  (canary.east) -- ++(12mm,0)
  |- node[pos=0.25, note, xshift=16mm, yshift=-10mm, text width=42mm, font=\normalsize] {%
    \textbf{Feedback:}\\
...
\node[draw=none, dashed, fill=cyan!5, rounded corners=5pt, fit=(pre)(post)(night), 
      inner sep=10pt, label={[note, below=5mm]south:\textbf{Artifacts:} versioned datasets, judges \& prompts; metric code; thresholds; HTML/JSON reports}, opacity=0.5] (bundle) {};

% Callouts - much larger text and better spacing
\node[note, above=6mm of pre, fill=yellow!15, rounded corners=4pt, inner sep=4pt, draw=none, 
      text width=36mm, align=center, font=\small] {regex PII / inj.; schema};
\node[note, above=6mm of post, fill=yellow!15, rounded corners=4pt, inner sep=4pt, draw=none,
      text width=40mm, align=center, font=\small] {helpfulness, groundedness; McNemar, bootstrap};
\node[note, above=6mm of night, fill=yellow!15, rounded corners=4pt, inner sep=4pt, draw=none,
      text width=42mm, align=center, font=\small] {red-team, NLI contradictions, fairness slices};
\node[note, above=6mm of canary, fill=yellow!15, rounded corners=4pt, inner sep=4pt, draw=none,
      text width=38mm, align=center, font=\small] {online evals; SLO monitors; rollback};

% Legend - much larger and better spaced
\node[stage, fill=gray!15, below=20mm of bundle, minimum width=55mm, minimum height=18mm] (legend) {\textbf{Legend}};
