...
\begin{tikzpicture}[
  ...
  stage/.style={rounded corners=5pt, draw=black!70, very thick, align=center, 
                minimum width=40mm, minimum height=28mm, inner sep=8pt, font=\normalsize},
  note/.style={align=left, font=\normalsize},
  >=Latex
]

% Color-coded stages - much larger boxes with bigger text
\node[stage, fill=blue!20, draw=blue!70!black] (commit) {/commit\\\normalsize code, prompts, data diffs};
\node[stage, fill=green!20, draw=green!70!black, right=of commit] (pre) {Pre-merge\\\normalsize \(\sim\)50--200 det. cases\\guards \& format checks};
\node[stage, fill=orange!20, draw=orange!70!black, right=of pre] (post) {Post-merge\\\normalsize 1--5k cases\\stat. gating vs. baseline};
\node[stage, fill=purple!20, draw=purple!70!black, right=of post] (night) {Nightly/Batch\\\normalsize 5k--20k cases\\adv., fairness, long ctx};
\node[stage, fill=red!20, draw=red!70!black, right=of night] (canary) {Canary/Shadow\\\normalsize live slice\\drift, safety, SLOs};
...
\node[draw=cyan!70!black, dashed, fill=cyan!5, rounded corners=5pt, fit=(pre)(post)(night), 
      inner sep=12pt, label={[note, below=6mm]south:\textbf{Artifacts:} versioned datasets, judges \& prompts; metric code; thresholds; HTML/JSON reports}, very thick, opacity=0.5] (bundle) {};

% Callouts - much larger text and better spacing
\node[note, above=8mm of pre, fill=yellow!15, rounded corners=4pt, inner sep=6pt, draw=yellow!50, 
      text width=42mm, align=center, font=\normalsize] {regex PII / inj.; schema};
\node[note, above=8mm of post, fill=yellow!15, rounded corners=4pt, inner sep=6pt, draw=yellow!50,
      text width=46mm, align=center, font=\normalsize] {helpfulness, groundedness; McNemar, bootstrap};
\node[note, above=8mm of night, fill=yellow!15, rounded corners=4pt, inner sep=6pt, draw=yellow!50,
      text width=48mm, align=center, font=\normalsize] {red-team, NLI contradictions, fairness slices};
\node[note, above=8mm of canary, fill=yellow!15, rounded corners=4pt, inner sep=6pt, draw=yellow!50,
      text width=44mm, align=center, font=\normalsize] {online evals; SLO monitors; rollback};

% Legend - much larger and better spaced
\node[stage, fill=gray!15, draw=gray!70!black, below=24mm of bundle, minimum width=65mm, minimum height=22mm] (legend) {\textbf{Legend}};
\node[note, below=4mm of legend, text width=65mm, font=\normalsize] {
