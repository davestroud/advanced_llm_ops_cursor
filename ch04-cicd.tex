...
\begin{tikzpicture}[
  ...
  stage/.style={rounded corners=4pt, draw=black!70, thick, align=center, 
                minimum width=28mm, minimum height=20mm, inner sep=5pt, font=\small},
...
% Arrows forward - thicker and colored
...
% Feedback loop - larger and more visible
\coordinate (looptop) at ($(canary.north)+(0,18mm)$);
\coordinate (loopbot) at ($(commit.south)-(0,18mm)$);

\draw[->, very thick, draw=teal!70!black, dashed]
  (canary.east) -- ++(8mm,0)
  |- node[pos=0.25, note, xshift=12mm, yshift=-6mm, text width=35mm] {%
    \textbf{Feedback:}\\
...
% Baseline/bundle box under pre/post/night - colored
\node[draw=cyan!70!black, fill=cyan!10, rounded corners=4pt, fit=(pre)(post)(night), 
      inner sep=6pt, label={[note, below=4mm]south:\textbf{Artifacts:} versioned datasets, judges \& prompts; metric code; thresholds; HTML/JSON reports}, thick] (bundle) {};

% Small callouts - positioned with more space and colored backgrounds
\node[note, above=5mm of pre, fill=yellow!15, rounded corners=3pt, inner sep=3pt, draw=yellow!50, 
      text width=32mm, align=center] {regex PII / inj.; schema};
\node[note, above=5mm of post, fill=yellow!15, rounded corners=3pt, inner sep=3pt, draw=yellow!50,
      text width=38mm, align=center] {helpfulness, groundedness; McNemar, bootstrap};
\node[note, above=5mm of night, fill=yellow!15, rounded corners=3pt, inner sep=3pt, draw=yellow!50,
      text width=40mm, align=center] {red-team, NLI contradictions, fairness slices};
\node[note, above=5mm of canary, fill=yellow!15, rounded corners=3pt, inner sep=3pt, draw=yellow!50,
      text width=35mm, align=center] {online evals; SLO monitors; rollback};

% Legend - larger and colored
\node[stage, fill=gray!15, draw=gray!70!black, below=22mm of post, minimum width=50mm, minimum height=18mm] (legend) {\textbf{Legend}};
\node[note, below=3mm of legend, text width=50mm] {
