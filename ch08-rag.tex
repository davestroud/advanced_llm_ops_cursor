...
A production RAG system consists of \emph{offline} components (ingestion, cleaning, chunking, embedding, indexing) and \emph{online} components (query processing, retrieval, reranking, context construction, and generation). Fig.~\ref{fig:ch08_rag_querytime} summarizes the online query-time loop.
...
Finally, always separate passages clearly (delimiters, document IDs, and optional ``BEGIN/END SOURCE'' markers) to reduce accidental blending. The generator should be able to unambiguously associate each claim with a specific evidence block, which is a prerequisite for reliable citations.
