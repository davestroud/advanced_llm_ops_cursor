...
A production RAG system consists of \emph{offline} components (ingestion, cleaning, chunking, embedding, indexing) and \emph{online} components (query processing, retrieval, reranking, context construction, and generation). Figure~\ref{fig:rag-querytime} summarizes the online query-time loop.

\begin{figure}[htbp]
\centering
\begin{llmfigbox}
\begin{tikzpicture}[node distance=2.6cm, auto,
    block/.style={rectangle, draw, fill=blue!10, rounded corners, minimum height=1cm, minimum width=2.8cm, align=center},
    arrow/.style={->, thick}
]
\node[block] (q) {User query};
\node[block, right of=q] (qp) {Query processing\\ \footnotesize rewrite, filters};
\node[block, right of=qp] (ret) {Retrieval\\ \footnotesize dense/sparse/hybrid};
\node[block, right of=ret] (rr) {Rerank \& filter\\ \footnotesize cross-encoder, RRF};
\node[block, right of=rr] (pack) {Context packer\\ \footnotesize dedupe, trim, cite};
\node[block, right of=pack] (gen) {LLM generation};
\node[block, below of=gen, node distance=1.6cm] (ans) {Answer + citations};
...
\end{tikzpicture}
\end{llmfigbox}
\caption{Query-time RAG pipeline. Retrieval supplies evidence; reranking and filtering improve top-$k$ quality; the context packer constructs a token-budgeted prompt with source identifiers; the generator produces an answer that can cite the retrieved chunks.}
\label{fig:rag-querytime}
\end{figure}

The remainder of this section walks through the main building blocks and the operational concerns attached to each.

\subsection{Document Ingestion}
Document ingestion collects, normalizes, and enriches knowledge sources to populate the retrieval corpus. In production, ingestion is not a one-time job: it is a continuously running subsystem responsible for freshness, quality, and provenance \cite{Pryon2023}. Typical steps include:
\begin{itemize}
    \item \textbf{Acquisition}: file sync, crawlers, API connectors, and event streams.
    \item \textbf{Normalization}: HTML/PDF extraction, encoding fixes, language detection, and format conversion.
    \item \textbf{Quality gates}: deduplication, spam filtering, and minimum-content checks.
    \item \textbf{Metadata enrichment}: timestamps, source identifiers, access control tags, and reliability scores.
\end{itemize}
For \ishtar{}, ingestion emphasizes timeliness and source reliability (e.g., prioritizing official bulletins and trusted news wires for critical facts).

\subsection{Chunking and Passage Construction}
RAG retrieval operates on \emph{chunks}, not entire documents. Chunking should preserve semantic coherence and include enough local context to support answer synthesis. Common strategies include structural chunking (by headings/sections), token-window chunking (fixed size with overlap), or hybrid approaches that respect both structure and size constraints. Chunking decisions directly affect retrieval quality and prompt efficiency, and should be treated as versioned configuration alongside the embedding model.

\subsection{Embedding Generation}
Each chunk is mapped to a vector representation so that semantic similarity search becomes possible. Dense retrieval models (e.g., dual encoders) learn representations where semantically related texts lie nearby in vector space \cite{Karpukhin2020DPR}. In practice:
\begin{itemize}
    \item Use \textbf{the same embedding model} for both document and query embeddings.
    \item Batch embedding jobs to reduce cost and improve throughput.
    \item Recompute embeddings when either the embedding model changes or chunking logic changes.
\end{itemize}
...
Raw data must be normalized into consistent, high-quality text:
\begin{itemize}
    \item strip boilerplate (navigation, scripts, ads),
    \item convert formats (PDF $\rightarrow$ text; HTML $\rightarrow$ text),
    \item detect language and preserve language metadata,
    \item remove obvious noise (very short pages, duplicated boilerplate).
\end{itemize}
Preprocessing quality often dominates downstream retrieval quality, especially in heterogeneous enterprise corpora.
...
Sharding distributes vectors across nodes. When possible, use \emph{logical} partitions (e.g., by region, product line, or tenant) to reduce the candidate search space and support independent scaling. In \ishtar{}, geographic partitioning is a natural fit: a query about Italy should not search a shard containing unrelated regions unless explicitly required.

\subsection{Replication and High Availability}
Replication improves both availability and throughput. Production systems should test failure scenarios (node loss, network partitions) and monitor p95/p99 retrieval latency to trigger scale-out before user-visible degradation.

\subsection{Persistence, Backups, and Re-indexing}
A vector index is rarely the only copy of truth. Preserve raw source documents and ingestion metadata so the index can be rebuilt when embeddings or chunking logic changes. Managed systems typically persist automatically; self-managed FAISS deployments require explicit snapshotting \cite{Johnson2019FAISS}.

\subsection{Metadata Filters and Hybrid Queries}
Metadata filters enable time-aware retrieval, tenant isolation, and compliance constraints. Hybrid retrieval (dense + sparse) is increasingly common in production systems because it improves robustness for rare terms, codes, and proper nouns \cite{Lin2021SPLADE}.

\subsection{Security Baselines}
Treat the vector database as sensitive infrastructure:
\begin{itemize}
    \item enforce strong authentication and authorization,
    \item encrypt in transit and at rest,
    \item log query access for auditing,
    \item isolate tenants via indexes or metadata filters.
\end{itemize}
...
\setlength{\tabcolsep}{4pt}
\begin{tabularx}{\textwidth}{p{2.2cm}p{2.4cm}p{2.4cm}p{2.4cm}X}
\toprule
...
Ordering matters because many models exhibit positional bias. Common strategies include:
\begin{itemize}
...
\end{itemize}
Always separate passages clearly (delimiters, document IDs) to reduce accidental blending.

\subsection{Prompt Instructions and Abstention}
After listing context, include explicit instructions such as:
\begin{quote}\small
...
\begin{quote}\small
\textbf{SYSTEM:} You are an assistant that answers using provided evidence.\\
\textbf{QUESTION:} \emph{<user question>}\\
\textbf{CONTEXT:}\\
{[}1{]} <title / source / date>\\
<passage>\\
{[}2{]} <title / source / date>\\
...
\end{itemize}
RAGAS provides a practical framework for scoring faithfulness and relevance using the retrieved passages and the generated answer \cite{RAGAS2023}.

\subsection{Citation Accuracy and Attribution}
If the system provides citations, measure:
\begin{itemize}
...
\end{itemize}
Citation evaluation is central to user trust: incorrect citations can be worse than no citations because they create a false sense of verification.
...
A practical latency model is:
\[
T_{\text{total}} \approx T_{\text{embed}} + T_{\text{retrieve}} + T_{\text{rerank}} + T_{\text{pack}} + T_{\text{generate}}.
\]
Instrument each term separately and track p95/p99, not just averages. In many systems, generation dominates; in retrieval-heavy systems (large corpora, multi-stage pipelines), vector search and reranking can become the bottleneck.

\subsection{Retrieval-Side Optimizations}
Common levers include:
\begin{itemize}
    \item \textbf{ANN tuning}: trade recall for latency by tuning HNSW/IVF parameters \cite{Malkov2018HNSW,Johnson2019FAISS}.
    \item \textbf{Metadata pruning}: filter by time/tenant/region to reduce the search space.
    \item \textbf{Caching}: cache frequent query embeddings and (when safe) retrieval results for repeated queries.
    \item \textbf{Batching}: batch query embeddings and reranker scoring when serving high throughput.
\end{itemize}

\subsection{Prompt and Context Optimizations}
Prompt cost scales with the number of input tokens. Reduce token load without sacrificing evidence quality:
\begin{itemize}
    \item pack fewer but higher-quality passages (rerank, dedupe),
    \item use extractive compression (answer-bearing sentence selection),
    \item summarize long passages when necessary,
    \item adopt templates that minimize boilerplate.
\end{itemize}
...
\subsubsection*{Summary}
Security in RAG is the combination of standard data-system controls (access control, encryption, audit logs) and AI-specific controls (prompt injection defenses, output leakage monitoring, provenance).
...
\begin{tikzpicture}[node distance=1.7cm, auto,
    block/.style={rectangle, draw, fill=gray!6, rounded corners, minimum height=1cm, minimum width=3.1cm, align=center},
    arrow/.style={->, thick}
]
...
\end{tikzpicture}
\end{llmfigbox}
\caption{\ishtar{} RAG pipeline at a glance. Each stage is observable and versioned: changes to ingestion, embeddings, retrieval, or prompting are evaluated via regression tests before release.}
\label{fig:ishtar-rag}
\end{figure}
...
\begin{tcolorbox}[
  title={\textbf{Best Practices Checklist for RAG Systems}},
  colback=violet!5,
...
\rowcolor{violet!15}
\textbf{Area} & \textbf{Practices} \\
\midrule
\rowcolor{violet!8}
\textbf{Corpus \& ingestion} &
Curate reliable sources; normalize and deduplicate; version chunking rules; enrich metadata (time, source, ACL). Monitor ingestion lag and parsing errors. \\
\rowcolor{violet!3}
\textbf{Embeddings \& indexing} &
Treat embedding model + chunking as a coupled versioned artifact. Plan for re-indexing and backfills. Benchmark ANN parameters against recall and latency SLOs. \\
\rowcolor{violet!8}
\textbf{Retrieval \& reranking} &
Use hybrid retrieval when queries mix jargon and natural language. Retrieve for recall, rerank for precision. Apply metadata filters for freshness and permissions. \\
\rowcolor{violet!3}
\textbf{Prompt construction} &
Pack a small, well-delimited evidence set. Include source identifiers and clear abstention instructions. Prefer extractive compression over truncation when possible. \\
\rowcolor{violet!8}
\textbf{Evaluation \& monitoring} &
Track Recall@k, faithfulness, and citation precision. Maintain regression sets and run them on every change. Log retrieved IDs, prompt sizes, and abstentions. \\
\rowcolor{violet!3}
\textbf{Security \& governance} &
Enforce access control at retrieval time, encrypt data, retain audit logs, and defend against prompt injection from retrieved text. \\
\bottomrule
\end{tabularx}
\end{tcolorbox}
