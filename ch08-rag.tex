...
\midrule
\rowcolor{violet!8}
\textbf{Keep Vector Stores Updated} & Keep vector stores updated with relevant, reliable content. Regularly ingest new data and remove outdated content to ensure answers remain current and accurate. \\
\rowcolor{violet!3}
\textbf{Use Hybrid Retrieval} & Use hybrid retrieval for balance between recall and precision. Combine dense semantic search with sparse keyword search for best coverage. \\
\rowcolor{violet!8}
\textbf{Always Cite Sources} & Always cite retrieved sources in responses. This builds user trust and forces the system to stay honest by grounding outputs in provided text. \\
\rowcolor{violet!3}
\textbf{Monitor Retrieval Quality} & Monitor retrieval quality alongside model performance. Log queries and retrieved documents, and periodically review relevance to identify improvement opportunities. \\
\end{tabularx}
\end{tcolorbox}
...
\begin{tcolorbox}[
  title={\textbf{Best Practices Checklist (Recap for Ishtar)}},
  colback=violet!5,
  colframe=violet!40!black,
  colbacktitle=violet!20,
  coltitle=black,
...
\midrule
\rowcolor{violet!8}
\textbf{Keep Vector Index Updated} & Keep the vector index updated with the latest reliable content; stale data can mislead the model. We have automated nightly jobs to remove or archive data older than X days or mark it as historical so that current queries favor recent info. \\
\rowcolor{violet!3}
\textbf{Use Hybrid Retrieval} & Use hybrid retrieval to balance recall and precision. Dense semantic search alone missed some exact figures, and pure keyword search missed synonyms – the combination was key. \\
\rowcolor{violet!8}
\textbf{Always Cite Sources} & Always cite sources in the output. It not only builds user trust\footnote{\url{https://www.pinecone.io}}, but also forces the system to stay honest (the model knows it should stick to provided text). \\
\rowcolor{violet!3}
\textbf{Monitor Retrieval Quality Continuously} & Continuously monitor retrieval quality. We log query + retrieved docs and occasionally manually review if the top docs were indeed relevant. If not, we tweak the embedding model or add stop-word removal or such. Similarly, monitor the model's usage of context – if it ignores context often, maybe the prompt needs adjustment. \\
\end{tabularx}
\end{tcolorbox}
...
\begin{tcolorbox}[
  title={\textbf{Best Practices Checklist (General)}},
  colback=violet!5,
  colframe=violet!40!black,
  colbacktitle=violet!20,
  coltitle=black,
