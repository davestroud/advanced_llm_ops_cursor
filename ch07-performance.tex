...
\begin{itemize}\setlength\itemsep{0.3em}
  \item \textbf{Quantization (INT8/FP8/4-bit):} $\sim$2$\times$ reduction in parameter memory; 1.3--2.0$\times$ higher tokens/s on supported hardware (model-dependent) \cite{Xiao2023SmoothQuant,Frantar2023GPTQ,Lin2023AWQ,NVIDIA2023Hopper}.
  \item \textbf{Operator fusion \& kernel tuning:} 10--30\% end-to-end speedups by reducing memory traffic and kernel-launch overhead \cite{Dao2022FlashAttention,Dao2023FlashAttention2}.
  \item \textbf{Dynamic/continuous batching:} 3--8$\times$ more tokens/s at steady state with a small ($<\!20$\,ms) batching window, especially under bursty traffic \cite{Kwon2023vLLM}.
  \item \textbf{KV-cache engineering (paged KV, prefix reuse):} 1.5--3.0$\times$ larger effective batches and reduced fragmentation under long contexts \cite{Kwon2023vLLM}.
  \item \textbf{Speculative decoding:} 1.4--2.0$\times$ lower latency on long generations via draft+verify \cite{Chen2023SpeculativeSampling}.
  \item \textbf{Response \& embedding caches:} can cut GPU load roughly in proportion to cache hit rate and reduce tail latency during bursts; validate correctness with TTLs, versioning, and guardrails \cite{langchain-cache}.
\end{itemize}
};
\end{tikzpicture}
\caption{Representative performance improvements from widely deployed techniques, based on published benchmarks and production deployments. Actual gains depend on model size, hardware, and traffic mix. See Sections~\ref{sec:perf-model}--\ref{sec:perf-system} and Section~\ref{sec:perf-prompt} for detailed discussions and implementation guidance.}
...
\end{axis}
\end{tikzpicture}
\caption{Illustrative example of latency improvements after batching, KV-cache engineering, and quantization. Absolute values vary by model, hardware, and traffic mix.}
...
\end{axis}
\end{tikzpicture}
\caption{Representative throughput scaling with batch size. Throughput rises rapidly at small $B$ as kernel-launch and memory overheads are amortized, then saturates once the accelerator becomes compute- or bandwidth-bound. In production, the optimal $B$ is constrained by latency SLOs and KV-cache memory.}
\label{fig:throughput-vs-batch}
\end{figure}
...
\begin{table}[htbp]
\centering
\caption{Illustrative kernel-time breakdown before vs.\ after hardware utilization tuning (e.g., kernel fusion, mixed precision, and overlap of I/O with compute). Percentages vary by model, sequence length, and runtime.}
...
\end{axis}
\end{tikzpicture}
\caption{Latency CDF with p50/p95/p99 markers. Shape and percentiles vary with model size, prompt length, batching policy, and traffic mix.}
...
\end{axis}
\end{tikzpicture}
\caption{Illustrative cost per 1k tokens across engines under a simple hourly-cost model (see Section~\ref{sec:perf-cost-per-token}). Absolute values depend on hardware pricing, utilization, and batch/sequence lengths.}
...
\begin{table}[htbp]
\centering
\caption{\ishtar{} optimization KPIs: before vs.\ after. Values are representative of the case study scenario and depend on workload and hardware.}
...
A number of specialized inference engines have been developed to serve LLMs efficiently. Here we compare three widely used runtimes for decoder-based LLM inference in the cloud: vLLM, Hugging Face Text Generation Inference (TGI), and NVIDIA TensorRT-LLM. Each runtime embodies a distinct approach to batching, memory management, and kernel optimization. Table~\ref{tab:runtimes} provides a qualitative comparison, focused on decoder-only use cases since they remain the most challenging to serve at scale. Table~\ref{tab:runtimes} provides a qualitative comparison drawn from public documentation and reported benchmarks; validate with your own workloads.
