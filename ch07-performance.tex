...
This chapter presents performance optimization techniques at four layers: (i) \emph{model-level} methods (quantization, pruning, and distillation), (ii) \emph{inference-engine} optimizations (kernel fusion, efficient attention implementations, and KV-cache management), (iii) \emph{system-level} techniques (batching, caching, asynchronous processing, request scheduling, and advanced decoding), and (iv) \emph{prompt-level} strategies that reduce token and retrieval overhead. The chapter closes with benchmarking guidance and \ishtar{} case studies that connect these methods to measurable outcomes such as TTFT, tokens/s, tail latency, and cost per request.
...
\begin{itemize}
    ...
    \item \textbf{Skyrocketing costs}: Inefficient use of hardware---such as idle GPUs, oversized models for the workload, or suboptimal batching---can drive cloud bills sharply upward. Self-hosting large models without careful optimization can cost far more than using a managed API; one industry analysis found that naive deployments cost \emph{far more than the API bill}, whereas optimized designs reduced costs by up to 90\%\footnote{\url{https://medium.com/}}. In a cloud environment, every millisecond and GPU-hour saved directly translates into cost savings. Large-scale services have long recognized that high tail latencies can dramatically undermine user satisfaction.
...
\centering
\begin{tikzpicture}
\node[draw, rounded corners, thick, fill=gray!5, align=left, inner sep=4mm, text width=0.88\linewidth]{
\textbf{Practical gains from common optimizations}\\[2pt]
\begin{itemize}\setlength\itemsep{0.3em}
  \item \textbf{Quantization (INT8/FP8):} $\sim$2$\times$ reduction in parameter memory; 1.3--2.0$\times$ throughput with TensorRT-LLM on H100 (model-dependent).
  \item \textbf{Operator fusion \& kernel tuning:} 10--30\% end-to-end speedups by reducing memory traffic and launch overheads.
  \item \textbf{Dynamic batching:} 3--8$\times$ more tokens/s at steady state with a small ($<\!20$\,ms) batching window (Section~6.4).
  \item \textbf{KV-cache engineering (paged KV, prefix reuse):} 1.5--3.0$\times$ larger effective batches and reduced fragmentation under long contexts.
  \item \textbf{Speculative decoding:} 1.4--2.0$\times$ latency reduction on long generations by draft+verify (Section~6.3.4).
  \item \textbf{Response \& embedding caches:} 5--30\% GPU-hour savings under real traffic mix (Section~6.6).
\end{itemize}
};
\end{tikzpicture}
\caption{Representative performance improvements from widely deployed techniques, based on published benchmarks and production deployments. Actual gains depend on model size, hardware, and traffic mix. See Sections 7.1--7.4 for detailed discussions and citations.}
\label{fig:opt-gains-callout}
\end{figure}
...
\begin{tikzpicture}
\begin{axis}[
    width=0.9\linewidth, height=5.0cm,
    ybar=0.25,
    bar width=10pt,
...
\addplot+[fill=red!55] coordinates {(p50,780) (p95,1800) (p99,3200)};
\addlegendentry{Before (baseline)}

\addplot+[fill=blue!65] coordinates {(p50,420) (p95,950) (p99,1800)};
\addlegendentry{After (batching + KV + quant.)}

\end{axis}
\end{tikzpicture}
\caption{Illustrative latency improvements after batching, KV-cache engineering, and quantization. Replace with your measured values to reflect your deployment.}
\label{fig:latency-before-after}
\end{figure}



\section{Model-Level Optimization Techniques}
...
Lower numerical precision also enables the use of specialized hardware instructions (tensor cores) that can dramatically increase throughput. For example, an NVIDIA A100 GPU can perform up to 1248 INT4 tera-operations per second versus 312 TFLOPS for FP16, a $\sim$4$\times$ theoretical speedup by leveraging low-precision tensor cores \cite{Dettmers2022LLMint8,Xiao2023SmoothQuant}. In practice, quantization can deliver substantial benefits. For example, applying 4-bit post-training quantization (e.g., GPTQ) to a 13B-parameter model has enabled it to be served on a single GPU instead of two, with only minor accuracy degradation \cite{Frantar2023GPTQ}\footnote{\url{https://cloud.google.com}}. Smaller memory requirements also reduce energy consumption per query, which can be significant at scale. Quantization can apply not only to the model weights but also to auxiliary structures like the Transformer’s key/value (KV) cache---quantizing the KV cache to 8-bit precision can further lower memory usage and improve latency, albeit with a potential impact on accuracy.
...
\begin{itemize}
    ...
    \item Certain methods require an offline pre-quantization step, while others (e.g., \texttt{bitsandbytes} INT8/INT4) can quantize on-the-fly at runtime\footnote{\url{https://cloud.google.com}}.
\end{itemize}
...
\begin{table}
\centering
\caption{Comparison of common quantization schemes for large language models.}
...
$\sim$50\% vs.\ FP16 &
Moderate &
Minimal with calibration &
\makecell[l]{Supported on most modern NVIDIA/AMD GPUs;\\ widely available in inference frameworks.} \\
FP8 &
$\sim$50\% vs.\ FP16 &
Moderate--High (on supported HW) &
Minimal--Low &
\makecell[l]{Native support on NVIDIA H100; partial support on\\ Hopper architecture; not supported on A100.} \\
4-bit &
$\sim$75\% vs.\ FP16 &
High &
Low--Moderate, task dependent &
\makecell[l]{Supported via software toolkits (e.g., GPTQ, bitsandbytes)\\ on NVIDIA GPUs; performance varies by architecture.} \\
Mixed Precision &
Variable &
Variable &
Tunable trade-off &
\makecell[l]{Framework-dependent; often combines FP16, FP8, and\\ INT8 in compatible layers.} \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{sidewaystable}
\centering
\caption{Summary of model compression and adaptation techniques for LLM deployment.}
...
Speed impact &
Quality impact &
Hardware / framework notes and when to use \\
\midrule
...
\makecell[l]{INT8 broadly supported; FP8 on H100; 4-bit via GPTQ/AWQ/bitsandbytes.\\Use to fit models on fewer/smaller GPUs and cut cost/latency with small accuracy loss.} \\
\addlinespace[2pt]
...
\makecell[l]{Structured: tangible latency gains;\\Unstructured: speedups only with sparse kernels.} &
Low to moderate if retrained; risk rises with aggressive sparsity. &
\makecell[l]{Structured works well on dense runtimes; unstructured needs sparse-friendly libraries/HW.\\Use to trim over-parameterized models; fine-tune post-pruning.} \\
\addlinespace[2pt]
...
\makecell[l]{Framework-agnostic. Use to create compact production models with teacher-like behavior and lower TCO.} \\
\addlinespace[2pt]
...
\makecell[l]{Excellent for task adaptation on limited GPUs; pair with quantization/distillation for deployment gains.} \\
\addlinespace[2pt]
...
\makecell[l]{Requires architectural support; best applied during pretraining or major refactors.} \\
\addlinespace[2pt]
...
\makecell[l]{Needs specialized runtimes/cluster comms. Use to scale parameters without proportional compute growth.} \\
\addlinespace[2pt]
...
\makecell[l]{Pairs well with long contexts and high concurrency; test for degradation on long-range tasks.} \\
\bottomrule
\end{tabularx}
\end{sidewaystable}
...
% A100 (illustrative)
\addplot+[fill=blue!55] coordinates {(FP16,1.00) (INT8,1.35) (FP8,1.00) (4-bit,1.55)};
\addlegendentry{A100 (normalized to FP16)}

% H100 (illustrative)
\addplot+[fill=orange!70] coordinates {(FP16,1.00) (INT8,1.40) (FP8,1.85) (4-bit,1.70)};
\addlegendentry{H100 (normalized to FP16)}

\end{axis}
\end{tikzpicture}
\caption{Illustrative throughput gains from quantization on A100/H100 (normalized to FP16). Replace values with your measured tokens/s to reflect your deployment. FP8 is native on H100; 4-bit performance varies by toolkit/model.}
\label{fig:quant-throughput-bars}
\end{figure}


\begin{table}[htbp]
\centering
\caption{Approximate parameter memory vs.\ precision for 13B and 70B models (weights only).}
\label{tab:mem-vs-precision}
\setlength{\tabcolsep}{6pt}
...
\makecell[l]{FP16 fits on A100-40GB/H100-80GB;\\ INT8/FP8 fits broadly (A10/L4);\\ 4-bit fits on mid-tier GPUs (e.g., 16--24\,GB).} \\
70B & $7.0\times 10^{10}$ & $\sim$140\,GB & $\sim$70\,GB & $\sim$35\,GB &
\makecell[l]{FP16 requires multi-GPU (tensor/pipeline parallel);\\ INT8/FP8 fits on single H100-80GB only for weights (KV extra);\\ 4-bit can fit weights on a single 48--80\,GB GPU (KV extra).} \\
\bottomrule
\end{tabularx}

\medskip
\footnotesize\emph{Notes}: Values exclude activations, KV cache, and framework overheads. For inference with long contexts or high concurrency, KV memory can exceed weights; see Eq.~(2.2) and Section~7.11.1 for sizing guidelines.
\end{table}
...
\begin{itemize}\setlength\itemsep{0.3em}
  ...
  \item \emph{KV cache (decoder-only)}: $\displaystyle M_{\text{KV}} \approx 2 \cdot L \cdot B \cdot S \cdot d \cdot b$, for $L$ layers, batch $B$, sequence length $S$, per-token hidden $d$ (or $H\cdot d_h$), bytes/elt $b$ (see Eq.~(2.2)).
  \item \emph{Rule of thumb}: For long contexts or high concurrency, $M_{\text{KV}}$ can exceed $M_{\text{weights}}$; plan headroom for paging/fragmentation (Section~7.11.1).
\end{itemize}
};
\end{tikzpicture}
\caption{Memory accounting at a glance. Use precision $b$ from Table~\ref{tab:quantization-comparison} and model dimensions from your architecture to size weights and KV cache before setting batch/sequence budgets.}
\label{fig:memory-cheatsheet}
\end{figure}



\section{Inference Engine Optimization}
...
\begin{table}[htbp]
\centering
\caption{Illustrative inference engine benchmarks (single GPU). Values are representative examples based on typical production deployments; actual performance depends on model size, hardware configuration, and workload characteristics.}
...
For \ishtar{}, reducing context size was important when summarizing sources. Early on, we fed the full text of articles (which could be thousands of tokens) along with the query to the model. We optimized this by summarizing or extracting only the relevant portions of the articles via a retriever component, which cut down the context length dramatically (more on this in \S7.5.3). We also ensured not to carry over too much conversation history: only the last few turns of Q/A were kept in the prompt for context, rather than the entire history, since older turns were often not needed.
...
\begin{table}[htbp]
\centering
\caption{Illustrative kernel-time breakdown before vs.\ after hardware utilization tuning. Replace with measured Nsight/PyTorch Profiler percentages.}
\label{tab:profiling-breakdown}
\setlength{\tabcolsep}{6pt}
\renewcommand{\arraystretch}{1.12}
\begin{tabularx}{\textwidth}{@{}l
...
\begin{itemize}
    ...
    \item Tokens/sec per GPU.
    \item Cost per 1,000 tokens.
\end{itemize}
Benchmarks should simulate realistic traffic patterns for \ishtar{}, including bursts during major events.
...
\begin{itemize}
\item \textbf{Latency distribution}: e.g., track the median latency (P50) and tail latencies (P95, P99) for requests. It is not enough to just look at the average; the 95th or 99th percentile might be much higher (indicating that some requests experience significantly worse performance). In many applications (especially user-facing ones), tail latency is critical \cite{Dean2013Tail}.
\item \textbf{Throughput}: measure tokens processed per second per GPU (tokens/sec/GPU), or requests per second if each request is similar. Tokens/sec is a convenient unit in LLM inference since it captures both input and output lengths normalized over time. Higher is better for throughput; but one must consider if those tokens are useful work (e.g., generating unneeded tokens is wasted throughput).
\item \textbf{Cost per 1,000 tokens}: if running in a cloud environment or even on-prem with an internal cost model, track how much it costs to serve 1k tokens. This incorporates both throughput and the cost of hardware instances. If a GPU costs \$X/hour and produces $Y$ tokens/sec, you can compute a \$ per token figure (as we formalize in Eq.~(7.4) later). Optimizations that improve throughput or allow cheaper hardware will reduce this cost.
\end{itemize}
...
% Example CDF (monotone increasing)
\addplot+[very thick, blue!70] table{
x   y
50   0.02
100  0.06
150  0.12
...
% Markers for p50, p95, p99
\addplot+[domain=0:1, samples=2, red!70, dashed, very thick] coordinates {(420,0) (420,0.5)};
\node[red!70] at (axis cs:435,0.52) {\small p50};

\addplot+[domain=0:1, samples=2, orange!90, dashed, very thick] coordinates {(2000,0) (2000,0.95)};
\node[orange!90] at (axis cs:2030,0.97) {\small p95};

\addplot+[domain=0:1, samples=2, black!70, dashed, very thick] coordinates {(3200,0) (3200,0.99)};
\node[black!70] at (axis cs:3230,1.01) {\small p99};

\end{axis}
\end{tikzpicture}
\caption{Latency CDF with p50/p95/p99 markers. Replace with measured points to reflect production baselines.}
...
\end{axis}
\end{tikzpicture}
\caption{Illustrative cost per 1k tokens across engines. Replace values with your economics (Section~7.3).}
\label{fig:cost-per-1k-by-engine}
\end{figure}
...
\emph{Fallback model policy.} A cascading design (2.7B fallback for trivial queries) yielded 3--5$\times$ faster answers on simple prompts; however, quality loss was noticeable on nuanced queries. We opted not to deploy globally, reserving the approach for future confidence-threshold routing (cf.\ §7.14.3).
...
\begin{itemize}
    \item Profile before optimizing—know your bottlenecks.
    \item Start with easy wins: batching, caching, prompt optimization.
    \item Use specialized runtimes for inference.
    \item Quantize and prune cautiously, testing quality impact.
    \item Automate performance regression tests in CI/CD.
\end{itemize}

Performance optimization is not a one-time task—it is an ongoing process that must adapt to changing workloads, hardware, and user expectations. For \ishtar{}, the combination of model, system, and prompt optimizations delivers the speed and cost-effectiveness needed for real-time, high-impact journalism.


\begin{table}[htbp]
\centering
\caption{IshtarAI optimization KPIs: before vs.\ after. Replace with exact measured values if available.}
\label{tab:ishtar-kpis}
\setlength{\tabcolsep}{6pt}
\renewcommand{\arraystretch}{1.12}
\begin{tabularx}{\textwidth}{@{}l
  >{\raggedleft\arraybackslash}p{2.6cm}
...
\begin{itemize}
\item \textbf{Profile before optimizing} --- Know your bottlenecks. Use profilers to identify whether compute, memory, or data transfer is limiting throughput. Focus efforts accordingly.
\item \textbf{Start with easy wins} --- Apply simple, high-impact optimizations first: enable batching, caching, and prompt simplification before more involved changes. These often yield immediate improvements with little downside.
\item \textbf{Use specialized runtimes} for inference --- Leverage high-performance inference engines (vLLM, TGI, DeepSpeed-Inference, TensorRT, etc.). They come with many optimizations out-of-the-box that are hard to replicate by hand.
...
where $B$ is batch size, $\Delta$ the generated length, $\tau_{\text{step}}$ the per-step time, and $\eta$ an efficiency factor accounting for utilization \cite{vllm-paper, fused-kernels}. Larger $B$ boosts throughput, but queueing delays increase latency.
...
\textbf{Cloud Implications:} Serving decoder-only models efficiently requires optimized runtimes (e.g., vLLM, TGI, TensorRT-LLM) with continuous batching to maintain utilization \cite{vllm-paper, tgi-whitepaper, tensorrt-llm}. Scaling options include vertical scaling (larger GPUs for longer contexts) and horizontal scaling (more replicas behind a load balancer). Trade-offs include: 
\begin{itemize}
...
These formulas link low-level engineering decisions (e.g., quantization, batching, runtime choice) to system-wide economics. For example, extending context length from 4k to 8k tokens doubles KV memory via Eq.~\eqref{eq:kv-total}, potentially halving feasible batch. Equation~\eqref{eq:throughput-main} then shows throughput reduction, and Eq.~\eqref{eq:cost-k} makes the resulting dollar impact explicit. This chain of reasoning was central in \ishtar{} capacity planning.

\begin{table}[htbp]
\centering
\caption{Comparison of Architectural Variants for Cloud Deployment}
...
\begin{table}[htbp]
\centering
\caption{Comparison of Architectural Variants for Cloud Deployment (Landscape)}
\label{tab:arch-comparison-landscape}
\renewcommand{\arraystretch}{1.15}
\setlength{\tabcolsep}{8pt}
\begin{tabularx}{1.05\linewidth}{p{4.0cm}X X X}
\toprule
\textbf{Dimension} & \textbf{Encoder-Only (BERT)} & \textbf{Decoder-Only (GPT)} & \textbf{Mixture-of-Experts (MoE)} \\
\midrule
\textbf{Latency / Throughput}
& Low latency; high throughput for short/fixed outputs; full-sequence parallelism.
& TTFT grows with prompt length; steady-state throughput in tokens/s; sequential decoding bottleneck.
& Sparsity lowers FLOPs/token; routing overhead and expert imbalance can increase tail latency. \\
\midrule
\textbf{Memory Behavior}
& Fixed per input (activations and attention over $T$); no cache growth.
& KV cache grows $\propto B \cdot T \cdot L$; can exceed weights for long contexts; paging/quantization essential.
& Experts sharded across devices; lower per-token compute/memory but larger aggregate parameters and router state. \\
\midrule
\textbf{Cloud Deployment Trade-offs}
& Cost-effective on CPUs or modest GPUs; aggressive batching/quantization; trivially replicated stateless workers.
& Needs optimized runtimes (vLLM, TGI, TensorRT-LLM); continuous batching; balance latency vs throughput; streaming helps.
& Benefits from expert parallelism on multi-GPU clusters; requires high-bandwidth interconnects; router-aware scheduling and complex autoscaling. \\
\bottomrule
\end{tabularx}
\end{table}
\end{landscape}
...
\label{sec:perf-runtimes}

\label{sec:inference-engines}

A number of specialized inference engines have been developed to serve LLMs efficiently. Here we compare three widely used runtimes for decoder-based LLM inference in the cloud: vLLM, Hugging Face Text Generation Inference (TGI), and NVIDIA TensorRT-LLM. Each runtime embodies a distinct approach to batching, memory management, and kernel optimization. Table~\ref{tab:runtimes} provides a qualitative comparison, focused on decoder-only use cases since they remain the most challenging to serve at scale.\footnote{Comparison based on public docs, blog posts, and community benchmarks; ideally substitute your internal benchmark data.}

\begin{table}[htbp]
  \centering
  \caption{Qualitative comparison of LLM serving runtimes (decoder-only focus).}
  \label{tab:runtimes}
  \renewcommand{\arraystretch}{1.15}
  \setlength{\tabcolsep}{6pt}
  \begin{tabular}{p{3.5cm}p{3.8cm}p{3.6cm}p{3.6cm}}
    \toprule
    & \textbf{vLLM} \cite{vllm-paper} & \textbf{Hugging Face TGI} \cite{tgi-whitepaper} & \textbf{TensorRT-LLM} \cite{tensorrt-llm} \\
    \midrule
...
In summary, vLLM \cite{vllm-paper} shines when maximum throughput and long context support are critical, but its complexity is higher. Hugging Face TGI \cite{tgi-whitepaper} is the most accessible option, offering fast setup, streaming, and multi-model support, though not always the fastest per-GPU. TensorRT-LLM \cite{tensorrt-llm} achieves the lowest latency and highest tokens/s on NVIDIA hardware, but requires offline engine compilation and is best suited to stable, repeatable workloads.
...
Implement a router that selects the smallest adequate model; escalate on uncertainty or policy triggers. In LangGraph, encode routing as guards on nodes; log route decisions for auditability. Expect $3$--$10\times$ cost improvements on mixed workloads with minimal quality loss \cite{model-cascade}.
...
\begin{enumerate}
  \item \textbf{Quantization} to 4-bit (GPTQ/AWQ), keeping task quality within tolerance \cite{gptq, awq}.
  \item \textbf{Runtime swap} to vLLM with continuous batching and PagedAttention \cite{vllm-paper}.
...
\label{sec:checklist-addendum}
\begin{itemize}
  \item \textbf{Measure first:} profile TTFT vs.\ $T_0$, tokens/s vs.\ $B$, and KV memory vs.\ precision.
  \item \textbf{Exploit precision:} FP16/BF16 by default; consider FP8/INT8; quantize KV where acceptable.
  \item \textbf{Adopt optimized runtimes:} vLLM/TGI/TensorRT-LLM; enable fused attention.
  \item \textbf{Continuously batch:} tune max batch delay; monitor effective batch size and tail latency.
  \item \textbf{Cache aggressively:} prompt and embedding caches with TTL/versioning; share across replicas.
  \item \textbf{Route smartly:} smallest-adequate model first; escalate on uncertainty; log routes.
  \item \textbf{Scale on tokens:} autoscale by tokens/s and queue depth; keep warm capacity for spikes.
  \item \textbf{Harden chains:} trace with LangSmith; define degraded modes and retry budgets.
\end{itemize}
