...
\begin{tikzpicture}[
  ...
  box/.style={draw, rounded corners, minimum width=24mm, minimum height=9mm, align=center, inner sep=3pt},
  agent/.style={box, fill=gray!10},
  ctrl/.style={box, fill=gray!20},
  decision/.style={diamond, draw, aspect=2.5, inner sep=3pt, align=center, font=\footnotesize},
  arrow/.style={-{Latex}, line width=0.7pt},
  title/.style={font=\bfseries\footnotesize}
]

% --- Panel containers using relative positioning --------------------------------
% Panel A (Rule-Based) - positioned relative to origin
\node[draw, rounded corners, inner sep=6mm, minimum width=6.2cm, minimum height=5.2cm] (panelA) {};
\node[title, anchor=west] at ([yshift=-3mm]panelA.north west) {(a) Rule-Based Orchestration};

% Panel B (Dynamic) - positioned to the right of Panel A
\node[draw, rounded corners, inner sep=6mm, minimum width=6.2cm, minimum height=5.2cm, right=7mm of panelA.east, anchor=west] (panelB) {};
\node[title, anchor=west] at ([yshift=-3mm]panelB.north west) {(b) Dynamic (LLM-driven) Orchestration};

% Panel C (Hierarchical) - positioned to the right of Panel B
\node[draw, rounded corners, inner sep=6mm, minimum width=6.2cm, minimum height=5.2cm, right=7mm of panelB.east, anchor=west] (panelC) {};
...
\node[agent, right=17mm of rb_ctrl] (rb_r) {Retrieval\\Agent};
\node[agent, right=15mm of rb_r] (rb_s) {Synthesis\\Agent};
\node[agent, right=15mm of rb_s] (rb_v) {Verification\\Agent};
\node[agent, right=15mm of rb_v] (rb_safe) {Safety\\Agent};
...
\node[decision, below=16mm of rb_v] (rb_cond) {Result\\OK?};
\draw[arrow] (rb_v) -- (rb_cond);
\draw[arrow] (rb_cond) -| node[pos=0.35, below, font=\tiny]{no} ([yshift=-2mm]rb_s.south);
\draw[arrow] (rb_cond) |- node[pos=0.35, right, font=\tiny]{yes} ([yshift=-2mm]rb_safe.south);

% --- (b) Dynamic (LLM-driven) -------------------------------------------------
\node[ctrl] (dyn_ctrl) at ([xshift=10mm, yshift=10mm]panelB.west) {LLM Orchestrator\\(Planner/Router)};
\node[agent, right=19mm of dyn_ctrl, yshift=11mm] (dyn_s1) {Synthesis\\Agent A};
\node[agent, right=19mm of dyn_ctrl, yshift=-11mm] (dyn_s2) {Synthesis\\Agent B};
\node[agent, right=15mm of dyn_s1] (dyn_v1) {Verification\\Agent};
\node[agent, right=15mm of dyn_s2] (dyn_safe) {Safety\\Agent};
...
\node[decision, below=14mm of dyn_v1] (dyn_cond) {Discrepancy?};
\draw[arrow] (dyn_v1) -- (dyn_cond);
\draw[arrow] (dyn_cond) -| node[pos=0.35, below, font=\tiny]{yes} ([yshift=-2mm]dyn_ctrl.south);
\draw[arrow] (dyn_cond) |- node[pos=0.35, right, font=\tiny]{no} ([yshift=-2mm]dyn_safe.south);

% --- (c) Hierarchical ---------------------------------------------------------
\node[ctrl] (hier_sup) at ([xshift=10mm, yshift=14mm]panelC.west) {Supervisor\\Agent};

\node[agent, below left=14mm and 9mm of hier_sup] (team1) {Team A\\Lead};
\node[agent, below right=14mm and 9mm of hier_sup] (team2) {Team B\\Lead};

\draw[arrow] (hier_sup) -- (team1);
\draw[arrow] (hier_sup) -- (team2);

\node[agent, below=14mm of team1, xshift=-15mm] (t1a) {Sub-Agent\\A1};
\node[agent, below=14mm of team1, xshift=15mm] (t1b) {Sub-Agent\\A2};
\node[agent, below=14mm of team2, xshift=-15mm] (t2a) {Sub-Agent\\B1};
\node[agent, below=14mm of team2, xshift=15mm] (t2b) {Sub-Agent\\B2};
...
\node[agent, right=13mm of team2] (merge) {Aggregation/\\Synthesis};
