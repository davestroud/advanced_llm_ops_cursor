\chapter{Multi-Agent Architectures and Orchestration}
\label{ch:multiagent}
\newrefsegment
...
We introduce core components (specialized agents, orchestration layer, shared memory, external tools), compare communication patterns, and analyze orchestration strategies (rule-based, dynamic, hierarchical).
We then address error handling, performance and cost governance, security controls, and conclude with an \ishtar{} case study demonstrating operationally grounded agent pipelines.
...
\begin{table}[t]
\centering
\caption{Monolithic vs.\ multi-agent LLM architectures: operational trade-offs.}
...
\centering
\begin{llmfigbox}
\begin{tikzpicture}[node distance=1.2cm and 1.7cm, every node/.style={font=\small}]
\node[draw, rounded corners, align=center, minimum width=2.8cm, minimum height=0.9cm] (ui) {User /\\Client};

\node[draw, rounded corners, align=center, minimum width=3.8cm, minimum height=1.0cm, below=of ui] (orch) {Orchestrator\\(routing, budgets, policy)};

\node[draw, rounded corners, align=center, minimum width=2.8cm, minimum height=0.9cm, left=of orch] (retr) {Retrieval\\Agent};
\node[draw, rounded corners, align=center, minimum width=2.8cm, minimum height=0.9cm, right=of orch] (synth) {Synthesis\\Agent};

\node[draw, rounded corners, align=center, minimum width=2.8cm, minimum height=0.9cm, below=of retr] (ver) {Verification\\Agent};
\node[draw, rounded corners, align=center, minimum width=2.8cm, minimum height=0.9cm, below=of synth] (safe) {Safety\\Agent};

\node[draw, rounded corners, align=center, minimum width=3.3cm, minimum height=0.9cm, below=of orch, yshift=-1.0cm] (mem) {Shared Memory\\(episodic + semantic)};

\node[draw, rounded corners, align=center, minimum width=3.3cm, minimum height=0.9cm, below=of mem] (tools) {External Tools\\(search, DBs, APIs)};
...
\end{tikzpicture}
\end{llmfigbox}
\caption{Control-plane and data-plane separation in an agentic LLM architecture. The orchestrator enforces routing decisions, budgets, and policies; agents implement scoped capabilities; shared memory provides a consistent state substrate; external tools provide grounded capabilities.}
...
\begin{table}[t]
\centering
\caption{Example interface contracts for core \ishtar{} agents. Contracts define inputs/outputs, invariants, and operational metrics.}
...
\begin{table}[t]
\centering
\caption{Communication patterns in agentic systems: trade-offs and typical use.}
...
\begin{tikzpicture}[
  ...
  agent/.style={box, fill=gray!10},
  ctrl/.style={box, fill=gray!20},
...
% --- Panel containers ---------------------------------------------------------
\node[draw, rounded corners, inner sep=4mm, fit={(0,0) (6,4.6)}] (panelA) {};
\node[draw, rounded corners, inner sep=4mm, fit={(7.2,0) (13.2,4.6)}] (panelB) {};
\node[draw, rounded corners, inner sep=4mm, fit={(14.4,0) (20.4,4.6)}] (panelC) {};

% Titles
\node[title, anchor=west] at ([yshift=-2mm]panelA.north west) {(a) Rule-Based Orchestration};
\node[title, anchor=west] at ([yshift=-2mm]panelB.north west) {(b) Dynamic (LLM-driven) Orchestration};
\node[title, anchor=west] at ([yshift=-2mm]panelC.north west) {(c) Hierarchical Orchestration};

% --- (a) Rule-Based -----------------------------------------------------------
% Orchestrator and fixed chain
\node[ctrl] (rb_ctrl) at ($(panelA.west)!0.2!(panelA.east)!.55!(panelA.north)$) {Orchestrator\\(Fixed Rules)};
\node[agent, right=15mm of rb_ctrl] (rb_r) {Retrieval Agent};
\node[agent, right=12mm of rb_r] (rb_s) {Synthesis Agent};
\node[agent, right=12mm of rb_s] (rb_v) {Verification Agent};
\node[agent, right=12mm of rb_v] (rb_safe) {Safety Agent};
...
\node[decision, below=11mm of rb_v] (rb_cond) {Result OK?};
\draw[arrow] (rb_v) |- (rb_cond.north);
\draw[arrow] (rb_cond) -| node[pos=0.25, below]{no} (rb_s.south);
\draw[arrow] (rb_cond) |- node[pos=0.25, right]{yes} (rb_safe.south);

% --- (b) Dynamic (LLM-driven) -------------------------------------------------
\node[ctrl] (dyn_ctrl) at ($(panelB.west)!0.17!(panelB.east)!.55!(panelB.north)$) {LLM Orchestrator\\(Planner/Router)};
\node[agent, right=20mm of dyn_ctrl, yshift=8mm] (dyn_s1) {Synthesis Agent A};
\node[agent, right=20mm of dyn_ctrl, yshift=-8mm] (dyn_s2) {Synthesis Agent B};
\node[agent, right=13mm of dyn_s1] (dyn_v1) {Verification};
\node[agent, right=13mm of dyn_s2] (dyn_safe) {Safety};
...
\node[decision, below=9mm of dyn_v1] (dyn_cond) {Discrepancy?};
\draw[arrow] (dyn_v1) |- (dyn_cond.north);
\draw[arrow] (dyn_cond) -| node[pos=0.25, below]{yes} (dyn_ctrl.south);
\draw[arrow] (dyn_cond) |- node[pos=0.25, right]{no} (dyn_safe.south);

% --- (c) Hierarchical ---------------------------------------------------------
\node[ctrl] (hier_sup) at ($(panelC.west)!0.25!(panelC.east)!.55!(panelC.north)$) {Supervisor Agent};

\node[agent, below left=10mm and 6mm of hier_sup] (team1) {Team A Lead};
\node[agent, below right=10mm and 6mm of hier_sup] (team2) {Team B Lead};

\draw[arrow] (hier_sup) -- (team1);
\draw[arrow] (hier_sup) -- (team2);

\node[agent, below=10mm of team1, xshift=-12mm] (t1a) {Sub-Agent A1};
\node[agent, below=10mm of team1, xshift=12mm] (t1b) {Sub-Agent A2};
\node[agent, below=10mm of team2, xshift=-12mm] (t2a) {Sub-Agent B1};
\node[agent, below=10mm of team2, xshift=12mm] (t2b) {Sub-Agent B2};
...
% aggregation back to supervisor
\node[agent, right=10mm of team2] (merge) {Aggregation/Synthesis};
...
\caption{Contrasting orchestration strategies in multi-agent systems. 
(\textbf{a}) \emph{Rule-Based}: fixed, testable pipelines with explicit branches (retrieval → synthesis → verification → safety). 
(\textbf{b}) \emph{Dynamic (LLM-driven)}: an LLM planner routes tasks at runtime, enabling adaptive flows and feedback. 
(\textbf{c}) \emph{Hierarchical}: a supervisor delegates to sub-teams (which may orchestrate further sub-agents) and aggregates results. 
...
\begin{table}[t]
\centering
\caption{Common failure modes in multi-agent systems and recommended orchestrator responses.}
...
\begin{table}[t]
\centering
\caption{Core performance metrics for agentic systems and their operational interpretation.}
...
\begin{table}[t]
\centering
\caption{Representative threats in agentic architectures and practical mitigations.}
...
\centering
\begin{llmfigbox}
\begin{tikzpicture}[node distance=1.1cm and 1.4cm, every node/.style={font=\small}]
% Top row
\node[draw, rounded corners, align=center, minimum width=2.2cm, minimum height=0.85cm] (ingest) {Ingestion};
\node[draw, rounded corners, align=center, minimum width=2.2cm, minimum height=0.85cm, right=of ingest] (retr) {Retrieval};
\node[draw, rounded corners, align=center, minimum width=2.2cm, minimum height=0.85cm, right=of retr] (synth) {Synthesis};

% Bottom row
\node[draw, rounded corners, align=center, minimum width=2.2cm, minimum height=0.85cm, below=of synth] (verify) {Verification};
\node[draw, rounded corners, align=center, minimum width=2.2cm, minimum height=0.85cm, left=of verify] (safety) {Safety};
\node[draw, rounded corners, align=center, minimum width=2.2cm, minimum height=0.85cm, left=of safety] (deliver) {Delivery};

% Orchestrator
\node[draw, rounded corners, align=center, minimum width=2.8cm, minimum height=0.9cm, below=of retr, xshift=-3.2cm] (orch) {Orchestrator\\(router + budgets)};

% Arrows with artifacts
\draw[->] (ingest) -- node[above]{chunks + metadata} (retr);
\draw[->] (retr) -- node[above]{context pack} (synth);
\draw[->] (synth) -- node[right]{draft + citations} (verify);
\draw[->] (verify) -- node[below]{verified answer} (safety);
\draw[->] (safety) -- node[below]{user response} (deliver);

% Orchestrator control edges (dashed)
\draw[->, dashed] (orch) -- (retr);
\draw[->, dashed] (orch) -- (synth);
\draw[->, dashed] (orch) -- (verify);
\draw[->, dashed] (orch) -- (safety);
\end{tikzpicture}
\end{llmfigbox}
\caption{\ishtar{} multi-agent pipeline with explicit handoffs and artifacts. The orchestrator enforces workflow order, budgets, and policy gates; retrieval produces a context pack that is preserved through synthesis and verification to support auditable citations.}
...
\textbf{Parallel and Background Activities:} While steps 2–7 were sequential, note that ingestion (step 0) runs in parallel continuously. Also, if multiple queries come in, orchestrator can handle them concurrently (spinning separate threads or tasks). If orchestrator was more advanced, some steps could be parallel: e.g., if multiple synthesis sub-tasks, etc., but in the straightforward pipeline they were sequential. The figure embedded above conceptually showed a router orchestrating agents; in Ishtar's case it would be retrieval → synthesis → verification → safety → (optional translation) in sequence for one query. We can imagine a similar diagram: User Query $\rightarrow$ Orchestrator $\rightarrow$ Retrieval Agent $\rightarrow$ (searches Vector Store) $\rightarrow$ returns context pack $\rightarrow$ Orchestrator $\rightarrow$ Synthesis Agent $\rightarrow$ returns draft $\rightarrow$ Orchestrator $\rightarrow$ Verification Agent $\rightarrow$ (calls external sources if needed) $\rightarrow$ returns verified info $\rightarrow$ Orchestrator $\rightarrow$ Safety Agent $\rightarrow$ returns safe answer $\rightarrow$ Orchestrator $\rightarrow$ (optional Translation Agent) $\rightarrow$ User. Meanwhile, the Ingestion Agent is off to the side updating the Vector Store continuously.
...
\ChecklistBox{
\begin{itemize}[leftmargin=1.5em, itemsep=3pt]
\item \textbf{Define Agent Roles Clearly:} Clearly define agent roles and responsibilities. Each agent should have a single well-defined purpose and scope of responsibility to avoid confusion or duplicated work.
\item \textbf{Choose Communication Patterns:} Choose communication patterns that match system scale and complexity. Use direct calls for simple systems; consider message buses or shared memory for larger, more complex agent ecosystems.
\item \textbf{Implement Robust Error Handling:} Implement robust error handling and fallbacks. Plan retries for transient issues and alternate paths for more serious failures. Use heartbeat checks or timeouts to detect stuck agents.
\item \textbf{Monitor Inter-Agent Performance:} Monitor inter-agent performance and optimize for minimal latency. Collect metrics on each agent's response time and identify bottlenecks. Use parallelism where possible and keep inter-agent communication payloads lean.
\item \textbf{Secure All Channels:} Secure all channels of communication. Apply authentication for agent communications, encrypt data in transit, and restrict each agent's access rights. Regularly audit who/what can invoke each agent.
\end{itemize}
}
