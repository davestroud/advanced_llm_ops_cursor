...
\begin{tikzpicture}[
  ...
  every node/.style={font=\small},
  user/.style={ellipse, draw=black!30, line width=0.8pt, fill=blue!10, rounded corners=5pt, align=center, inner sep=6pt, font=\small\bfseries},
  orchestrator/.style={rectangle, draw=black!40, line width=1pt, fill=teal!15, rounded corners=6pt, align=center, inner sep=8pt, font=\small\bfseries, minimum width=32mm, minimum height=14mm},
  agent/.style={rectangle, draw=black!30, line width=0.8pt, rounded corners=5pt, align=center, inner sep=6pt, font=\small\bfseries, minimum width=28mm, minimum height=11mm},
  store/.style={cylinder, draw=black!30, line width=0.8pt, fill=gray!10, shape aspect=0.4, align=center, inner sep=6pt, font=\small\bfseries, minimum width=24mm, minimum height=12mm},
  background/.style={rectangle, draw=black!20, line width=0.6pt, fill=gray!5, rounded corners=4pt, align=center, inner sep=4pt, font=\footnotesize\itshape, minimum width=26mm, minimum height=10mm},
  arrow/.style={-{Latex}, line width=1.5pt, color=black!75},
  arrowDashed/.style={-{Latex}, dashed, dash pattern=on 4pt off 3pt, line width=1pt, color=black!50},
  arrowThin/.style={-{Latex}, line width=1pt, color=black!60}
]
...
\node[agent, fill=orange!15, above right=12mm and 20mm of orch.north east] (retrieval) {Retrieval\\Agent};
...
\node[agent, fill=green!15, right=of safety, minimum width=26mm] (translation) {Translation\\Agent\\(optional)};
...
\node[background, left=of vectorstore, xshift=-5mm] (ingestion) {Ingestion\\Agent};

% External sources (for verification)
\node[background, above=18mm of verification] (externalsources) {External\\Sources};

% User response (final output)
\node[user, right=of translation, xshift=5mm] (userresponse) {User};

% Main sequential flow: User → Orchestrator → Agents → User
\draw[arrow] (user) -- node[above, font=\footnotesize, midway]{query} (orch);
\draw[arrow] (orch.north) .. controls +(0,8mm) and +(-8mm,0) .. node[left, font=\footnotesize, midway, xshift=-3mm]{invoke} (retrieval.west);
\draw[arrow] (retrieval) -- node[above, font=\footnotesize, midway]{context pack} (synthesis);
\draw[arrow] (synthesis) -- node[above, font=\footnotesize, midway]{draft} (verification);
\draw[arrow] (verification) -- node[above, font=\footnotesize, midway]{verified info} (safety);
\draw[arrow] (safety) -- node[above, font=\footnotesize, midway]{safe answer} (translation);
\draw[arrow] (translation) -- node[above, font=\footnotesize, midway]{translated} (userresponse);

% Return paths to orchestrator (dashed)
\draw[arrowDashed] (retrieval.south) .. controls +(0,-6mm) and +(8mm,0) .. node[right, font=\footnotesize, midway, xshift=2mm]{context pack} (orch.north east);
\draw[arrowDashed] (synthesis.south) .. controls +(0,-6mm) and +(12mm,0) .. (orch.north east);
\draw[arrowDashed] (verification.south) .. controls +(0,-6mm) and +(16mm,0) .. (orch.north east);
\draw[arrowDashed] (safety.south) .. controls +(0,-6mm) and +(20mm,0) .. (orch.north east);
\draw[arrowDashed] (translation.south) .. controls +(0,-6mm) and +(24mm,0) .. (orch.north east);

% Retrieval → Vector Store
\draw[arrowThin] (retrieval.north) -- node[left, font=\footnotesize, midway]{searches} (vectorstore.south);

% Ingestion → Vector Store (continuous/parallel)
\draw[arrowThin] (ingestion.east) -- node[above, font=\footnotesize, midway]{updates} (vectorstore.west);

% Verification → External Sources
\draw[arrowThin] (verification.north) -- node[right, font=\footnotesize, midway]{calls if needed} (externalsources.south);

% Background process indicator
\node[font=\footnotesize\itshape, color=black!50, above=4mm of ingestion] {Background Process};
