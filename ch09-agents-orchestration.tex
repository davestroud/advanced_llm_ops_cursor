...
\begin{tikzpicture}[
  ...
  box/.style={draw, rounded corners, minimum width=28mm, minimum height=10mm, align=center, inner sep=4pt},
  agent/.style={box, fill=gray!10},
  ctrl/.style={box, fill=gray!20},
  decision/.style={diamond, draw, aspect=2.8, inner sep=4pt, align=center, font=\small},
  arrow/.style={-{Latex}, line width=0.8pt},
  title/.style={font=\bfseries\small}
]

% --- Panel A: Rule-Based Orchestration ----------------------------------------
\node[draw, rounded corners, inner sep=8mm, minimum width=5.8cm, minimum height=6cm] (panelA) {};
\node[title, anchor=west] at ([yshift=-4mm]panelA.north west) {(a) Rule-Based Orchestration};

% Rule-Based flow (vertical layout for clarity)
\node[ctrl] (rb_ctrl) at ([xshift=12mm, yshift=12mm]panelA.west) {Orchestrator\\(Fixed Rules)};
\node[agent, below=14mm of rb_ctrl] (rb_r) {Retrieval\\Agent};
\node[agent, below=12mm of rb_r] (rb_s) {Synthesis\\Agent};
\node[agent, below=12mm of rb_s] (rb_v) {Verification\\Agent};
\node[agent, below=12mm of rb_v] (rb_safe) {Safety\\Agent};
...
\draw[arrow] (rb_cond) |- node[pos=0.4, above, font=\footnotesize]{yes} (rb_safe);
\draw[arrow] (rb_cond) |- node[pos=0.6, below, font=\footnotesize]{no} (rb_s);

% --- Panel B: Dynamic (LLM-driven) Orchestration -----------------------------
\node[draw, rounded corners, inner sep=8mm, minimum width=5.8cm, minimum height=6cm, right=8mm of panelA.east, anchor=west] (panelB) {};
\node[title, anchor=west] at ([yshift=-4mm]panelB.north west) {(b) Dynamic (LLM-driven) Orchestration};
...
\draw[arrow] (dyn_cond) |- node[pos=0.5, above, font=\footnotesize]{yes} ([xshift=5mm]dyn_ctrl.east);
\draw[arrow] (dyn_cond) |- node[pos=0.4, below, font=\footnotesize]{no} (dyn_safe);

% --- Panel C: Hierarchical Orchestration --------------------------------------
\node[draw, rounded corners, inner sep=8mm, minimum width=5.8cm, minimum height=6cm, right=8mm of panelB.east, anchor=west] (panelC) {};
\node[title, anchor=west] at ([yshift=-4mm]panelC.north west) {(c) Hierarchical Orchestration};
