...
\node[box, minimum width=28mm, minimum height=10mm] (ui) {User /\\Client};

\node[box, minimum width=32mm, minimum height=12mm, below=of ui] (orch) {Orchestrator\\(routing, budgets, policy)};

\node[box, minimum width=28mm, minimum height=10mm, left=of orch] (retr) {Retrieval\\Agent};
\node[box, minimum width=28mm, minimum height=10mm, right=of orch] (synth) {Synthesis\\Agent};

\node[box, minimum width=28mm, minimum height=10mm, below=of retr] (ver) {Verification\\Agent};
\node[box, minimum width=28mm, minimum height=10mm, below=of synth] (safe) {Safety\\Agent};

\node[box, minimum width=32mm, minimum height=10mm, below=of orch, yshift=-12mm] (mem) {Shared Memory\\(episodic + semantic)};

\node[box, minimum width=32mm, minimum height=10mm, below=of mem] (tools) {External Tools\\(search, DBs, APIs)};
...
\begin{tikzpicture}[
  ...
  box/.style={draw, rounded corners, align=center, inner sep=4pt},
  arrow/.style={-{Latex}, line width=0.8pt},
  arrowDashed/.style={-{Latex}, line width=0.8pt, dashed}
]
% Top row
\node[box, minimum width=24mm, minimum height=9mm] (ingest) {Ingestion};
\node[box, minimum width=24mm, minimum height=9mm, right=of ingest] (retr) {Retrieval};
\node[box, minimum width=24mm, minimum height=9mm, right=of retr] (synth) {Synthesis};

% Bottom row
\node[box, minimum width=24mm, minimum height=9mm, below=of synth] (verify) {Verification};
\node[box, minimum width=24mm, minimum height=9mm, left=of verify] (safety) {Safety};
\node[box, minimum width=24mm, minimum height=9mm, left=of safety] (deliver) {Delivery};

% Orchestrator
\node[box, minimum width=28mm, minimum height=10mm, below=of retr, xshift=-32mm] (orch) {Orchestrator\\(router + budgets)};

% Arrows with artifacts
\draw[arrow] (ingest) -- node[above, font=\footnotesize]{chunks + metadata} (retr);
\draw[arrow] (retr) -- node[above, font=\footnotesize]{context pack} (synth);
\draw[arrow] (synth) -- node[right, font=\footnotesize]{draft + citations} (verify);
\draw[arrow] (verify) -- node[below, font=\footnotesize]{verified answer} (safety);
\draw[arrow] (safety) -- node[below, font=\footnotesize]{user response} (deliver);

% Orchestrator control edges (dashed)
\draw[arrowDashed] (orch) -- (retr);
\draw[arrowDashed] (orch) -- (synth);
\draw[arrowDashed] (orch) -- (verify);
\draw[arrowDashed] (orch) -- (safety);
\end{tikzpicture}
\end{llmfigbox}
