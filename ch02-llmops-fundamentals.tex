...
\begin{figure}[t]
    ...
    \begin{tikzpicture}[
      ...
      >=Stealth,
      node distance=9mm,
      box/.style={draw, rounded corners, minimum width=35mm, minimum height=9mm, align=center, inner sep=4pt}
    ]
        % Nodes (grayscale-safe with different fills)
        \node[box, fill=gray!10] (input) {Input (tokens)};
        \node[box, fill=gray!15, below=of input] (mha) {Multi-Head Attention};
        \node[box, fill=gray!10, below=7mm of mha] (addnorm1) {Add \& Norm};
        \node[box, fill=gray!20, below=7mm of addnorm1] (ffn) {Feed-Forward Network};
        \node[box, fill=gray!10, below=7mm of ffn] (addnorm2) {Add \& Norm};
        \node[box, fill=gray!10, below=9mm of addnorm2] (output) {Hidden States / Next Block};

        % Arrows
        \draw[-{Latex}, line width=0.8pt] (input) -- (mha);
        \draw[-{Latex}, line width=0.8pt] (mha) -- (addnorm1);
        \draw[-{Latex}, line width=0.8pt] (addnorm1) -- (ffn);
        \draw[-{Latex}, line width=0.8pt] (ffn) -- (addnorm2);
        \draw[-{Latex}, line width=0.8pt] (addnorm2) -- (output);

        % Residual connections (dashed for distinction)
        \draw[-{Latex}, dashed, line width=0.8pt] ($(input.south west)+(1mm,-1mm)$) -- ++(0,-21mm) -| (addnorm1.west);
        \draw[-{Latex}, dashed, line width=0.8pt] ($(addnorm1.south west)+(1mm,-1mm)$) -- ++(0,-16mm) -| (addnorm2.west);
