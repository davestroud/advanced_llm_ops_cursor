...
The concepts introduced here form the foundation for subsequent chapters: Chapter~\ref{ch:infra} builds on these fundamentals to cover infrastructure and environment design, Chapter~\ref{ch:cicd} addresses CI/CD practices for LLM systems, Chapter~\ref{ch:monitoring} focuses on monitoring and observability, and Chapter~\ref{ch:scaling} examines scaling strategies. Each of these chapters assumes familiarity with the core concepts and terminology established in this chapter.
...
\begin{itemize}
    \item \textbf{Prompt engineering and management as a first-class artifact} \parencite{Chowdhery2022palm} (i.e., treating prompts and prompt templates like code that can be versioned, tested, and optimized over time).
    \item \textbf{Managing non-deterministic outputs} \parencite{kamathDeepDive} (designing systems to handle variability and possible unexpected or stochastic responses from the model).
    \item \textbf{Integrating retrieval-augmented generation (RAG) pipelines} \parencite{Borzunov2023distributed} (to supplement the model with relevant external knowledge retrieved at query time).
    \item \textbf{Continuous updates to knowledge without retraining from scratch} \parencite{Zheng2024sglang} (so the system can stay up-to-date with new information by injecting knowledge via retrieval or lightweight fine-tuning, rather than expensive full retraining).
...
KV-cache memory is often the limiting factor in high-throughput serving. PagedAttention introduces paging-inspired
KV-cache management to reduce fragmentation and improve batchability, and serving engines such as vLLM operationalize
...
\end{equation}
If we checkpoint (recompute) half of the layers, for example, we might set $\chi=0.5$, halving the activation memory at the cost of some extra computation. In addition to activations, modern optimizers (like Adam) maintain additional state for each parameter (e.g., momentum and variance estimates). These optimizer states can add roughly $2$--$4\times M_{\text{params}}$ to the memory footprint. This means that training a model can require several times the memory of just storing the model. For example, a 175B parameter model in FP16 is about 350~GB of weights; if using Adam, the optimizer states might consume another 700GB (if 4x the parameter count), and then activations on top of that. This is why training large LLMs is incredibly memory-intensive and invariably done in distributed fashion. Activation checkpointing is a common LLMOps practice to trade off extra compute for lower memory usage, making otherwise impossible training runs feasible on a given hardware setup.
...
\begin{figure}[t]
    ...
    \begin{tikzpicture}[font=\small,>=latex,scale=0.95, every node/.style={scale=0.95}]
        % Nodes
        \node[draw, rounded corners, minimum width=3.5cm, minimum height=0.9cm, fill=gray!10] (input) {Input (tokens)};
        \node[draw, rounded corners, minimum width=3.5cm, minimum height=0.9cm, fill=blue!10, below=0.9cm of input] (mha) {Multi-Head Attention};
        \node[draw, rounded corners, minimum width=3.5cm, minimum height=0.9cm, fill=green!10, below=0.7cm of mha] (addnorm1) {Add \& Norm};
        \node[draw, rounded corners, minimum width=3.5cm, minimum height=0.9cm, fill=orange!10, below=0.7cm of addnorm1] (ffn) {Feed-Forward Network};
        \node[draw, rounded corners, minimum width=3.5cm, minimum height=0.9cm, fill=green!10, below=0.7cm of ffn] (addnorm2) {Add \& Norm};
        \node[draw, rounded corners, minimum width=3.5cm, minimum height=0.9cm, fill=gray!10, below=0.9cm of addnorm2] (output) {Hidden States / Next Block};
        ...
        \draw[->] (addnorm1) -- (ffn);
        \draw[->] (ffn) -- (addnorm2);
        \draw[->] (addnorm2) -- (output);

        % Residual connections
        \draw[->, dashed] ($(input.south west)+(0.1,-0.1)$) -- ++(0,-2.1) -| (addnorm1.west);
        \draw[->, dashed] ($(addnorm1.south west)+(0.1,-0.1)$) -- ++(0,-1.6) -| (addnorm2.west);
    ...
    \end{tikzpicture}
    \caption{Schematic of a Transformer block highlighting attention, FFN, and residual paths. Inference memory is dominated by parameters and the KV cache (Eq.~\ref{eq:param-mem}, \ref{eq:kv-cache}); compute cost scales with Eq.~\ref{eq:flops-layer}.}
...
\begin{figure}[t]
    ...
    \begin{tikzpicture}
        \begin{axis}[
            ...
            \addplot+[thick, mark=none, domain=0:4096, samples=200]
                {(x^2)/(4096^2)};
            \addlegendentry{$\Theta(T^2)$ attention (Eq.~\ref{eq:attn-complexity})};

            \addplot+[thick, dashed, mark=none, domain=0:4096, samples=200]
                {x/4096};
            \addlegendentry{Linear reference};

        \end{axis}
    \end{tikzpicture}
    \caption{Attention cost grows quadratically with context length $T$ (solid), outpacing linear strategies (dashed). This motivates paged attention, local attention, and RAG to manage long contexts in production.}
    \label{fig:quadratic-attn}
\end{figure}
...
\begin{tcolorbox}[title={Capacity Planning: A100/H100 Quick Rules}, colback=gray!5, colframe=gray!50, breakable]
\textbf{GPU memory budgets (inference):}
\begin{center}
\begin{tabular}{lcc}
\toprule
Device & HBM (GB) & Notes \\
...
\medskip
\textbf{Rule-of-thumb parameter memory (FP16):}
\begin{center}
\begin{tabular}{lrr}
\toprule
...
\medskip
\textbf{KV-cache sizing examples (FP16, Eq.~\ref{eq:kv-cache}):}
\begin{itemize}
\item \textbf{7B-like} ($L{=}32,\,d{=}4096$): coefficient $\approx \num{524288}$ bytes per token per batch.\\
$B{=}8,\,T{=}2048 \Rightarrow M_{\text{KV}}\!\approx\!\SI{8}{\gibi\byte}$;\quad
$B{=}8,\,T{=}8192 \Rightarrow \SI{32}{\gibi\byte}$.
\item \textbf{13B-like} ($L{=}40,\,d{=}5120$): coefficient $\approx \num{819200}$ bytes per token per batch.\\
$B{=}4,\,T{=}4096 \Rightarrow \approx \SI{12.5}{\gibi\byte}$;\quad
$B{=}8,\,T{=}8192 \Rightarrow \approx \SI{50}{\gibi\byte}$.
\end{itemize}

\medskip
\textbf{Fit heuristics (single GPU, FP16):}
\begin{itemize}
...
\medskip
\textbf{Ops recommendations:}
\begin{itemize}
\item Reserve \(\mathbf{10\%{-}20\%}\) headroom for fragmentation/overheads.
...
\begin{figure}[t]
    ...
    \begin{tikzpicture}
        \begin{axis}[
            ...
            \addplot+[thick, domain=0:2.0, samples=200] {(1*120) / (x + 120*0.02)};
            \addlegendentry{$B{=}1$};

            \addplot+[thick, domain=0:2.0, samples=200, dashed] {(4*120) / (x + 120*0.02)};
            \addlegendentry{$B{=}4$};

            \addplot+[thick, domain=0:2.0, samples=200, dotted] {(8*120) / (x + 120*0.02)};
            \addlegendentry{$B{=}8$};

            \addplot+[thick, domain=0:2.0, samples=200, dashdotted] {(16*120) / (x + 120*0.02)};
            \addlegendentry{$B{=}16$};
        \end{axis}
    \end{tikzpicture}
    \caption{Idealized throughput vs. TTFT using Eq.~\ref{eq:throughput} with $N{=}120$ tokens/request and $\tau_1{=}\SI{0.02}{s/token}$. Larger batches ($B$) amortize TTFT, but real systems saturate due to kernel/IO limits; use as a planning guideline, not a guarantee.}
    \label{fig:ttft-throughput}
\end{figure}
...
\bottomrule
\end{tabular}
\caption{Approximate parameter memory requirements for different model scales (FP16 weights).}
\label{tab:param-memory}
\end{table}
