...
This chapter synthesizes the methods developed throughout the book into a single, end-to-end implementation narrative for \ishtar{}: an LLM-driven assistant built for journalists operating in conflict and crisis zones.
Unlike low-stakes consumer chatbots, \ishtar{} is designed for \emph{time-critical}, \emph{high-uncertainty}, and \emph{high-consequence} decisions where misinformation and sensitive-data exposure can cause direct harm.
Accordingly, the system is engineered as a full LLMOps stack---from ingestion and retrieval to serving, evaluation, observability, and governance---rather than as a single model invocation.

\noindent\textbf{Chapter roadmap.}
We begin by restating the \ishtar{} problem framing and concrete operational requirements.
We then recap the system architecture as implemented, and walk step-by-step through the build: infrastructure provisioning, ingestion and knowledge-base construction, retrieval-augmented generation, multi-agent orchestration, CI/CD and evaluation gates, observability and SLO enforcement, and security and responsible-deployment controls.
We close with measured performance outcomes, lessons learned, and forward-looking extensions.
...
\begin{tcolorbox}[title={At a Glance: The End-to-End \ishtar{} Build},colback=black!2,colframe=black!35,breakable]
\small
\begin{enumerate}[leftmargin=*,label=\textbf{Step \arabic*:}]
    ...
    \item \textbf{Construct the knowledge base:} embedding services, FAISS/HNSW index design, metadata schema, and snapshot/version strategy (\S\ref{sec:ishtar-step-kb}).
...
Table~\ref{tab:ishtar-requirements} summarizes representative targets used to guide \ishtar{} design.
...
\begin{tabularx}{\textwidth}{@{}p{32mm}p{55mm}X@{}}
\toprule
...
\bottomrule
\end{tabularx}
\caption{Representative \ishtar{} end-to-end requirements used as design targets.}
\label{tab:ishtar-requirements}
\end{table}
...
\begin{itemize}
    ...
    \item \textbf{Vector index} (FAISS/HNSW) for approximate nearest-neighbor retrieval with strict latency budgets~\cite{johnson2019billion,Malkov2018HNSW}.
    ...
    \item \textbf{LLM serving} on GPU nodes using optimized inference engines (vLLM/TGI) (Chapters~\ref{ch:infra} and~\ref{ch:performance}).
...
Figure~\ref{fig:ishtar-architecture-ch12} depicts the end-to-end flows. The diagram is intentionally operational: it shows where persistence boundaries exist, where latency budgets apply, and where safety checks are enforced.

\begin{figure}[tb]
\centering
\begin{tikzpicture}[
  font=\small,
  box/.style={draw=black!45, rounded corners=4pt, fill=black!2, align=center, minimum height=10mm, inner sep=3pt},
  lane/.style={draw=black!15, rounded corners=6pt, fill=black!1},
  arrow/.style={-{Stealth[length=2.2mm,width=1.8mm]}, line width=0.6pt, draw=black!70},
  dashedarrow/.style={-{Stealth[length=2.2mm,width=1.8mm]}, dashed, line width=0.6pt, draw=black!55},
  note/.style={font=\scriptsize, text=black!70, align=left}
]

% Lanes
\node[lane, minimum width=0.96\textwidth, minimum height=26mm] (inglane) {};
\node[lane, minimum width=0.96\textwidth, minimum height=30mm, below=8mm of inglane] (querylane) {};
\node[note, anchor=west] at ([xshift=4mm]inglane.north west) {\textbf{Continuous ingestion \& indexing (background)}};
\node[note, anchor=west] at ([xshift=4mm]querylane.north west) {\textbf{Online query handling (interactive path)}};

% Ingestion lane nodes
\node[box, anchor=west] (sources) at ([xshift=6mm]inglane.west) {Sources\\\scriptsize (news, NGO,\\social, health)};
\node[box, right=6mm of sources] (ingest) {Ingestion\\\scriptsize parse \& normalize};
\node[box, right=6mm of ingest] (scrub) {Safety preprocessing\\\scriptsize PII/coord scrubbing};
\node[box, right=6mm of scrub] (chunkembed) {Chunk \& embed\\\scriptsize batch GPU/CPU};
\node[box, right=6mm of chunkembed] (docstore) {Doc store\\\scriptsize text + metadata};
\node[box, right=6mm of docstore] (vec) {Vector index\\\scriptsize FAISS/HNSW};
...
% Query lane nodes
\node[box, anchor=west] (user) at ([xshift=6mm]querylane.west) {Journalist\\\scriptsize query/UI};
\node[box, right=6mm of user] (gateway) {API gateway\\\scriptsize auth \& rate limits};
\node[box, right=6mm of gateway] (controller) {Orchestrator\\\scriptsize agent controller};
\node[box, right=6mm of controller] (retr) {Retriever\\\scriptsize ANN + filters};
\node[box, right=6mm of retr] (llm) {LLM serving\\\scriptsize vLLM/TGI};
\node[box, right=6mm of llm] (agents) {Post-processing\\\scriptsize verify \& safety};
\node[box, right=6mm of agents] (resp) {Answer\\\scriptsize citations + disclaimers};
...
\end{tikzpicture}
\caption{End-to-end \ishtar{} architecture. Background ingestion continuously updates a document store and vector index. Online query handling performs authenticated retrieval-augmented generation, followed by verification and safety post-processing, under end-to-end tracing and monitoring.}
\label{fig:ishtar-architecture-ch12}
\end{figure}
...
\textbf{Goal.} Provision a reproducible execution platform for GPU inference and data pipelines, with security and observability baked in from day one (Chapter~\ref{ch:infra-env}).
...
In practice, memory headroom was managed with quantization on selected routes and with serving-runtime techniques such as continuous batching and paged KV-cache allocation (vLLM/PagedAttention)~\cite{Kwon2023vLLM,tgi_docs}.
...
For \ishtar{}, a FAISS-based ANN index provides predictable latency at million-scale corpora~\cite{johnson2019billion}.
...
In crisis intelligence, recall is prioritized over precision: it is preferable to retrieve extra context than to omit a key report.
...
Table~\ref{tab:ishtar-outcomes} summarizes representative outcomes observed after full operationalization.
...
\bottomrule
\end{tabularx}
\caption{Representative \ishtar{} performance outcomes after end-to-end implementation.}
\label{tab:ishtar-outcomes}
\end{table}
...
Optimized serving engines (vLLM/TGI) reduce TTFT and improve throughput (Chapters~\ref{ch:performance} and~\ref{ch:scaling}).
...
FAISS offers speed with engineering overhead for metadata; managed vector DBs trade cost for operational simplicity.
...
\begin{tcolorbox}[title={End-to-End Checklist for Production LLM Systems (Ishtar-derived)},colback=black!2,colframe=black!35,breakable]
\small
\begin{itemize}
    \item \textbf{Define operational contracts:} output schema, citation rules, and uncertainty policy are explicit and testable.
    \item \textbf{Version everything:} prompts, retrieval parameters, tool permissions, model configs, and index snapshots.
    \item \textbf{Instrument first:} traces/metrics/logs exist before user launch; alerts map to user impact.
    \item \textbf{Prefer recall in retrieval:} missing a critical source is worse than retrieving extra context (then rerank).
    \item \textbf{Make safety defense-in-depth:} ingestion scrubbing + output filtering + refusal logic + audits.
    \item \textbf{Gate releases on evaluation:} offline metrics + canary rollouts + rollback automation.
    \item \textbf{Plan for bursty demand:} GPU autoscaling, caching/hot tiers, and priority scheduling for critical users.
    \item \textbf{Treat hallucination as a defect:} root-cause via traces; fix via data, prompts, or retrieval tuning.
    \item \textbf{Embed human oversight:} escalation paths for high-impact outputs and continuous domain review.
\end{itemize}
\end{tcolorbox}
...
RAG-specific evaluation (citation precision, faithfulness, and evidence coverage) should evolve from ad hoc audits to standardized, automated suites (e.g., RAGAS)~\cite{ragas,ragas_eacl2024}.
