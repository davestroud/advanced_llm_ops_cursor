...
\node[box, anchor=west] (sources) at ([xshift=6mm]inglane.west) {Sources\\\footnotesize (news, NGO,\\social, health)};
\node[box, right=6mm of sources] (ingest) {Ingestion\\\footnotesize parse \& normalize};
\node[box, right=6mm of ingest] (scrub) {Safety preprocessing\\\footnotesize PII/coord scrubbing};
\node[box, right=6mm of scrub] (chunkembed) {Chunk \& embed\\\footnotesize batch GPU/CPU};
\node[box, right=6mm of chunkembed] (docstore) {Doc store\\\footnotesize text + metadata};
\node[box, right=6mm of docstore] (vec) {Vector index\\\footnotesize FAISS/HNSW};
...
% Query lane nodes
\node[box, anchor=west] (user) at ([xshift=6mm]querylane.west) {Journalist\\\footnotesize query/UI};
\node[box, right=6mm of user] (gateway) {API gateway\\\footnotesize auth \& rate limits};
\node[box, right=6mm of gateway] (controller) {Orchestrator\\\footnotesize agent controller};
\node[box, right=6mm of controller] (retr) {Retriever\\\footnotesize ANN + filters};
\node[box, right=6mm of retr] (llm) {LLM serving\\\footnotesize vLLM/TGI};
\node[box, right=6mm of llm] (agents) {Post-processing\\\footnotesize verify \& safety};
\node[box, right=6mm of agents] (resp) {Answer\\\footnotesize citations + disclaimers};
