...
\begin{llmfigbox}
\begin{tikzpicture}[
  layer/.style={rectangle, rounded corners, draw=none,
...
% Layers
\node[layer] (system) {System Telemetry\\[2pt]\small GPU/CPU util., memory, latency, throughput};
\node[layer, below=of system,yshift=-6mm] (model) {Model Telemetry\\[2pt]\small Tokens/request, hallucination rate, factuality checks};
\node[layer, below=of model,yshift=-6mm] (pipeline) {Pipeline Telemetry\\[2pt]\small Prompt traces, retrieval hits, multi-agent orchestration};
...
\begin{tikzpicture}[
  ...
  box/.style={rectangle, rounded corners=4pt, draw=none, inner sep=4mm, text width=45mm},
  boxwide/.style={rectangle, rounded corners=4pt, draw=none, inner sep=4mm, text width=0.25\linewidth},
...
% --- Central TRACE panel ---
\node[boxwide, fill=panelgray!6] (trace) {%
  \textbf{Trace} (execution waterfall)\\[1mm]
  \(\bullet\) \texttt{trace\_id}, \texttt{span\_id}, \texttt{conversation\_id}\\[0.5mm]
  \(\bullet\) \texttt{baggage}: tenant/user, privacy class\\[0.5mm]
  \(\bullet\) rollup fields for P50/P95 latency, tokens, cost
};

% --- Span taxonomy boxes (two columns) ---
\node[box, fill=llmblue!10, right=20mm of trace.north east, anchor=north west] (llm) {%
  \header{\texttt{llm.call}}\\[1mm]
  model/version; temperature, $p$; TTFT; tokens/s; token counts
};
\node[box, fill=retrgreen!10, below=of llm] (retr) {%
  \header{\texttt{retriever.search}}\\[1mm]
  index/version; \texttt{topK}; latency; hit set (doc IDs); re-ranker scores
};
\node[box, fill=toolorange!10, below=of retr] (tool) {%
  \header{\texttt{tool.invoke.<name>}}\\[1mm]
  schema version; payload bytes; retries/backoff; downstream status
};

\node[box, fill=parserviolet!10, right=of llm] (parser) {%
  \header{\texttt{parser.jsonschema}}\\[1mm]
  schema ID; failure class: \texttt{missing\_field}, \texttt{type\_mismatch}
};
\node[box, fill=guardred!10, below=of parser] (guard) {%
  \header{\texttt{guard.rail}}\\[1mm]
  policy ID; triggers (jailbreak, toxicity); action (block/sanitize/route)
};

% --- Arrows from TRACE to spans (routed completely outside boxes) ---
\draw[arrow] (trace.east) -- ++(10mm,0) coordinate (mid1) -- (mid1 |- llm.west) -- (llm.west);
\draw[arrow] (trace.east) -- ++(10mm,0) coordinate (mid2) -- (mid2 |- retr.west) -- (retr.west);
\draw[arrow] (trace.east) -- ++(10mm,0) coordinate (mid3) -- (mid3 |- tool.west) -- (tool.west);
\draw[arrow] (trace.east) -- ++(10mm,0) coordinate (mid4) -- (mid4 |- parser.west) -- (parser.west);
\draw[arrow] (trace.east) -- ++(10mm,0) coordinate (mid5) -- (mid5 |- guard.west) -- (guard.west);

% --- Legend / Notes panel ---
\node[rectangle, rounded corners=4pt, draw=none, fill=panelgray!5,
      below=16mm of tool, text width=0.9\linewidth, inner sep=4mm] (legend) {%
  ...
  Example rollup: ``P95 latency attributable to \texttt{retriever.search} increased 30\% after build 2025-08-12.''
};

% --- Title badge ---
\node[rectangle, rounded corners=3pt, draw=none, fill=panelgray!8,
      above left=4mm and -4mm of trace.north east, anchor=south east, inner sep=4mm] (title) {%
