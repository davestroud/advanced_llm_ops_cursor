...
\begin{tikzpicture}[
  ...
  box/.style={rectangle, rounded corners=4pt, very thick, inner sep=10mm, minimum width=100mm, minimum height=30mm},
  boxwide/.style={rectangle, rounded corners=4pt, very thick, inner sep=10mm, minimum width=105mm, minimum height=32mm},
...
\node[boxwide, fill=panelgray!6, draw=panelgray] (trace) {%
  \textbf{Trace} (execution waterfall)\\[-4mm]
  \begin{tabular}{@{}l@{}}
...
\node[box, fill=llmblue!10, draw=llmblue, right=30mm of trace.north east, anchor=north west] (llm) {%
  \header{\texttt{llm.call}}\\[-4mm]
  \attr{model/version; temperature, $p$; TTFT; tokens/s; token counts}
};
\node[box, fill=retrgreen!10, draw=retrgreen, below=of llm] (retr) {%
  \header{\texttt{retriever.search}}\\[-4mm]
...
\node[box, fill=toolorange!10, draw=toolorange, below=of retr] (tool) {%
  \header{\texttt{tool.invoke.<name>}}\\[-4mm]
  \attr{schema version; payload bytes; retries/backoff; downstream status}
};

\node[box, fill=parserviolet!10, draw=parserviolet, right=30mm of llm] (parser) {%
  \header{\texttt{parser.jsonschema}}\\[-4mm]
...
\node[box, fill=guardred!10, draw=guardred, below=of parser] (guard) {%
  \header{\texttt{guard.rail}}\\[-4mm]
  \attr{policy ID; triggers (jailbreak, toxicity); action (block/sanitize/route)}
};

% --- Arrows from TRACE to spans ---
\draw[arrow] (trace.east) -- ++(14mm,0) |- (llm.west);
\draw[arrow] (trace.east) -- ++(14mm,0) |- (retr.west);
\draw[arrow] (trace.east) -- ++(14mm,0) |- (tool.west);
\draw[arrow] (trace.east) -- ++(14mm,0) |- (parser.west);
\draw[arrow] (trace.east) -- ++(14mm,0) |- (guard.west);

% --- Legend / Notes panel ---
\node[rectangle, rounded corners=4pt, draw=panelgray, fill=panelgray!5,
      below=26mm of tool, minimum width=0.95\linewidth, inner sep=8mm, minimum height=22mm] (legend) {%
  ...
  form a stable taxonomy for queryable traces.\\[-4mm]
...
\node[rectangle, rounded corners=3pt, draw=panelgray, fill=panelgray!8,
      above left=6mm and -6mm of trace.north east, anchor=south east, inner sep=6mm] (title) {%
