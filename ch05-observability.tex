...
\begin{tikzpicture}[
  ...
  attr/.style={font=\normalsize},
  box/.style={rectangle, rounded corners=4pt, very thick, inner sep=8mm, text width=58mm},
  boxwide/.style={rectangle, rounded corners=4pt, very thick, inner sep=8mm, text width=0.3\linewidth},
  arrow/.style={->, very thick, line width=1.2pt},
  dot/.style={circle, inner sep=2pt, fill=black}
]
...
\node[box, fill=llmblue!10, draw=llmblue, right=28mm of trace.north east, anchor=north west] (llm) {%
...
\node[box, fill=parserviolet!10, draw=parserviolet, right=of llm] (parser) {%
...
\draw[arrow] (trace.east) -- ++(16mm,0) coordinate (mid1) -- (mid1 |- llm.west) -- (llm.west);
\draw[arrow] (trace.east) -- ++(16mm,0) coordinate (mid2) -- (mid2 |- retr.west) -- (retr.west);
\draw[arrow] (trace.east) -- ++(16mm,0) coordinate (mid3) -- (mid3 |- tool.west) -- (tool.west);
\draw[arrow] (trace.east) -- ++(16mm,0) coordinate (mid4) -- (mid4 |- parser.west) -- (parser.west);
\draw[arrow] (trace.east) -- ++(16mm,0) coordinate (mid5) -- (mid5 |- guard.west) -- (guard.west);

% --- Legend / Notes panel ---
\node[rectangle, rounded corners=4pt, draw=panelgray, fill=panelgray!5,
      below=24mm of tool, text width=0.95\linewidth, inner sep=8mm] (legend) {%
  ...
  form a stable taxonomy for queryable traces.\\[2mm]
...
\node[rectangle, rounded corners=3pt, draw=panelgray, fill=panelgray!8,
      above left=7mm and -7mm of trace.north east, anchor=south east, inner sep=7mm] (title) {%
  \bfseries\normalsize Span Taxonomy \& Naming
};

\end{tikzpicture}
\end{llmfigboxnoscale}
