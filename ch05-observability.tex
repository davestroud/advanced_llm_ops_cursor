...
\begin{tikzpicture}[
  ...
  header/.style={font=\bfseries\ttfamily\large},
  attr/.style={font=\large},
  box/.style={rectangle, rounded corners=4pt, very thick, inner sep=8mm, minimum width=90mm, minimum height=24mm},
  boxwide/.style={rectangle, rounded corners=4pt, very thick, inner sep=8mm, minimum width=95mm, minimum height=26mm},
  arrow/.style={->, very thick, line width=1.8pt},
  dot/.style={circle, inner sep=1.8pt, fill=black}
]
...
\node[boxwide, fill=panelgray!6, draw=panelgray] (trace) {%
  \textbf{Trace} (execution waterfall)\\[-3mm]
  \begin{tabular}{@{}l@{}}
...
\node[box, fill=llmblue!10, draw=llmblue, right=26mm of trace.north east, anchor=north west] (llm) {%
  \header{\texttt{llm.call}}\\[-3mm]
  \attr{model/version; temperature, $p$; TTFT; tokens/s; token counts}
};
\node[box, fill=retrgreen!10, draw=retrgreen, below=of llm] (retr) {%
  \header{\texttt{retriever.search}}\\[-3mm]
...
\node[box, fill=toolorange!10, draw=toolorange, below=of retr] (tool) {%
  \header{\texttt{tool.invoke.<name>}}\\[-3mm]
  \attr{schema version; payload bytes; retries/backoff; downstream status}
};

\node[box, fill=parserviolet!10, draw=parserviolet, right=26mm of llm] (parser) {%
  \header{\texttt{parser.jsonschema}}\\[-3mm]
...
\node[box, fill=guardred!10, draw=guardred, below=of parser] (guard) {%
  \header{\texttt{guard.rail}}\\[-3mm]
  \attr{policy ID; triggers (jailbreak, toxicity); action (block/sanitize/route)}
};

% --- Arrows from TRACE to spans ---
\draw[arrow] (trace.east) -- ++(12mm,0) |- (llm.west);
\draw[arrow] (trace.east) -- ++(12mm,0) |- (retr.west);
\draw[arrow] (trace.east) -- ++(12mm,0) |- (tool.west);
\draw[arrow] (trace.east) -- ++(12mm,0) |- (parser.west);
\draw[arrow] (trace.east) -- ++(12mm,0) |- (guard.west);

% --- Legend / Notes panel ---
\node[rectangle, rounded corners=4pt, draw=panelgray, fill=panelgray!5,
      below=22mm of tool, minimum width=0.95\linewidth, inner sep=6mm, minimum height=18mm] (legend) {%
  ...
  form a stable taxonomy for queryable traces.\\[-3mm]
  \attr{Example rollup: ``P95 latency attributable to \texttt{retriever.search} increased 30\% after build 2025-08-12.''}
};

% --- Title badge ---
\node[rectangle, rounded corners=3pt, draw=panelgray, fill=panelgray!8,
      above left=5mm and -5mm of trace.north east, anchor=south east, inner sep=5mm] (title) {%
  \bfseries\large Span Taxonomy \& Naming
};
