...
\begin{tikzpicture}[
  ...
  arrow/.style={-Stealth, very thick, line width=1.2pt, color=black!70},
  feedback/.style={-Stealth, dashed, dash pattern=on 4pt off 2pt, very thick, line width=1.2pt, color=black!60},
  note/.style={font=\small, align=center}
]

% ---- Top row in a matrix (clean alignment) ----
\matrix (top) [row sep=0mm, column sep=5mm] {
  \node[phase, fill=fundblue!15] (fund) {LLMOps Fundamentals\\(Ch.~\ref{ch:llmops-fundamentals})}; &
  \node[phase, fill=infragreen!15] (infra) {Infrastructure \& Environment\\(Ch.~\ref{ch:infra})}; &
  \node[phase, fill=cicdorange!15] (cicd)  {CI/CD\\(Ch.~\ref{ch:cicd})}; &
  \node[phase, fill=monpurple!15] (mon)   {Monitoring\\(Ch.~\ref{ch:monitoring})}; &
  \node[phase, fill=scaleviolet!15] (scale) {Scaling\\(Ch.~\ref{ch:scaling})}; &
  \node[phase, fill=perfred!15] (perf)  {Performance\\(Ch.~\ref{ch:performance})}; \\
};

% ---- Second row: RAG and Agents ----
\node[phase, fill=ragteal!15, below=10mm of perf] (rag) {RAG\\(Ch.~\ref{ch:rag})};
\node[phase, fill=orchcyan!15, right=4mm of rag] (orch) {Multi‑Agent\\(Ch.~\ref{ch:multiagent})};

% ---- Bottom row placed relative to second row ----
\node[phase, fill=testyellow!15, below=10mm of rag]   (test)   {Testing \& Robustness\\(Ch.~\ref{ch:testing})};
\node[phase, fill=ethicspink!15, right=4mm of test]   (ethics) {Ethics \& Governance\\(Ch.~\ref{ch:ethics})};
\node[phase, fill=casebrown!15, right=4mm of ethics] (case)   {Case Study\\(Ch.~\ref{ch:case-study})};
