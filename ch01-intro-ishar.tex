...
\begin{tikzpicture}[
  ...
  source/.style={ellipse, fill=blue!20, draw=blue!60!black, line width=0.7pt, 
                 minimum width=22mm, minimum height=11mm, align=center, inner sep=4pt},
  ingest/.style={rectangle, fill=cyan!20, draw=cyan!60!black, rounded corners=4pt,
                 line width=0.7pt, minimum width=26mm, minimum height=13mm, align=center, inner sep=4pt},
  vector/.style={cylinder, fill=teal!20, draw=teal!60!black, line width=0.7pt,
                 shape aspect=0.3, minimum width=24mm, minimum height=18mm, align=center, inner sep=4pt},
  rag/.style={rectangle, fill=purple!20, draw=purple!60!black, rounded corners=4pt,
              line width=0.7pt, minimum width=30mm, minimum height=15mm, align=center, inner sep=4pt},
  agent/.style={rectangle, fill=green!20, draw=green!60!black, rounded corners=4pt,
                line width=0.7pt, minimum width=24mm, minimum height=11mm, align=center, inner sep=3pt},
  orchestrator/.style={rectangle, fill=orange!20, draw=orange!60!black, rounded corners=4pt,
                       line width=0.8pt, minimum width=32mm, minimum height=13mm, align=center, inner sep=4pt, font=\small\bfseries},
  gpu/.style={rectangle, fill=red!20, draw=red!60!black, rounded corners=4pt,
              line width=0.7pt, minimum width=28mm, minimum height=15mm, align=center, inner sep=4pt},
  response/.style={ellipse, fill=gray!20, draw=gray!60!black, line width=0.7pt,
                  minimum width=24mm, minimum height=11mm, align=center, inner sep=4pt},
  obs/.style={rectangle, fill=yellow!8, draw=black!25, rounded corners=3pt, dashed,
              line width=0.6pt, inner sep=4pt, font=\footnotesize},
  arrow/.style={-{Stealth[length=2.5mm,width=2mm]}, line width=0.8pt, color=black!75},
  arrowthin/.style={-{Stealth[length=2mm,width=1.5mm]}, line width=0.6pt, color=black!60}
]

% ========== LEFT LANE: Data Sources & Ingestion ==========
% Data sources (left column)
\node[source] (news) at (0, 15mm) {News wires\\\& reports};
\node[source] (ngo) at (0, 0mm) {NGO\\bulletins};
\node[source] (social) at (0, -15mm) {Social media\\(vetted)};

% Ingestion agents (middle-left column)
\node[ingest] (ingest1) at (35mm, 15mm) {Ingestion\\Agent 1};
\node[ingest] (ingest2) at (35mm, 0mm) {Ingestion\\Agent 2};
\node[ingest] (ingest3) at (35mm, -15mm) {Ingestion\\Agent 3};

% Vector database (center-left)
\node[vector] (vectordb) at (70mm, 0mm) {Vector\\Database};

% ========== MIDDLE LANE: RAG & Multi-Agent ==========
% Query input (top center)
\node[source] (query) at (110mm, 20mm) {Journalist\\Query};

% RAG retrieval (center)
\node[rag] (retrieval) at (110mm, 0mm) {RAG\\Retrieval};

% Multi-agent orchestration (below RAG)
\node[orchestrator] (orchestrator) at (110mm, -20mm) {Orchestrator};

% Agents (below orchestrator)
\node[agent] (summarizer) at (95mm, -35mm) {Summarizer};
\node[agent] (factchecker) at (110mm, -35mm) {Fact-Checker};
\node[agent] (refiner) at (125mm, -35mm) {Refiner};

% ========== RIGHT LANE: GPU Inference & Response ==========
% GPU inference cluster
\node[gpu] (gpu1) at (155mm, -10mm) {GPU\\Node 1};
\node[gpu] (gpu2) at (155mm, -30mm) {GPU\\Node 2};

% Response (far right)
\node[response] (response) at (190mm, -20mm) {Response\\+ Citations};
