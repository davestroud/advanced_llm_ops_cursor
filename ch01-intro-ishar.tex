...
\begin{tikzpicture}[
  ...
  obs/.style={rectangle, fill=yellow!8, draw=black!25, rounded corners=3pt, dashed,
              line width=0.6pt, inner sep=3pt, font=\footnotesize},
...
% Data sources (stacked vertically, left side) - more compact
\node[source] (news) at (-35mm, 18mm) {News wires\\\& reports};
\node[source] (ngo) at (-35mm, 0mm) {NGO\\bulletins};
\node[source] (social) at (-35mm, -18mm) {Social media\\(vetted)};

% Ingestion agents (middle-left)
\node[ingest] (ingest1) at (-15mm, 18mm) {Ingestion\\Agent 1};
\node[ingest] (ingest2) at (-15mm, 0mm) {Ingestion\\Agent 2};
\node[ingest] (ingest3) at (-15mm, -18mm) {Ingestion\\Agent 3};

% Vector database (center-left, below)
\node[vector] (vectordb) at (-15mm, -35mm) {Vector\\Database};

% ========== CENTER COLUMN: Query, RAG & Orchestration ==========
% Query input (top center)
\node[source] (query) at (5mm, 22mm) {Journalist\\Query};

% RAG retrieval (center)
\node[rag] (retrieval) at (5mm, 0mm) {RAG\\Retrieval};

% Multi-agent orchestration (below RAG)
\node[orchestrator] (orchestrator) at (5mm, -22mm) {Orchestrator};

% Agents (below orchestrator, compact row)
\node[agent] (summarizer) at (-8mm, -38mm) {Summarizer};
\node[agent] (factchecker) at (5mm, -38mm) {Fact-Checker};
\node[agent] (refiner) at (18mm, -38mm) {Refiner};

% ========== RIGHT COLUMN: GPU & Response ==========
% GPU inference cluster
\node[gpu] (gpu1) at (35mm, -12mm) {GPU\\Node 1};
\node[gpu] (gpu2) at (35mm, -32mm) {GPU\\Node 2};

% Response (right of GPU)
\node[response] (response) at (55mm, -22mm) {Response\\+ Citations};
...
\draw[arrowthin, dotted, color=black!35] ([xshift=-5mm]obstop.south) -- ([xshift=-5mm]vectordb.north);
\draw[arrowthin, dotted, color=black!35] (obstop.south) -- (retrieval.north);
\draw[arrowthin, dotted, color=black!35] ([xshift=5mm]obstop.south) -- ([xshift=5mm]orchestrator.north);
\draw[arrowthin, dotted, color=black!35] ([xshift=-5mm]obsbot.north) -- ([xshift=-5mm]orchestrator.south);
