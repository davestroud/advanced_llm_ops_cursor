...
\begin{tikzpicture}[
  ...
  phase/.style={
    ...
    text width=28mm,
    minimum height=12mm, inner sep=5pt,
    align=center, font=\small\bfseries
  },
  arrow/.style={-Stealth, very thick, line width=1.5pt, color=black!75},
  feedback/.style={-Stealth, dashed, dash pattern=on 5pt off 3pt, very thick, line width=1.3pt, color=black!50},
  note/.style={font=\small, align=center, color=black!70},
  partlabel/.style={font=\footnotesize\bfseries, color=black!60, anchor=west}
]
...
\node[partlabel] at ([yshift=8mm]fund.north) {\textbf{Part I: Foundations}};
\node[partlabel] at ([yshift=8mm]cicd.north) {\textbf{Part II: Delivery Operations}};
\node[partlabel] at ([yshift=8mm]perf.north) {\textbf{Part III: Optimization}};

% ---- Second row: RAG and Agents ----
\node[phase, fill=ragteal!12, draw=ragteal!40, below=12mm of perf] (rag) {RAG\\(Ch.~\ref{ch:rag})};
\node[phase, fill=orchcyan!12, draw=orchcyan!40, right=5mm of rag] (orch) {Multi‑Agent\\(Ch.~\ref{ch:multiagent})};
...
\node[phase, fill=ethicspink!12, draw=ethicspink!40, right=5mm of test]   (ethics) {Ethics \& Governance\\(Ch.~\ref{ch:ethics})};
\node[phase, fill=casebrown!12, draw=casebrown!40, right=5mm of ethics] (case)   {Case Study\\(Ch.~\ref{ch:case-study})};

% ---- Part label for bottom row ----
\node[partlabel] at ([yshift=8mm]test.north) {\textbf{Part IV: Governance}};
...
\path (mon.north) ++(0,20mm) coordinate (M1);
\path (fund.north) ++(0,20mm) coordinate (M2);
\draw[feedback] (mon.north)
  .. controls (M1) and (M2) ..
  node[note, above, pos=0.5, text width=50mm]
  {\emph{Feedback: patterns, lessons, new requirements}}
  (fund.north);

% ---- Feedback arc from Case Study (high route) ----
\path (case.north) ++(0,18mm) coordinate (C1);
\path (fund.north) ++(0,18mm) coordinate (C2);
