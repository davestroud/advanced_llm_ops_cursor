...
\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{images/ch01-llmops-lifecycle}
\caption{The LLMOps continuous improvement lifecycle, showing how design, deployment, operation, and monitoring feed into iterative refinement.}
\label{fig:ch01:lifecycle}
\end{figure}
...
Even with managed APIs, LLM-powered applications behave like distributed systems. A single user query may traverse multiple components—retrieval, reranking, tool calls, safety checks, and the model itself—each introducing failure modes and latency variance \cite{edge2025llmops}. Common issues include maximizing GPU utilization (e.g., via batching or async streams) and managing the limited context window (prompts can exceed model limits as instructions, history, and retrieved documents accumulate). Systems must implement truncation or segmentation strategies for long contexts \cite{liu2023promptSurvey}. Reliability is also a concern: when a subcomponent fails (retriever times out, a tool API is down, or the model hits a rate limit), the overall system should gracefully degrade (e.g., use cached or partial results, fall back to a simpler response) rather than fail catastrophically.

\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{images/ch01-llm-architecture}
...
\begin{itemize}
  ...
  outputs, and maintain auditable traces for forensics and incident response.
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.98\textwidth]{images/ch01-security-threat-model}
...
LLM inference is fundamentally more expensive than most classical ML workloads because each request
consumes substantial compute and memory bandwidth, and because request \emph{shape} (prompt length,
output length, and concurrency) drives non-linear resource usage (e.g., KV-cache pressure under long
contexts). In production, the binding question is rarely ``can we run the model?'' but rather ``can we run it
\emph{reliably}, \emph{quickly}, and \emph{affordably} under peak load?''

A practical way to operationalize this is to manage a \emph{latency budget} and an \emph{economics budget}
simultaneously. End-to-end latency should be decomposed by stage (tokenization/policy checks, retrieval,
model prefill/decoding, post-processing), and reported by tail percentiles (p95/p99), not only averages.
In interactive systems, \emph{time-to-first-token (TTFT)} is often the dominant user-perceived metric, while
\emph{tokens-per-second} determines how quickly the response completes.

\begin{figure}[t]
  \centering
  \includegraphics[width=0.98\textwidth]{images/ch01-cost-latency-throughput}
  \caption{Latency decomposition and scale levers for LLM serving. End-to-end latency is the sum of stage latencies
  (tokenization/policy checks, retrieval, LLM prefill+decode, post-processing). Sustaining throughput and cost efficiency
  requires coordinated controls: batching and scheduling for throughput, caching/quantization/routing for unit economics,
  and explicit capacity planning (peak demand, tail latency, and autoscaling on queue depth).}
...
Traditional monitoring focuses on uptime, CPU, and error rates. LLMOps requires observability that captures
\emph{semantic} and \emph{behavioral} properties, because a generative system can return HTTP~200 while producing
an ungrounded, unsafe, or policy-violating answer. In practice, LLM applications behave like composed systems:
a single user request may traverse routing logic, retrieval, reranking, multi-step prompting, tool calls, and
post-processing. Failures are therefore frequently \emph{cross-layer} (e.g., a retrieval miss causing hallucination,
or a tool timeout causing a partial answer that still looks fluent). The operational goal is not merely to detect that
an error occurred, but to explain \emph{why} an output was produced and \emph{which component} (prompt, retriever,
tool, model version, or safety layer) drove the observed behavior.

A useful mental model is to treat observability as an \emph{evidence ledger} for each response: record the exact
prompt template and parameters, the evidence retrieved (with identifiers and timestamps), the tool calls performed,
and the model configuration (provider, version, temperature/top-$p$, context length). These traces support three
production-critical workflows: (i) incident response (rapid attribution and rollback), (ii) continuous evaluation
(turning production failures into regression tests), and (iii) governance (audit trails for safety, privacy, and policy compliance).
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.98\textwidth]{images/ch01-scale-constraints}
...
% ----------------------------------------------------------------------
% FIGURE: Inputs taxonomy (professional layout + auto-sized to \linewidth)
% Preamble (once): \usepackage{tikz,adjustbox}
...
\begin{tikzpicture}[
  font=\small,
  >=Stealth,
  hub/.style={draw=black!25, rounded corners=7pt, fill=black!2, inner sep=10pt},
  src/.style={draw=black!35, rounded corners=4pt, align=center,
              minimum width=34mm, minimum height=11mm, inner sep=4pt},
  cNews/.style={src, fill=blue!12,   draw=blue!55!black},
  cNGO/.style={src, fill=teal!12,   draw=teal!55!black},
  cSocial/.style={src, fill=purple!10, draw=purple!60!black},
  cHealth/.style={src, fill=orange!12, draw=orange!65!black},
  funnel/.style={draw=black!45, rounded corners=5pt, fill=black!4,
                 minimum width=26mm, minimum height=12mm, align=center, inner sep=4pt},
  query/.style={draw=black!70, rounded corners=5pt, fill=black!1,
                minimum width=36mm, minimum height=14mm, align=center, inner sep=5pt},
  arrow/.style={->, line width=0.8pt, draw=black!75},
  note/.style={font=\scriptsize, text=black!70, align=left}
]

% --- Evidence panel (2x2 grid) ---
\matrix (m) [matrix of nodes, row sep=7mm, column sep=10mm] {
  |[cNews]|   {News wires\\\& reports} &
  |[cNGO]|    {NGO bulletins} \\
  |[cSocial]| {Social media\\\scriptsize (vetted)} &
  |[cHealth]| {Public health\\\& infrastructure} \\
};

\node[hub, fit=(m-1-1)(m-2-2)] (group) {};
\node[anchor=south, font=\scriptsize, text=black!60] at (group.north)
  {Curated external evidence (ingested and vetted)};

% --- Retrieval + user query blocks ---
\node[funnel, right=18mm of group] (rag) {RAG\\\scriptsize retrieval};
\node[query, right=18mm of rag] (journalist)
  {\textbf{Journalist queries}\\\scriptsize (questions and tasks)};

% --- Clean, non-overlapping arrows from sources -> retrieval ---
\coordinate (ragTop) at ($(rag.west)+(0,0.45)$);
\coordinate (ragMid) at ($(rag.west)+(0,0.00)$);
\coordinate (ragBot) at ($(rag.west)+(0,-0.45)$);

\draw[arrow] (m-1-1.east) to[out=0,in=180] (ragTop);
\draw[arrow] (m-1-2.east) to[out=0,in=180] (ragMid);
\draw[arrow] (m-2-1.east) to[out=0,in=180] (ragMid);
\draw[arrow] (m-2-2.east) to[out=0,in=180] (ragBot);

\draw[arrow] (rag.east) -- (journalist.west);

% --- Compact note (optional; remove if you want even cleaner) ---
\node[note, below=7mm of group, anchor=west] {
...
\end{tikzpicture}
\end{adjustbox}
\caption{Evidence sources powering \ishtar{}. Curated inputs (news wires, NGO bulletins, vetted social media,
and public health \& infrastructure feeds) are routed through retrieval to support journalist queries.}
