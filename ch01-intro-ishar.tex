...
\begin{figure}[p]
  ...
  \begin{tikzpicture}[
    ...
    % Box styles - clear and non-overlapping
    surface/.style={rectangle, draw=blue!70!black, fill=blue!10, very thick, rounded corners=6pt, minimum width=2.2cm, minimum height=1.4cm, align=center, font=\footnotesize, inner sep=3pt},
    threat/.style={rectangle, draw=red!70!black, fill=red!12, very thick, rounded corners=6pt, minimum width=2.2cm, minimum height=1.3cm, align=center, font=\footnotesize, inner sep=3pt},
    control/.style={rectangle, draw=green!70!black, fill=green!10, very thick, rounded corners=6pt, minimum width=2.2cm, minimum height=1.4cm, align=center, font=\footnotesize, inner sep=3pt},
    crosscut/.style={rectangle, draw=purple!70!black, fill=purple!8, very thick, rounded corners=5pt, minimum width=11cm, minimum height=1cm, align=center, font=\footnotesize, inner sep=4pt},
    arrow/.style={->, very thick, color=black!70},
    label/.style={font=\footnotesize, color=black!60}
  ]

  % Top layer: Threat surfaces (well-spaced, no overlap)
  \node[surface] (input) at (0,6) {\begin{tabular}{c}\textbf{Untrusted input}\\\footnotesize User query\\Uploads\\Links\end{tabular}};
  \node[surface, right=0.6cm of input] (prompt) {\begin{tabular}{c}\textbf{Prompt / policy}\\\footnotesize System rules\\Templates\\Routing\end{tabular}};
  \node[surface, right=0.6cm of prompt] (retrieval) {\begin{tabular}{c}\textbf{Retrieval (RAG)}\\\footnotesize Search + filters\\Evidence bundle\\Citations\end{tabular}};
  \node[surface, right=0.6cm of retrieval] (tools) {\begin{tabular}{c}\textbf{Tools / actions}\\\footnotesize APIs\\DB writes\\Workflows\end{tabular}};
  \node[surface, right=0.6cm of tools] (output) {\begin{tabular}{c}\textbf{Model output}\\\footnotesize Answer text\\Citations\\Logs\end{tabular}};
  ...
  % Middle layer: Threats (well-spaced, no overlap)
  \node[threat, below=2cm of input] (inj) {\begin{tabular}{c}\textbf{Prompt injection}\\\footnotesize User text overrides\\instructions\end{tabular}};
  \node[threat, below=2cm of prompt] (bypass) {\begin{tabular}{c}\textbf{Policy bypass}\\\footnotesize Hidden rules exposed\\or weakened\end{tabular}};
  \node[threat, below=2cm of retrieval] (exfil) {\begin{tabular}{c}\textbf{Data exfiltration}\\\footnotesize Unauthorized docs\\retrieved\end{tabular}};
  \node[threat, below=2cm of tools] (misuse) {\begin{tabular}{c}\textbf{Tool misuse}\\\footnotesize (confused deputy)\\\footnotesize Unsafe side effects\end{tabular}};
  \node[threat, below=2cm of output] (leak) {\begin{tabular}{c}\textbf{Output leakage}\\\footnotesize (PII/secrets)\\\footnotesize Sensitive content\end{tabular}};
  ...
  % Cross-cutting element (positioned between threats and controls, no overlap)
  \node[crosscut, below=1.6cm of inj, anchor=west, xshift=-1.3cm] (tracing) {\textbf{Cross-cutting:} structured tracing + audit logs (prompt IDs, retrieved doc IDs, tool calls, policy decisions) for incident response and rollback};

  % Bottom layer: Controls (well-spaced, no overlap)
  \node[control, below=1.6cm of inj] (sanitize) {\begin{tabular}{c}\textbf{Input sanitization}\\\footnotesize Separate user vs\\system content\\Block known patterns\end{tabular}};
  \node[control, below=1.6cm of bypass] (hierarchy) {\begin{tabular}{c}\textbf{Instruction hierarchy}\\\footnotesize System > developer > user\\Constrained formats\end{tabular}};
  \node[control, below=1.6cm of exfil] (governance) {\begin{tabular}{c}\textbf{Retrieval governance}\\\footnotesize ACL checks\\Provenance + recency\\Trust allow-list\end{tabular}};
  \node[control, below=1.6cm of misuse] (toolsec) {\begin{tabular}{c}\textbf{Tool security}\\\footnotesize Allow-list tools\\Argument validation\\Sandbox side effects\end{tabular}};
  \node[control, below=1.6cm of leak] (outctrl) {\begin{tabular}{c}\textbf{Output controls}\\\footnotesize PII/secret redaction\\Citation requirements\\Rate limits \& HITL\end{tabular}};
