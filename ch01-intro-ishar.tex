...
% Vector database (below ingestion agents)
\node[vector, below=20mm of ingest2] (vectordb) {Vector\\Database};
...
% GPU inference cluster (to the right of agents, properly aligned)
\node[gpu, right=of summarizer, yshift=16mm] (gpu1) {GPU\\Node 1};
\node[gpu, right=of factchecker] (gpu2) {GPU\\Node 2};

% Response (to the right of GPU)
\node[response, right=20mm of gpu2] (response) {Response\\+ Citations};
...
\draw[arrow] (refiner.east) .. controls +(4mm,0) and +(0,-4mm) .. (gpu1.south west);

% GPU to response (horizontal)
\draw[arrow] (gpu1.east) -- (response.west);
\draw[arrowthin] (gpu2.east) .. controls +(4mm,0) and +(0,4mm) .. (response.south west);
...
\node[rectangle, fill=yellow!8, draw=black!30, dashed, line width=0.8pt, rounded corners=4pt,
      minimum width=150mm, minimum height=10mm,
