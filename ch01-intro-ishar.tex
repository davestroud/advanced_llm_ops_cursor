...
% ----------------------------------------------------------------------
% FIGURE: Call-out band (redesigned with larger boxes and better spacing)
...
\begin{tikzpicture}[
  ...
  seg/.style    ={rounded corners=3pt, minimum width=28mm, minimum height=12mm,
                  inner sep=4pt, align=center, line width=0.6pt, font=\small},
...
% Band container - larger to accommodate bigger boxes
\node[band, minimum width=0.95\linewidth, minimum height=35mm] (bandbox) {};

% Segment boxes positioned with proper spacing - start from left edge
\node[seg,cIngest, anchor=west] (ing) at ([xshift=6mm]bandbox.west) {Ingestion\\\scriptsize (feeds, ETL)};
...
\node[seg,cObs, right=of serve] (obs) {Observability\\\scriptsize (traces, SLOs)};
...
% Chapter labels above segments with proper spacing
\node[note, above=4mm of ing] (ingC) {Ch.~\ref{ch:infra}, \ref{ch:performance}};
\node[note, above=4mm of rag] (ragC) {Ch.~\ref{ch:rag}};
\node[note, above=4mm of agent] (agtC) {Ch.~\ref{ch:multiagent}};
\node[note, above=4mm of serve] (srvC) {Ch.~\ref{ch:infra}, \ref{ch:performance}};
\node[note, above=4mm of obs] (obsC) {Ch.~\ref{ch:monitoring}};
\node[note, above=4mm of test] (tstC) {Ch.~\ref{ch:testing}};
\node[note, above=4mm of eth] (ethC) {Ch.~\ref{ch:ethics}};
...
% End-to-End label positioned at top right
\node[e2e, anchor=north east] at ([xshift=-4mm,yshift=-4mm]bandbox.north east)
  {\textbf{End-to-End:} \scriptsize \ishtar{} $\rightarrow$ Ch.~\ref{ch:case-study}};

% Mapping note at bottom left
\node[note, anchor=north west] at ([xshift=4mm,yshift=-4mm]bandbox.south west)
