...
% Band container - larger to accommodate stacked boxes
\node[band, minimum width=0.95\linewidth, minimum height=55mm] (bandbox) {};

% Top row: 4 boxes (Ingestion, Retrieval, Agents, Serving)
\node[seg,cIngest] (ing) at ([xshift=-60mm,yshift=8mm]bandbox.center) {Ingestion\\\scriptsize (feeds, ETL)};
...
% Bottom row: 3 boxes (Observability, Testing, Ethics) - centered
\node[seg,cObs] (obs) at ([xshift=-40mm,yshift=-12mm]bandbox.center) {Observability\\\scriptsize (traces, SLOs)};
...
\node[note, above=5mm of ing] (ingC) {Ch.~\ref{ch:infra}, \ref{ch:performance}};
\node[note, above=5mm of rag] (ragC) {Ch.~\ref{ch:rag}};
\node[note, above=5mm of agent] (agtC) {Ch.~\ref{ch:multiagent}};
\node[note, above=5mm of serve] (srvC) {Ch.~\ref{ch:infra}, \ref{ch:performance}};

% Chapter labels above bottom row segments
\node[note, above=5mm of obs] (obsC) {Ch.~\ref{ch:monitoring}};
\node[note, above=5mm of test] (tstC) {Ch.~\ref{ch:testing}};
\node[note, above=5mm of eth] (ethC) {Ch.~\ref{ch:ethics}};
