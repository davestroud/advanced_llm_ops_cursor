...
\begin{tikzpicture}[
  ...
  % Color styles for different components - more compact sizes
  source/.style={ellipse, fill=blue!20, draw=blue!60!black, line width=0.7pt, 
                 minimum width=18mm, minimum height=9mm, align=center, inner sep=3pt},
  ingest/.style={rectangle, fill=cyan!20, draw=cyan!60!black, rounded corners=4pt,
                 line width=0.7pt, minimum width=20mm, minimum height=10mm, align=center, inner sep=3pt},
  vector/.style={cylinder, fill=teal!20, draw=teal!60!black, line width=0.7pt,
                 shape aspect=0.3, minimum width=20mm, minimum height=14mm, align=center, inner sep=3pt},
  rag/.style={rectangle, fill=purple!20, draw=purple!60!black, rounded corners=4pt,
              line width=0.7pt, minimum width=24mm, minimum height=12mm, align=center, inner sep=3pt},
  agent/.style={rectangle, fill=green!20, draw=green!60!black, rounded corners=4pt,
                line width=0.7pt, minimum width=20mm, minimum height=9mm, align=center, inner sep=2pt},
  orchestrator/.style={rectangle, fill=orange!20, draw=orange!60!black, rounded corners=4pt,
                       line width=0.8pt, minimum width=26mm, minimum height=11mm, align=center, inner sep=3pt, font=\small\bfseries},
  gpu/.style={rectangle, fill=red!20, draw=red!60!black, rounded corners=4pt,
              line width=0.7pt, minimum width=22mm, minimum height=12mm, align=center, inner sep=3pt},
  response/.style={ellipse, fill=gray!20, draw=gray!60!black, line width=0.7pt,
                  minimum width=20mm, minimum height=9mm, align=center, inner sep=3pt},
...
% ========== LEFT LANE: Data Sources & Ingestion ==========
% Data sources (left column) - start from center and work out
\node[source] (news) {News wires\\\& reports};
\node[source, below=of news] (ngo) {NGO\\bulletins};
\node[source, below=of ngo] (social) {Social media\\(vetted)};

% Ingestion agents (middle-left column)
\node[ingest, right=of news] (ingest1) {Ingestion\\Agent 1};
\node[ingest, right=of ngo] (ingest2) {Ingestion\\Agent 2};
\node[ingest, right=of social] (ingest3) {Ingestion\\Agent 3};

% Vector database (center-left)
\node[vector, right=of ingest2] (vectordb) {Vector\\Database};

% ========== MIDDLE LANE: RAG & Multi-Agent ==========
% Query input (top center)
\node[source, right=of vectordb, yshift=8mm] (query) {Journalist\\Query};

% RAG retrieval (center)
\node[rag, right=of vectordb] (retrieval) {RAG\\Retrieval};

% Multi-agent orchestration (below RAG)
\node[orchestrator, right=of vectordb, yshift=-8mm] (orchestrator) {Orchestrator};

% Agents (below orchestrator)
\node[agent, below left=of orchestrator, xshift=-3mm] (summarizer) {Summarizer};
\node[agent, below=of orchestrator] (factchecker) {Fact-Checker};
\node[agent, below right=of orchestrator, xshift=3mm] (refiner) {Refiner};

% ========== RIGHT LANE: GPU Inference & Response ==========
% GPU inference cluster
\node[gpu, right=of orchestrator] (gpu1) {GPU\\Node 1};
\node[gpu, right=of factchecker] (gpu2) {GPU\\Node 2};

% Response (far right)
\node[response, right=of gpu1] (response) {Response\\+ Citations};
...
\draw[arrowthin] (ingest1.east) -- (vectordb.west);
\draw[arrowthin] (ingest2.east) -- (vectordb.west);
\draw[arrowthin] (ingest3.east) -- (vectordb.west);

% Query to RAG
\draw[arrow] (query.south) -- (retrieval.north);

% RAG to vector database (retrieval)
