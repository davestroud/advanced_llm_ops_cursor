...
\node[vector, below=12mm of ingest2] (vectordb) {Vector\\Database};

% ========== CENTER COLUMN: Query, RAG & Orchestration ==========
% Query input (above RAG)
\node[source, right=of ingest1, yshift=8mm] (query) {Journalist\\Query};
...
% Agents (below orchestrator, in a row)
\node[agent, below left=6mm and 4mm of orchestrator] (summarizer) {Summarizer};
\node[agent, below=of orchestrator] (factchecker) {Fact-Checker};
\node[agent, below right=6mm and 4mm of orchestrator] (refiner) {Refiner};

% ========== RIGHT COLUMN: GPU & Response ==========
% GPU inference cluster (to the right of agents)
\node[gpu, right=of summarizer, yshift=8mm] (gpu1) {GPU\\Node 1};
\node[gpu, right=of factchecker] (gpu2) {GPU\\Node 2};

% Response (to the right of GPU)
\node[response, right=of gpu2] (response) {Response\\+ Citations};

% ========== ARROWS: Data Flow ==========
% Sources to ingestion (horizontal)
\draw[arrowthin] (news) -- (ingest1);
\draw[arrowthin] (ngo) -- (ingest2);
\draw[arrowthin] (social) -- (ingest3);

% Ingestion to vector database (downward)
\draw[arrowthin] (ingest1) -- (vectordb);
\draw[arrowthin] (ingest2) -- (vectordb);
\draw[arrowthin] (ingest3) -- (vectordb);

% Query to RAG (downward)
\draw[arrow] (query) -- (retrieval);

% RAG to vector database (retrieval - horizontal, dashed)
\draw[arrowthin, dashed] (retrieval) -- (vectordb);

% RAG to orchestrator (downward)
\draw[arrow] (retrieval) -- (orchestrator);

% Orchestrator to agents (downward)
\draw[arrowthin] (orchestrator) -- (summarizer);
\draw[arrowthin] (orchestrator) -- (factchecker);
\draw[arrowthin] (orchestrator) -- (refiner);

% Agents to GPU (horizontal)
\draw[arrow] (summarizer) -- (gpu1);
\draw[arrow] (factchecker) -- (gpu2);
\draw[arrow] (refiner) -- (gpu1);

% GPU to response (horizontal)
\draw[arrow] (gpu1) -- (response);
\draw[arrowthin] (gpu2) -- (response);

% ========== OBSERVABILITY LAYER ==========
% Top observability band - positioned above all components
\node[rectangle, fill=yellow!8, draw=black!30, dashed, line width=0.8pt, rounded corners=4pt,
      minimum width=120mm, minimum height=8mm,
      above=6mm of query] (obstop) {};
\node[font=\small\bfseries, color=black!85, anchor=center] at (obstop.center) {Observability \& Telemetry};

% Bottom observability band - positioned below all components
\node[rectangle, fill=yellow!8, draw=black!30, dashed, line width=0.8pt, rounded corners=4pt,
      minimum width=120mm, minimum height=8mm,
      below=6mm of vectordb] (obsbot) {};
