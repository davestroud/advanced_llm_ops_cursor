...
\begin{figure}[tb]
\centering
% Increase maximum width to accommodate larger spacing
\renewcommand{\LLMFigMaxWidth}{2.2\linewidth}
\begin{llmfigbox}
\begin{tikzpicture}[
  >=Stealth,
  node distance=12mm and 18mm,
  font=\small,
  % Professional component styles with borders and consistent colors
  source/.style={ellipse, fill=blue!15, draw=blue!40, line width=0.6pt,
                 minimum width=20mm, minimum height=9mm, align=center, inner sep=3pt, font=\small},
  ingest/.style={rectangle, fill=cyan!15, draw=cyan!40, line width=0.6pt, rounded corners=5pt,
                 minimum width=22mm, minimum height=10mm, align=center, inner sep=3pt, font=\small},
  vector/.style={cylinder, fill=teal!15, draw=teal!40, line width=0.6pt,
                 shape aspect=0.3, minimum width=22mm, minimum height=14mm, align=center, inner sep=3pt, font=\small},
  rag/.style={rectangle, fill=purple!15, draw=purple!40, line width=0.6pt, rounded corners=5pt,
              minimum width=24mm, minimum height=12mm, align=center, inner sep=3pt, font=\small\bfseries},
  agent/.style={rectangle, fill=green!15, draw=green!40, line width=0.6pt, rounded corners=5pt,
                minimum width=20mm, minimum height=9mm, align=center, inner sep=3pt, font=\footnotesize},
  orchestrator/.style={rectangle, fill=orange!15, draw=orange!50, line width=0.8pt, rounded corners=5pt,
                       minimum width=26mm, minimum height=11mm, align=center, inner sep=4pt, font=\small\bfseries},
  gpu/.style={rectangle, fill=red!15, draw=red!40, line width=0.6pt, rounded corners=5pt,
              minimum width=22mm, minimum height=12mm, align=center, inner sep=3pt, font=\small},
  response/.style={ellipse, fill=gray!15, draw=gray!40, line width=0.6pt,
                  minimum width=22mm, minimum height=9mm, align=center, inner sep=3pt, font=\small\bfseries},
  % Arrow styles
  arrow/.style={-Stealth, very thick, line width=1.2pt, color=black!75},
  arrowthin/.style={-Stealth, line width=0.8pt, color=black!60},
  arrowobs/.style={-Stealth, dashed, dash pattern=on 3pt off 2pt, line width=0.5pt, color=black!40}
]
...
% Ingestion agents (to the right of sources)
...
% Vector database (below ingestion agents, moved left and lower to avoid overlap)
\node[vector, below=24mm of ingest3, xshift=-12mm] (vectordb) {Vector\\Database};

% ========== CENTER COLUMN: Query, RAG & Orchestration ==========
% Query input (above RAG, properly spaced)
\node[source, right=of ingest1, yshift=16mm] (query) {Journalist\\Query};

% RAG retrieval (center)
\node[rag, right=of ingest2] (retrieval) {RAG\\Retrieval};

% Multi-agent orchestration (below RAG)
\node[orchestrator, below=18mm of retrieval] (orchestrator) {Orchestrator};

% Agents (below orchestrator, in a row with proper spacing)
\node[agent, below left=14mm and 8mm of orchestrator] (summarizer) {Summarizer};
\node[agent, below=14mm of orchestrator] (factchecker) {Fact-Checker};
\node[agent, below right=14mm and 8mm of orchestrator] (refiner) {Refiner};

% ========== RIGHT COLUMN: GPU & Response ==========
% GPU inference cluster (to the right of orchestrator, properly aligned)
\node[gpu, right=20mm of orchestrator, yshift=8mm] (gpu1) {GPU\\Node 1};
\node[gpu, right=20mm of orchestrator, yshift=-8mm] (gpu2) {GPU\\Node 2};

% Response (to the right of GPU, moved further right)
\node[response, right=30mm of gpu1] (response) {Response\\+ Citations};
...
% Ingestion to vector database (downward, curved to avoid overlap)
\draw[arrowthin] (ingest1.south) .. controls +(0,-4mm) and +(-2mm,0) .. (vectordb.north west);
\draw[arrowthin] (ingest2.south) -- (vectordb.north);
\draw[arrowthin] (ingest3.south) .. controls +(0,-4mm) and +(2mm,0) .. (vectordb.north east);

% Query to RAG (downward)
\draw[arrow] (query.south) -- (retrieval.north);

% RAG to vector database (retrieval - horizontal, dashed, curved)
\draw[arrowthin, dashed] (retrieval.west) .. controls +(-4mm,0) and +(0,4mm) .. (vectordb.east);

% RAG to orchestrator (downward)
\draw[arrow] (retrieval.south) -- (orchestrator.north);

% Orchestrator to agents (downward, angled)
...
% Agents to GPU (horizontal, curved to reach GPU nodes positioned right of orchestrator)
\draw[arrow] (summarizer.east) .. controls +(8mm,0) and +(-6mm,6mm) .. (gpu1.west);
\draw[arrow] (factchecker.east) .. controls +(8mm,0) and +(-6mm,0) .. (gpu2.west);
\draw[arrow] (refiner.east) .. controls +(8mm,0) and +(-6mm,-6mm) .. (gpu1.west);

% Orchestrator to GPU (direct connection, dashed)
\draw[arrowthin, dashed] (orchestrator.east) .. controls +(6mm,0) and +(-6mm,6mm) .. (gpu1.west);
\draw[arrowthin, dashed] (orchestrator.east) .. controls +(6mm,0) and +(-6mm,-6mm) .. (gpu2.west);

% GPU to response (horizontal)
\draw[arrow] (gpu1.east) -- (response.west);
\draw[arrowthin] (gpu2.east) .. controls +(8mm,0) and +(0,4mm) .. (response.south west);

% ========== OBSERVABILITY LAYER ==========
% Top observability band - positioned above all components with proper clearance
\node[rectangle, fill=yellow!8, draw=black!30, dashed, line width=0.8pt, rounded corners=4pt,
      minimum width=180mm, minimum height=10mm,
      above=14mm of query] (obstop) {};
\node[font=\small\bfseries, color=black!85, anchor=center] at (obstop.center) {Observability \& Telemetry};

% Bottom observability band - positioned below all components with proper clearance
\node[rectangle, fill=yellow!8, draw=black!30, dashed, line width=0.8pt, rounded corners=4pt,
      minimum width=180mm, minimum height=10mm,
      below=16mm of vectordb] (obsbot) {};
\node[font=\small\bfseries, color=black!85, anchor=center] at (obsbot.center) {Traces, Metrics, Logs};

% Observability connections (subtle dotted lines from bands to components)
\draw[arrowobs] ([xshift=-10mm]obstop.south) -- ([xshift=-2mm]vectordb.north);
\draw[arrowobs] (obstop.south) -- (retrieval.north);
\draw[arrowobs] ([xshift=10mm]obstop.south) -- ([xshift=2mm]orchestrator.north);
\draw[arrowobs] ([xshift=-10mm]obsbot.north) -- ([xshift=-2mm]orchestrator.south);
\draw[arrowobs] (obsbot.north) -- (gpu1.south);
\draw[arrowobs] ([xshift=10mm]obsbot.north) -- (gpu2.south);
