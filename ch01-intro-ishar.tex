...
\begin{tikzpicture}[
  ...
  phase/.style={
    ...
    text width=34mm,
    minimum height=16mm, inner sep=6pt,
...
\matrix (top) [row sep=0mm, column sep=7mm] {
...
\node[phase, fill=ragteal!12, draw=ragteal!40, below=14mm of perf] (rag) {RAG\\(Ch.~\ref{ch:rag})};
...
\node[phase, fill=testyellow!12, draw=testyellow!40, below=14mm of rag]   (test)   {Testing \& Robustness\\(Ch.~\ref{ch:testing})};
...
\path (mon.north) ++(0,22mm) coordinate (M1);
\path (fund.north) ++(0,22mm) coordinate (M2);
...
\path (case.north) ++(0,20mm) coordinate (C1);
\path (fund.north) ++(0,20mm) coordinate (C2);
