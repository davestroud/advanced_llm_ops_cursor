...
\node[vector, below=16mm of ingest2] (vectordb) {Vector\\Database};

% ========== CENTER COLUMN: Query, RAG & Orchestration ==========
% Query input (above RAG, properly spaced)
\node[source, right=of ingest1, yshift=12mm] (query) {Journalist\\Query};

% RAG retrieval (center)
\node[rag, right=of ingest2] (retrieval) {RAG\\Retrieval};

% Multi-agent orchestration (below RAG)
\node[orchestrator, below=14mm of retrieval] (orchestrator) {Orchestrator};

% Agents (below orchestrator, in a row with proper spacing)
\node[agent, below left=10mm and 6mm of orchestrator] (summarizer) {Summarizer};
\node[agent, below=10mm of orchestrator] (factchecker) {Fact-Checker};
\node[agent, below right=10mm and 6mm of orchestrator] (refiner) {Refiner};

% ========== RIGHT COLUMN: GPU & Response ==========
% GPU inference cluster (to the right of agents, properly aligned)
\node[gpu, right=of summarizer, yshift=12mm] (gpu1) {GPU\\Node 1};
\node[gpu, right=of factchecker] (gpu2) {GPU\\Node 2};

% Response (to the right of GPU)
\node[response, right=16mm of gpu2] (response) {Response\\+ Citations};
...
\node[rectangle, fill=yellow!8, draw=black!30, dashed, line width=0.8pt, rounded corners=4pt,
      minimum width=130mm, minimum height=9mm,
      above=10mm of query] (obstop) {};
