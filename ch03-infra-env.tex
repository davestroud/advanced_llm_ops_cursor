...
\noindent\textbf{Chapter roadmap.} This chapter develops the infrastructure layer as a first-class component of LLMOps.
We begin with workload-driven hardware selection (GPU/TPU and accelerator alternatives), then formalize cost and capacity planning.
Next, we cover infrastructure-as-code patterns for reproducibility and compliance, followed by containerization and Kubernetes-based orchestration for GPU fleets.
We close with serving-stack design (engines, scheduling, and memory management), deployment patterns (cloud, hybrid, multi-region), and a concrete \ishtar{} implementation blueprint.
...
% ==================================================================
% ==================================================================
\section{Hardware Selection for LLM Workloads}
...
\subsection{GPU Architectures and Choices}
...
\begin{table}[H]
  \centering
  \caption{Representative GPU accelerators for LLM workloads (throughput approximate for OPT-13B inference).}
...
\subsection{TPU Architectures and Considerations}
...
% ==================================================================
\section{Cost Modeling and Economics}
...
From an infrastructure owner's perspective, tracking cost per 1k tokens is very useful. Table~\ref{tab:cost-per-token} illustrates this with concrete examples comparing GPU options under different utilization scenarios.

\begin{table}[t]
  \centering
  \caption{Cost per thousand tokens comparison: A100 vs.\ H100 under different utilization scenarios.}
  \label{tab:cost-per-token}
  \small
...
\begin{figure}[t]
  ...
  \begin{tikzpicture}
    ...
    \end{axis}
  \end{tikzpicture}
  \caption{Throughput vs.\ batch size (schematic). Increasing batch size initially yields large throughput gains (e.g., 3$\times$ improvement from batch 1 to 4), but returns diminish beyond batch size 32. Eventually throughput saturates (batch 64 and 128 achieve similar tokens/sec) as memory or kernel launch overhead becomes the bottleneck.}
  \label{fig:batchtradeoff}
\end{figure}
...
To make the abstract formulas more tangible, Table~\ref{tab:costcompare} provides a back-of-the-envelope comparison of cost per million tokens across common accelerators. 
...
\begin{table}[H]
  \centering
  \caption{Approximate cost per million tokens across representative accelerators. Assumes typical sustained throughput for a 13B--70B model and on-demand cloud pricing. Lower is better.}
...
% ==================================================================
\section{Infrastructure-as-Code (IaC) for LLMOps}
\label{sec:iac}
...
In practice, LLMOps stacks often combine these tools. For instance, \ishtar{} uses Terraform to provision cloud resources (VMs, networks, EKS clusters) and Helm to deploy LLM services onto those clusters. A summary is shown in Table~\ref{tab:iaccompare}.

\begin{table}[H]
  \centering
  \caption{Comparison of Infrastructure-as-Code tools (selected for LLMOps).}
  \label{tab:iaccompare}
  \small
...
\begin{itemize}
  \item \textbf{Store all infrastructure code in source control.}  
  IaC should be treated the same as application code: version-controlled in Git (or equivalent), with every change reviewed and traceable~\cite{terraform-docs}.
  \item \textbf{Use remote state with locking (e.g., S3 + DynamoDB) to avoid conflicts.}  
  Storing state remotely with distributed locking ensures that multiple engineers do not make conflicting changes, preventing state corruption.
  \item \textbf{Modularize code for clusters, networking, and node groups.}  
  Encapsulation into reusable modules reduces duplication and enforces consistency across environments.
  \item \textbf{Integrate automated checks:} \texttt{terraform plan}, \texttt{tflint}, security scans, and policy-as-code.  
  Automated validation pipelines catch misconfigurations and enforce organizational policies before changes are applied~\cite{opa-docs,sentinel-docs}.
  \item \textbf{Manage secrets via secure stores (Vault, Secrets Manager).}  
  Sensitive values must never be hardcoded in templates; instead, use secure vaults or cloud-native secret managers to inject values at runtime.
  \item \textbf{Tag and name all resources for cost and ownership tracking.}  
  Consistent tagging supports cost allocation, monitoring, and compliance audits across large-scale LLM deployments.
  \item \textbf{Ensure idempotency and test rollbacks in sandbox environments.}  
  Code should safely reapply without side effects. Rollback scenarios must be validated to guarantee disaster recovery capabilities.
  \item \textbf{Use multiple environments (dev, staging, prod) with promotion gates.}  
  Changes should flow progressively, with staging as a proving ground before reaching production. Promotion gates provide controlled approvals.
  \item \textbf{Keep documentation co-located with code (README, diagrams).}  
  Documentation must evolve with the code itself, ensuring that infrastructure knowledge remains accurate and accessible.
  \item \textbf{Minimize manual console operations; backport emergency fixes into IaC immediately.}  
  The console should never be the source of truth. If an emergency console change is required, it must be reflected back into the IaC repository at once to avoid drift.
\end{itemize}
...
\subsection{Kubernetes for LLMs}
...
\begin{table}[H]
\centering
\caption{Comparison of selected LLM serving frameworks. Strengths and limitations reflect typical deployments in 2024--2025.}
\label{tab:servingcompare}
...
\begin{figure}[t]
  \centering
  % If it's too wide in svmono, uncomment ONE of these:
  % \resizebox{\linewidth}{!}{%
  ...
  \begin{tikzpicture}[
    ...
    cloud/.style={ellipse, draw=blue!60!black, fill=blue!15, thick,
      minimum width=3.6cm, minimum height=1.8cm, align=center, font=\small\bfseries},
    az/.style={rectangle, draw=green!60!black, fill=green!10, thick,
      rounded corners=5pt, minimum width=3.7cm, minimum height=4.3cm},
    k8s/.style={rectangle, draw=purple!60!black, fill=purple!10, thick,
      rounded corners=3pt, minimum width=2.9cm, minimum height=1.2cm, align=center, font=\footnotesize\bfseries},
    gpu/.style={rectangle, draw=orange!70!black, fill=orange!15, thick,
      rounded corners=3pt, minimum width=1.55cm, minimum height=0.72cm, align=center, font=\tiny},
    service/.style={rectangle, draw=teal!60!black, fill=teal!10, thick,
      rounded corners=3pt, minimum width=1.9cm, minimum height=0.78cm, align=center, font=\tiny},
    security/.style={rectangle, draw=red!60!black, fill=red!10, thick,
      rounded corners=3pt, minimum width=1.7cm, minimum height=0.72cm, align=center, font=\tiny\bfseries},
    network/.style={rectangle, draw=cyan!60!black, fill=cyan!10, thick,
      rounded corners=3pt, minimum width=2.9cm, minimum height=0.95cm, align=center, font=\tiny\bfseries},
    arrow/.style={->, thick, draw=black!60, line width=0.95pt},
    dasharrow/.style={->, thick, dashed, draw=teal!60!black, line width=0.95pt}
  ]

    % Top layers
    \node[cloud, align=center] (cloud) {Cloud Provider\\(AWS/Azure/GCP)};
    \node[network, below=0.85cm of cloud, align=center] (gateway) {API Gateway\\Load Balancer};

    \node[security, left=2.3cm of gateway]  (iam) {IAM};
    \node[security, right=2.3cm of gateway, align=center] (sg)  {Security\\Groups};

    % Availability zones containers
    \node[az, below=1.15cm of gateway, xshift=-3.9cm] (az1) {};
    \node[az, below=1.15cm of gateway, xshift= 3.9cm] (az2) {};

    \node[font=\small\bfseries, color=green!70!black, above=3pt of az1.north] (az1lbl) {AZ-1};
    \node[font=\small\bfseries, color=green!70!black, above=3pt of az2.north] (az2lbl) {AZ-2};

    % K8s clusters
    \node[k8s] (k8s1) at ($(az1.center)+(0,0.65)$) {K8s Cluster};
    \node[k8s] (k8s2) at ($(az2.center)+(0,0.65)$) {K8s Cluster};

    % GPU nodes (AZ-1)
    \node[gpu, below=0.5cm of k8s1.south, xshift=-0.68cm, align=center] (gpu1a) {GPU\\Node};
    \node[gpu, below=0.5cm of k8s1.south, align=center]                 (gpu1b) {GPU\\Node};
    \node[gpu, below=0.5cm of k8s1.south, xshift= 0.68cm, align=center] (gpu1c) {GPU\\Node};

    \node[gpu, opacity=0.35, below=0.95cm of k8s1.south, xshift=-0.34cm, align=center] (gpu1d) {GPU\\Node};
    \node[gpu, opacity=0.35, below=0.95cm of k8s1.south, xshift= 0.34cm, align=center] (gpu1e) {GPU\\Node};

    % GPU nodes (AZ-2)
    \node[gpu, below=0.5cm of k8s2.south, xshift=-0.52cm, align=center] (gpu2a) {GPU\\Node};
    \node[gpu, below=0.5cm of k8s2.south, xshift= 0.52cm, align=center] (gpu2b) {GPU\\Node};

    % Auto-scaling indicator (clean + robust)
    \draw[arrow, draw=orange!70!black, line width=1.2pt, <->]
      ($(gpu1c.east)+(0.22,0)$) -- ($(gpu1d.east)+(0.22,0)$);
    \node[font=\tiny\bfseries, color=orange!70!black, right=0.35cm of gpu1c, align=center] {Auto\\Scale};

    % Managed services stack (right side)
    \node[service, right=2.35cm of az2.east, yshift=1.05cm, align=center] (sqs)     {SQS\\Pub/Sub};
    \node[service, below=0.52cm of sqs, align=center]     (vector)  {Vector\\DB};
    \node[service, below=0.52cm of vector, align=center]  (storage) {Object\\Store};
    \node[service, below=0.52cm of storage, align=center] (bedrock) {Bedrock\\OpenAI};

    % Private subnet boundary using fit (reliable)
    \node[draw=cyan!60!black, dashed, line width=0.95pt, rounded corners=5pt,
          fit=(az1)(az2)(az1lbl)(az2lbl), inner sep=6pt] (priv) {};
    \node[font=\tiny\bfseries, color=cyan!70!black, below=2pt of priv.south] {Private Subnets};

    % DR region boundary using fit (reliable)
    \node[draw=gray!60, dotted, line width=1.2pt, rounded corners=8pt,
          fit=(sqs)(bedrock), inner sep=6pt] (dr) {};
    \node[font=\tiny\bfseries, color=gray!70, right=0.35cm of dr.east, align=center] {DR Region\\via IaC};
    ...
    % Service integration arrows (clean + non-overlapping)
    \draw[dasharrow] (k8s1.east) .. controls +(1.2,0.4) and +(-1.2,0.4) .. (sqs.west);
    \draw[dasharrow] (k8s2.east) -- (sqs.west);

    \draw[dasharrow] (k8s1.east) .. controls +(1.2,0.0) and +(-1.2,0.0) .. (vector.west);
    \draw[dasharrow] (k8s2.east) -- (vector.west);

    \draw[dasharrow] (k8s1.east) .. controls +(1.2,-0.4) and +(-1.2,-0.4) .. (storage.west);
    \draw[dasharrow] (k8s2.east) -- (storage.west);

    % Optional: show model provider calls too (you had the node but no arrow)
    \draw[dasharrow] (k8s2.east) -- (bedrock.west);

    % Legend
    \node[font=\footnotesize\bfseries, color=black!80, below=0.65cm of priv.south west, anchor=north west] (legend) {Key Features:};

    \node[gpu, below=0.25cm of legend.south west, anchor=north west, minimum width=1.2cm] (leg1) {};
    \node[font=\tiny, right=0.15cm of leg1.east, anchor=west] {GPU Nodes (Auto-scaling)};

    \node[service, below=0.22cm of leg1.south west, anchor=north west, minimum width=1.2cm] (leg2) {};
    \node[font=\tiny, right=0.15cm of leg2.east, anchor=west] {Managed Services};

    \node[security, below=0.22cm of leg2.south west, anchor=north west, minimum width=1.2cm] (leg3) {};
    \node[font=\tiny, right=0.15cm of leg3.east, anchor=west] {Security \& IAM};

  \end{tikzpicture}
  % } % end resizebox if you enable it above

  \caption{Cloud-native deployment architecture for LLMOps. The diagram illustrates elastic GPU scaling across multiple availability zones, integration with managed services (queues, vector databases, object storage), secure networking through API gateways and private subnets, and disaster recovery capabilities. GPU nodes auto-scale based on demand, while IAM and security groups enforce access controls.}
  \label{fig:cloud-native-deployment}
\end{figure}
...
\begin{tikzpicture}[
  ...
  box/.style={rectangle, rounded corners=3pt, draw=black!70, thick, align=center},
  lbl/.style={font=\small\bfseries, inner sep=2pt},
  res/.style={rectangle, rounded corners=2pt, draw=black!70, thick, minimum width=8mm, minimum height=5mm, font=\tiny, align=center},
  arrow/.style={->, thick, line width=1pt},
  cloud/.style={box, fill=blue!12, draw=blue!60!black, minimum width=3.2cm, minimum height=2.2cm, inner sep=3pt},
  vpc/.style={box, fill=blue!20, draw=blue!70!black, inner sep=2pt},
  onprem/.style={box, fill=gray!12, draw=gray!60!black, minimum width=3.2cm, minimum height=2.2cm, inner sep=3pt},
  k8s/.style={box, fill=green!12, draw=green!60!black, inner sep=2pt},
  lb/.style={res, fill=orange!20, draw=orange!70!black},
  gw/.style={res, fill=teal!20, draw=teal!70!black},
  gpu/.style={res, fill=violet!20, draw=violet!70!black},
  store/.style={res, fill=yellow!20, draw=yellow!70!black}
]

% ================== Column anchors (properly spaced) ==================
\node (col1) at (-4.5, 0) {};
\node (col2) at (0, 0) {};
\node (col3) at (4.5, 0) {};

% ================== (A) CLOUD-NATIVE ==================
\node[cloud, label={[lbl, above=0.3cm]Cloud-Native}] (cloudA) at (col1) {};
\node[vpc, minimum width=2.6cm, minimum height=1.6cm, label={[font=\footnotesize\bfseries, above=0.15cm]VPC}] (vpcA) at (cloudA.center) {};
\node[lb, minimum width=1cm, minimum height=0.5cm, align=center] (apigwA) at ($(vpcA.north) + (0, -0.3cm)$) {API\\GW};
\node[k8s, minimum width=2.2cm, minimum height=0.7cm, label={[font=\tiny\bfseries, above=0.1cm]K8s}] (k8sA) at ($(vpcA.center) + (0, -0.2cm)$) {};
\node[gpu, minimum width=0.7cm] (gpuA1) at ($(k8sA.south) + (-0.4cm, -0.25cm)$) {GPU};
\node[gpu, minimum width=0.7cm] (gpuA2) at ($(k8sA.south) + (0.4cm, -0.25cm)$) {GPU};
\node[store, minimum width=1cm, align=center] (objA) at ($(gpuA1.south) + (0.4cm, -0.3cm)$) {Object\\Store};

\draw[arrow, orange!80!black] (apigwA) -- (k8sA);
\draw[arrow, violet!80!black] (k8sA) -- (objA);

% ================== (B) HYBRID ==================
\node[onprem, label={[lbl, above=0.3cm]Hybrid}] (premB) at ($(col2) + (-1.2cm, 0)$) {};
\node[k8s, minimum width=2.2cm, minimum height=0.7cm, label={[font=\tiny\bfseries, above=0.1cm]On-Prem K8s}] (k8sBprem) at ($(premB.center) + (0, 0.1cm)$) {};
\node[gpu, minimum width=0.7cm] (gpuB1) at ($(k8sBprem.south) + (-0.4cm, -0.25cm)$) {GPU};
\node[gpu, minimum width=0.7cm] (gpuB2) at ($(k8sBprem.south) + (0.4cm, -0.25cm)$) {GPU};
\node[store, minimum width=1cm, align=center] (vecB) at ($(gpuB1.south) + (0.4cm, -0.3cm)$) {Vector\\DB};

\node[cloud] (cloudB) at ($(col2) + (1.2cm, 0)$) {};
\node[vpc, minimum width=2.6cm, minimum height=1.6cm, label={[font=\footnotesize\bfseries, above=0.15cm]Cloud VPC}] (vpcB) at (cloudB.center) {};
\node[lb, minimum width=1cm, minimum height=0.5cm, align=center] (apigwB) at ($(vpcB.north) + (0, -0.3cm)$) {API\\GW};
\node[k8s, minimum width=2.2cm, minimum height=0.7cm, label={[font=\tiny\bfseries, above=0.1cm]Cloud K8s}] (k8sBcloud) at ($(vpcB.center) + (0, -0.2cm)$) {};
\node[gpu, minimum width=0.7cm] (gpuB3) at ($(k8sBcloud.south) + (-0.4cm, -0.25cm)$) {GPU};
\node[gpu, minimum width=0.7cm] (gpuB4) at ($(k8sBcloud.south) + (0.4cm, -0.25cm)$) {GPU};

\node[gw, minimum width=0.9cm, minimum height=0.5cm] (vpnL) at ($(premB.east) + (0.3cm, 0)$) {VPN};
\node[gw, minimum width=0.9cm, minimum height=0.5cm] (vpnR) at ($(cloudB.west) + (-0.3cm, 0)$) {VPN};

\draw[arrow, teal!80!black] (k8sBprem) -- (vpnL);
\draw[arrow, teal!80!black] (vpnR) -- (k8sBcloud);
\draw[arrow, gray!70, dashed, line width=1.2pt] (vpnL) -- node[font=\tiny, align=center, midway, above] {Direct\\Connect} (vpnR);
\draw[arrow, violet!80!black] (k8sBprem) -- (vecB);
\draw[arrow, orange!80!black] (apigwB) -- (k8sBcloud);

% ================== (C) MULTI-CLUSTER ==================
\node[cloud, label={[lbl, above=0.3cm]Multi-Cluster / Multi-Region}] (cloudC) at (col3) {};
\node[lb, minimum width=1.2cm, minimum height=0.5cm, align=center] (glbC) at ($(cloudC.north) + (0, -0.2cm)$) {Global\\LB/DNS};

\node[vpc, minimum width=2.4cm, minimum height=1.6cm, label={[font=\footnotesize\bfseries, above=0.15cm]Region A}] (vpcC1) at ($(cloudC.center) + (-0.7cm, -0.1cm)$) {};
\node[k8s, minimum width=2cm, minimum height=0.7cm, label={[font=\tiny\bfseries, above=0.1cm]K8s A}] (k8sC1) at ($(vpcC1.center) + (0, 0.1cm)$) {};
\node[gpu, minimum width=0.7cm] (gpuC1) at ($(k8sC1.south) + (-0.35cm, -0.25cm)$) {GPU};
\node[gpu, minimum width=0.7cm] (gpuC2) at ($(k8sC1.south) + (0.35cm, -0.25cm)$) {GPU};

\node[vpc, minimum width=2.4cm, minimum height=1.6cm, label={[font=\footnotesize\bfseries, above=0.15cm]Region B}] (vpcC2) at ($(cloudC.center) + (0.7cm, -0.1cm)$) {};
\node[k8s, minimum width=2cm, minimum height=0.7cm, label={[font=\tiny\bfseries, above=0.1cm]K8s B}] (k8sC2) at ($(vpcC2.center) + (0, 0.1cm)$) {};
\node[gpu, minimum width=0.7cm] (gpuC3) at ($(k8sC2.south) + (-0.35cm, -0.25cm)$) {GPU};
\node[gpu, minimum width=0.7cm] (gpuC4) at ($(k8sC2.south) + (0.35cm, -0.25cm)$) {GPU};

\draw[arrow, orange!80!black] (glbC) -- ($(glbC) + (-0.7cm, -0.5cm)$) -- (k8sC1);
\draw[arrow, orange!80!black] (glbC) -- ($(glbC) + (0.7cm, -0.5cm)$) -- (k8sC2);
\draw[arrow, gray!70, dashed, line width=1.2pt] (k8sC1) -- node[font=\tiny, align=center, midway, above, sloped] {spillover/\\failover} (k8sC2);

% ================== Clean legend (below all patterns) ==================
\node[box, fill=white!95, draw=black!50, rounded corners=4pt, minimum width=14cm, minimum height=1.2cm, inner sep=4pt] (legend) at ($(cloudB.south) + (0, -1.8cm)$) {};

\node[cloud, minimum width=8mm, minimum height=5mm] (L1) at ($(legend.west) + (0.8cm, 0.25cm)$) {};
\node[font=\tiny, right=0.15cm of L1.east, anchor=west] {Cloud/Region};

\node[vpc, minimum width=8mm, minimum height=5mm] (L2) at ($(L1.east) + (1cm, 0)$) {};
\node[font=\tiny, right=0.15cm of L2.east, anchor=west] {VPC};

\node[k8s, minimum width=8mm, minimum height=5mm] (L3) at ($(L2.east) + (1cm, 0)$) {};
\node[font=\tiny, right=0.15cm of L3.east, anchor=west] {K8s};

\node[gpu, minimum width=8mm, minimum height=5mm] (L4) at ($(L3.east) + (1cm, 0)$) {};
\node[font=\tiny, right=0.15cm of L4.east, anchor=west] {GPU Node};

\node[lb, minimum width=8mm, minimum height=5mm] (L5) at ($(L1) + (0, -0.5cm)$) {};
\node[font=\tiny, right=0.15cm of L5.east, anchor=west] {API GW/LB};

\node[gw, minimum width=8mm, minimum height=5mm] (L6) at ($(L2) + (0, -0.5cm)$) {};
\node[font=\tiny, right=0.15cm of L6.east, anchor=west] {VPN/Direct Connect};

\node[store, minimum width=8mm, minimum height=5mm] (L7) at ($(L3) + (0, -0.5cm)$) {};
\node[font=\tiny, right=0.15cm of L7.east, anchor=west] {Object/Vector Store};

\end{tikzpicture}
\caption{Balanced schematic of deployment patterns: \textbf{Cloud-Native} (left), \textbf{Hybrid} (middle), and \textbf{Multi-Cluster} (right).}
\label{fig:deploy-patterns-balanced}
\end{figure}
...
\begin{tikzpicture}[
  ...
  box/.style={rectangle, rounded corners, draw=black, thick, align=center, font=\scriptsize},
  gpu/.style={rectangle, rounded corners=2pt, draw=black, very thick,
              minimum width=12mm, minimum height=6mm, font=\scriptsize, align=center},
  arrow/.style={-{Stealth[length=2.5mm,width=1.8mm]}, very thick},
  lbl/.style={font=\scriptsize}
]

% ============= On-Prem =============
\node[box, fill=gray!15, minimum width=38mm, minimum height=28mm,
      label={[lbl]above:{\textbf{On-Prem (L4 Cluster)}}}] (onprem) {};
\node[gpu, fill=violet!25] (l4a) at ([yshift=5mm]onprem.center) {L4 GPU};
\node[gpu, fill=violet!25, right=3mm of l4a] (l4b) {L4 GPU};
\node[box, fill=yellow!25, below=4mm of onprem.center, align=center] (embed) {Embeddings /\\NER Tagging};
\draw[arrow, gray!70] (l4a) -- (embed);
\draw[arrow, gray!70] (l4b) -- (embed);

% VPN to cloud
\node[box, fill=teal!25, minimum width=12mm, minimum height=6mm,
      right=8mm of onprem.east] (vpn) {VPN};

\draw[arrow, teal!80] (onprem.east) -- (vpn.west);

% ============= Cloud Baseline A100 =============
\node[box, fill=blue!10, minimum width=48mm, minimum height=36mm,
      right=12mm of vpn, label={[lbl]above:{\textbf{Cloud (AWS EKS)}}}] (cloud) {};

\node[box, fill=green!10, minimum width=42mm, minimum height=14mm,
      above=2mm of cloud.south, anchor=south, label={[lbl]above:{Baseline Node Group}}] (a100group) {};
\node[gpu, fill=violet!25] (a100a) at ([xshift=-10mm]a100group.center) {A100};
\node[gpu, fill=violet!25] (a100b) at ([xshift= 10mm]a100group.center) {A100};

\node[box, fill=green!10, minimum width=42mm, minimum height=14mm,
      above=2mm of a100group.north, anchor=south, label={[lbl]above:{Autoscaling Node Group}}] (h100group) {};
\node[gpu, fill=violet!40] (h100a) at ([xshift=-10mm]h100group.center) {H100};
\node[gpu, fill=violet!40] (h100b) at ([xshift= 10mm]h100group.center) {H100};

% Serving pods
\node[box, fill=orange!25, above=2mm of h100group.north, anchor=south,
      minimum width=42mm, minimum height=10mm] (serving) {Model Serving Pods (TGI / vLLM)};

% Terraform + K8s control
\node[box, fill=blue!25, minimum width=28mm, minimum height=8mm,
...
% ============= Legend =============
\node[box, fill=white, draw=black, very thick, inner sep=1mm,
      below=10mm of cloud.south, minimum width=0.9\linewidth] (legend) {};

\node[gpu, fill=violet!25, minimum width=8mm, minimum height=5mm, label={[lbl]right:L4 / A100 GPU}] (L1) at ($(legend.west) + (0.5cm, 0.15cm)$) {};
\node[gpu, fill=violet!40, minimum width=8mm, minimum height=5mm, label={[lbl]right:H100 GPU}] (L2) at ($(L1.east) + (2cm, 0)$) {};
\node[box, fill=orange!25, minimum width=8mm, minimum height=5mm, label={[lbl]right:Serving Pods}] (L3) at ($(L2.east) + (2cm, 0)$) {};
\node[box, fill=blue!25, minimum width=8mm, minimum height=5mm, label={[lbl]right:IaC / Control Plane}] (L4) at ($(L3.east) + (2cm, 0)$) {};

\end{tikzpicture}
\caption{Hybrid Ishtar AI infrastructure. On-prem L4 GPUs handle embeddings and preprocessing; AWS EKS hosts baseline A100s and autoscaling H100s for summarization. Terraform, ArgoCD, and Kubernetes manage deployments and scaling.}
\label{fig:ishtar-infra}
\end{figure}
...
\begin{itemize}
  \item \textbf{Ensure Sufficient GPU Memory.} Calculate or empirically measure memory usage (including KV cache) at target sequence lengths. Avoid contexts that trigger out-of-memory errors. For longer contexts, employ higher-memory GPUs (80GB A100/H100) or KV compression. As a rule of thumb, a 13B model consumes $\sim$1.6 GB per 2048 tokens~\cite{vllm-pagedattention,pagedattention_paper}.
  \item \textbf{Benchmark Cost-Per-Token.} For each GPU/TPU candidate, benchmark tokens/sec and compute cost per 1k tokens~\cite{fabricated-knowledge-costs,lambda-gpt3-cost}. A more expensive hourly instance may be cheaper per output due to higher throughput.
  \item \textbf{Utilization Monitoring.} Keep GPUs busy. If average utilization $<$30\%, increase batch sizes or consolidate jobs. Low utilization = wasted money.
  \item \textbf{Batching Tuning.} Determine optimal batch size relative to latency goals. Benchmark throughput/latency curves (e.g., with Hugging Face’s scripts) and configure dynamic batching accordingly~\cite{tgi-github,tgi_docs}.
  \item \textbf{Adaptive Batching \& Queuing.} Introduce short request queues (10–30 ms) to improve batching without perceptible user impact. Use servers/frameworks that natively support this.
  \item \textbf{Profile End-to-End Latency.} Break down latency into preprocessing, model compute, post-processing, and network transfer. Optimize each stage; GPU-accelerate post-processing if necessary.
  \item \textbf{Plan for Spikes.} Define peak QPS and establish autoscaling or load-shedding strategies. Test spike scenarios to ensure capacity expansion occurs in time.
  \item \textbf{Graceful Degradation.} Employ fallback mechanisms under overload: route to faster, smaller models, or reduce output length. Ensemble routing approaches such as Smoothie illustrate this principle~\cite{smoothie-routing}.
  \item \textbf{Log and Analyze Tail Latencies.} Monitor p95/p99 latency for outliers. Identify causes (e.g., long inputs, hardware stalls) and mitigate (reject ultra-long prompts, split jobs).
  \item \textbf{Thermal Monitoring.} In on-prem clusters, monitor GPU temperature and power draw. Thermal throttling reduces throughput; adequate cooling and power monitoring are essential.
\end{itemize}
...
\begin{itemize}
  \item \textbf{Version Control Everything.} All infrastructure changes should flow through Git; no manual console edits~\cite{terraform-docs,pulumi-docs}.
  \item \textbf{Use Remote State with Locking (e.g., S3 + DynamoDB) to Avoid Conflicts.} Storing state remotely with distributed locking ensures that multiple engineers do not make conflicting changes, preventing state corruption.
  \item \textbf{Modularize Code for Clusters, Networking, and Node Groups.} Encapsulation into reusable modules reduces duplication and enforces consistency across environments.
  \item \textbf{Integrate Automated Checks:} \texttt{terraform plan}, \texttt{tflint}, security scans, and policy-as-code. Automated validation pipelines catch misconfigurations and enforce organizational policies before changes are applied~\cite{opa-docs,sentinel-docs}.
  \item \textbf{Manage Secrets via Secure Stores (Vault, Secrets Manager).} Sensitive values must never be hardcoded in templates; instead, use secure vaults or cloud-native secret managers to inject values at runtime.
  \item \textbf{Tag and Name All Resources for Cost and Ownership Tracking.} Consistent tagging supports cost allocation, monitoring, and compliance audits across large-scale LLM deployments.
  \item \textbf{Ensure Idempotency and Test Rollbacks in Sandbox Environments.} Code should safely reapply without side effects. Rollback scenarios must be validated to guarantee disaster recovery capabilities.
  \item \textbf{Use Multiple Environments (dev, staging, prod) with Promotion Gates.} Changes should flow progressively, with staging as a proving ground before reaching production. Promotion gates provide controlled approvals.
  \item \textbf{Keep Documentation Co-located with Code (README, diagrams).} Documentation must evolve with the code itself, ensuring that infrastructure knowledge remains accurate and accessible.
  \item \textbf{Minimize Manual Console Operations; Backport Emergency Fixes into IaC Immediately.} The console should never be the source of truth. If an emergency console change is required, it must be reflected back into the IaC repository at once to avoid drift.
\end{itemize}
...
\begin{itemize}
  \item \textbf{Use Health Probes.} Configure liveness and readiness probes for all LLM pods~\cite{nvidia-k8s-device-plugin}.
  \item \textbf{Enable Logging and Tracing.} Integrate tracing (e.g., OpenTelemetry) and correlation IDs. Log major lifecycle events (model load, request timings).
  \item \textbf{Batching Enabled.} Verify serving stack batches requests effectively. Enable dynamic batching in frameworks (TGI, Triton, vLLM) and tune batch delays~\cite{tgi-github,vllm-pagedattention}.
  \item \textbf{Autoscaling Policies.} Configure horizontal pod autoscalers (by QPS, latency, or custom metrics) and GPU node autoscaling. Test synthetic loads for responsiveness.
  \item \textbf{Graceful Shutdown.} Use \texttt{preStop} hooks and termination grace periods to let pods drain requests before eviction.
  \item \textbf{Model Rollout Strategy.} Adopt rolling or canary updates. Never replace all pods simultaneously; validate new model quality before full rollout.
  \item \textbf{Concurrency Limits.} Enforce maximum concurrent requests per instance. Return “retry later” rather than crashing under overload.
  \item \textbf{Resource Requests \& Limits.} Assign accurate CPU/GPU/memory requests so schedulers pack pods appropriately.
  \item \textbf{Error Handling.} Implement retries for transient errors. Handle OOMs gracefully (e.g., friendly error or fallback response).
  \item \textbf{Secure the Endpoint.} Restrict access with mTLS, network policies, or API gateways. Never expose open model endpoints.
  \item \textbf{Observability on Quality.} Track not only performance but also output quality (manual ratings or automated classifiers). Monitor drift.
  \item \textbf{Plan for Scaling Limits.} Know cluster scaling ceilings (e.g., max 10 H100 nodes). Plan partitioning or caching before reaching limits.
  \item \textbf{Client-Side Rate Limiting.} Apply per-user/API key request limits to prevent misuse and runaway costs.
  \item \textbf{Continuous Load Testing.} Periodically stress test systems after major changes (new models, infra updates) to ensure scaling and SLAs hold.
\end{itemize}
