...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[
    ...
    cloud/.style={ellipse, draw=blue!60!black, fill=blue!15, thick, minimum width=3.5cm, minimum height=1.8cm, align=center, font=\small\bfseries},
    az/.style={rectangle, draw=green!60!black, fill=green!10, thick, rounded corners=5pt, minimum width=3.5cm, minimum height=4.2cm},
    k8s/.style={rectangle, draw=purple!60!black, fill=purple!10, thick, rounded corners=3pt, minimum width=2.8cm, minimum height=1.2cm, align=center, font=\footnotesize\bfseries},
    gpu/.style={rectangle, draw=orange!70!black, fill=orange!15, thick, rounded corners=3pt, minimum width=1.5cm, minimum height=0.7cm, align=center, font=\tiny},
    service/.style={rectangle, draw=teal!60!black, fill=teal!10, thick, rounded corners=3pt, minimum width=1.8cm, minimum height=0.75cm, align=center, font=\tiny},
    security/.style={rectangle, draw=red!60!black, fill=red!10, thick, rounded corners=3pt, minimum width=1.6cm, minimum height=0.7cm, align=center, font=\tiny\bfseries},
    network/.style={rectangle, draw=cyan!60!black, fill=cyan!10, thick, rounded corners=3pt, minimum width=2.8cm, minimum height=0.9cm, align=center, font=\tiny\bfseries},
    arrow/.style={->, thick, color=black!60, line width=1pt},
    label/.style={font=\scriptsize, color=black!70}
  ]
    
    % Cloud provider background
    \node[cloud] (cloud) at (0, 6) {\textbf{Cloud Provider}\\(AWS/Azure/GCP)};
    
    % API Gateway / Load Balancer layer
    \node[network, below=0.8cm of cloud] (gateway) {API Gateway\\Load Balancer};
    
    % Security layer - properly aligned
    \node[security, left=2.2cm of gateway] (iam) {IAM};
    \node[security, right=2.2cm of gateway, align=center] (sg) {Security\\Groups};
    
    % Availability Zones - properly aligned and sized
    \node[az, below=1.1cm of gateway, xshift=-3.8cm] (az1) {};
    \node[font=\small\bfseries, color=green!70!black, above=3pt of az1.north] {AZ-1};
    
    \node[az, below=1.1cm of gateway, xshift=3.8cm] (az2) {};
    \node[font=\small\bfseries, color=green!70!black, above=3pt of az2.north] {AZ-2};
    
    % Kubernetes clusters in each AZ - centered horizontally
    \node[k8s] (k8s1) at ($(az1.center) + (0, 0.6)$) {K8s Cluster};
    \node[k8s] (k8s2) at ($(az2.center) + (0, 0.6)$) {K8s Cluster};
    
    % GPU nodes - perfectly centered and aligned under K8s clusters
    % AZ-1: 3 nodes in top row (centered), 2 nodes in bottom row (scaling)
    \node[gpu, below=0.5cm of k8s1.south, xshift=-0.65cm] (gpu1a) {GPU\\Node};
    \node[gpu, below=0.5cm of k8s1.south] (gpu1b) {GPU\\Node};
    \node[gpu, below=0.5cm of k8s1.south, xshift=0.65cm] (gpu1c) {GPU\\Node};
    \node[gpu, opacity=0.4, below=0.95cm of k8s1.south, xshift=-0.325cm] (gpu1d) {GPU\\Node};
    \node[gpu, opacity=0.4, below=0.95cm of k8s1.south, xshift=0.325cm] (gpu1e) {GPU\\Node};
    
    % AZ-2: 2 nodes perfectly centered
    \node[gpu, below=0.5cm of k8s2.south, xshift=-0.5cm] (gpu2a) {GPU\\Node};
    \node[gpu, below=0.5cm of k8s2.south, xshift=0.5cm] (gpu2b) {GPU\\Node};
    
    % Auto-scaling indicator - better positioned
    \draw[arrow, color=orange!70!black, line width=1.5pt, <->] ($(gpu1c.east) + (0.2, 0)$) -- ($(gpu1d.east) + (0.2, 0)$);
    \node[font=\tiny\bfseries, color=orange!70!black, right=0.25cm of gpu1c, align=center] {Auto\\Scale};
    
    % Managed services - properly aligned vertically, centered
    \node[service, right=2.2cm of az2.east, yshift=1cm] (sqs) {SQS\\Pub/Sub};
    \node[service, below=0.5cm of sqs] (vector) {Vector\\DB};
    \node[service, below=0.5cm of vector] (storage) {Object\\Store};
    \node[service, below=0.5cm of storage] (bedrock) {Bedrock\\OpenAI};
    
    % Private subnet indicator - properly sized to encompass both AZs
    \draw[dashed, color=cyan!60!black, line width=1pt, rounded corners=5pt] ($(az1.south west) + (-0.3, -0.3)$) rectangle ($(az2.north east) + (0.3, 0.3)$);
    \node[font=\tiny\bfseries, color=cyan!70!black, below=0.2cm of az1.south] {Private Subnets};
    
    % Connections - cleaner, more direct paths
    ...
    \draw[arrow, dashed, color=teal!60] (k8s1.east) .. controls (3.5, 3.2) .. (sqs.west);
    \draw[arrow, dashed, color=teal!60] (k8s2.east) -- (sqs.west);
    \draw[arrow, dashed, color=teal!60] (k8s1.east) .. controls (3.5, 2.2) .. (vector.west);
    \draw[arrow, dashed, color=teal!60] (k8s2.east) -- (vector.west);
    \draw[arrow, dashed, color=teal!60] (k8s1.east) .. controls (3.5, 1.2) .. (storage.west);
    \draw[arrow, dashed, color=teal!60] (k8s2.east) -- (storage.west);
    
    % Disaster Recovery indicator - properly positioned around services
    \draw[dotted, color=gray!60, line width=1.5pt, rounded corners=8pt] ($(sqs.north east) + (0.2, 0.2)$) rectangle ($(bedrock.south east) + (0.2, -0.2)$);
    \node[font=\tiny\bfseries, color=gray!70, right=0.3cm of sqs.east, align=center] {DR Region\\via IaC};
    
    % Legend - properly aligned with consistent spacing
    \node[font=\footnotesize\bfseries, color=black!80, below=0.6cm of az1.south west, anchor=north west, xshift=-0.5cm] (legend) {Key Features:};
    \node[gpu, below=0.2cm of legend.south west, anchor=north west, minimum width=1.2cm] (leg1) {};
    \node[font=\tiny, right=0.15cm of leg1.east, anchor=west] {GPU Nodes (Auto-scaling)};
    \node[service, below=0.2cm of leg1.south west, anchor=north west, minimum width=1.2cm] (leg2) {};
    \node[font=\tiny, right=0.15cm of leg2.east, anchor=west] {Managed Services};
    \node[security, below=0.2cm of leg2.south west, anchor=north west, minimum width=1.2cm] (leg3) {};
...
\begin{tikzpicture}[
  ...
  box/.style={rectangle, rounded corners, draw=black, thick, align=center, inner sep=1pt},
  lbl/.style={font=\scriptsize, inner sep=0pt},
  res/.style={rectangle, rounded corners=1pt, draw=black, thick, minimum width=5.6mm, minimum height=3.6mm, font=\scriptsize, align=center},
  arrow/.style={-{Stealth[length=1.6mm,width=1.2mm]}, thick},
  cloud/.style={box, fill=blue!10, inner sep=1mm, minimum width=26mm, minimum height=18mm},
  vpc/.style={box, fill=blue!18, inner sep=0.6mm},
  onprem/.style={box, fill=gray!10, inner sep=1mm, minimum width=26mm, minimum height=18mm},
  k8s/.style={box, fill=green!10, inner sep=0.6mm},
  lb/.style={res, fill=orange!25},
  gw/.style={res, fill=teal!25},
  gpu/.style={res, fill=violet!25},
  store/.style={res, fill=yellow!25}
]

% ---------- Column anchors for visual balance ----------
\node (col1anchor) at (0,0) {};
\node (col2anchor) [right=32mm of col1anchor] {};
\node (col3anchor) [right=32mm of col2anchor] {};

% ================== (A) CLOUD-NATIVE ==================
\node[cloud, label={[lbl]above:{\textbf{Cloud-Native}}}] (cloudA) at (col1anchor) {};
\node[vpc, minimum width=22mm, minimum height=14mm, label={[lbl]above:{VPC}}] (vpcA) at (cloudA.center) {};
\node[lb] (apigwA) at ([yshift=3.2mm]vpcA.north) {API GW};
\node[k8s, minimum width=20mm, minimum height=8mm, label={[lbl]above:{K8s}}] (k8sA) at ([yshift=-1mm]apigwA.south) {};
\node[gpu] (gpuA1) at ([xshift=-5mm,yshift=-1mm]k8sA.center) {GPU};
\node[gpu] (gpuA2) at ([xshift= 5mm,yshift=-1mm]k8sA.center) {GPU};
\node[store] (objA) [below=2.3mm of k8sA] {Obj};

\draw[arrow, orange!80] (apigwA) -- (k8sA);
\draw[arrow, violet!80] (k8sA) -- (objA);

% ================== (B) HYBRID ==================
\node[onprem, label={[lbl]above:{\textbf{Hybrid}}}] (premB) at (col2anchor) {};
\node[k8s, minimum width=22mm, minimum height=8mm, label={[lbl]above:{On-Prem}}] (k8sBprem) at ([xshift=-6mm]premB.center) {};
\node[gpu] (gpuB1) at ([xshift=-5mm,yshift=-1mm]k8sBprem.center) {GPU};
\node[gpu] (gpuB2) at ([xshift= 5mm,yshift=-1mm]k8sBprem.center) {GPU};
\node[store] (vecB) [below=2.3mm of k8sBprem] {Vec DB};

\node[cloud] (cloudB) at ([xshift=13mm]premB.east) {};
\node[vpc, minimum width=22mm, minimum height=14mm, label={[lbl]above:{Cloud}}] (vpcB) at (cloudB.center) {};
\node[lb] (apigwB) at ([yshift=3.2mm]vpcB.north) {API GW};
\node[k8s, minimum width=20mm, minimum height=8mm, label={[lbl]above:{Cloud K8s}}] (k8sBcloud) at ([yshift=-1mm]apigwB.south) {};
\node[gpu] (gpuB3) at ([xshift=-5mm,yshift=-1mm]k8sBcloud.center) {GPU};
\node[gpu] (gpuB4) at ([xshift= 5mm,yshift=-1mm]k8sBcloud.center) {GPU};

\node[gw, minimum width=7mm] (vpnL) at ([xshift=6mm]premB.east) {VPN};
\node[gw, minimum width=7mm] (vpnR) at ([xshift=-6mm]cloudB.west) {VPN};

\draw[arrow, teal!80] (k8sBprem) -- (vpnL);
\draw[arrow, teal!80] (vpnR) -- (k8sBcloud);
\draw[arrow, gray!70, dashed] (vpnL) -- (vpnR);
\draw[arrow, violet!80] (k8sBprem) -- (vecB);
\draw[arrow, orange!80] (apigwB) -- (k8sBcloud);

% ================== (C) MULTI-CLUSTER ==================
\node[cloud, label={[lbl]above:{\textbf{Multi-Cluster / Multi-Region}}}] (cloudC) at (col3anchor) {};
\node[lb] (glbC) at ([yshift=3.6mm]cloudC.north) {Global LB/DNS};

% Make both regions equal height to balance vertically
\node[vpc, minimum width=23mm, minimum height=14mm, label={[lbl]above:{Region A}}] (vpcC1) at ([xshift=-6mm,yshift=-0.3mm]cloudC.center) {};
\node[k8s, minimum width=19mm, minimum height=7mm, label={[lbl]above:{K8s A}}] (k8sC1) at (vpcC1.center) {};
\node[gpu] (gpuC1) at ([xshift=-5mm,yshift=-1mm]k8sC1.center) {GPU};
\node[gpu] (gpuC2) at ([xshift= 5mm,yshift=-1mm]k8sC1.center) {GPU};

\node[vpc, minimum width=23mm, minimum height=14mm, label={[lbl]above:{Region B}}] (vpcC2) at ([xshift=8mm,yshift=-0.3mm]cloudC.center) {};
\node[k8s, minimum width=19mm, minimum height=7mm, label={[lbl]above:{K8s B}}] (k8sC2) at (vpcC2.center) {};
\node[gpu] (gpuC3) at ([xshift=-5mm,yshift=-1mm]k8sC2.center) {GPU};
\node[gpu] (gpuC4) at ([xshift= 5mm,yshift=-1mm]k8sC2.center) {GPU};

\draw[arrow, orange!80] (glbC) |- (k8sC1);
\draw[arrow, orange!80] (glbC) |- (k8sC2);
\draw[arrow, gray!70, dashed] (k8sC1) -- node[lbl, midway, fill=white, inner sep=0.5pt]{spillover/failover} (k8sC2);

% ================== Two-row compact legend ==================
\node[box, fill=white, draw=black, inner sep=0.8mm, minimum width=0.86\linewidth] (legend) at ([yshift=-13mm]cloudB.south) {
\begin{tikzpicture}[baseline=(current bounding box.center), node distance=1mm and 2mm]
  % Row 1
  \node[cloud, minimum width=6mm, minimum height=4mm, label={[lbl]right:Cloud/Region}] (L1) {};
  \node[vpc,   minimum width=6mm, minimum height=4mm, right=4mm of L1, label={[lbl]right:VPC}] (L2) {};
  \node[k8s,   minimum width=6mm, minimum height=4mm, right=4mm of L2, label={[lbl]right:K8s}] (L3) {};
  \node[gpu,   minimum width=6mm, minimum height=4mm, right=6mm of L3, label={[lbl]right:GPU Node}] (L4) {};

  % Row 2
  \node[lb,    minimum width=6mm, minimum height=4mm, below=2mm of L1, label={[lbl]right:API GW/LB}] (L5) {};
  \node[gw,    minimum width=6mm, minimum height=4mm, right=4mm of L5, label={[lbl]right:VPN/Direct Connect}] (L6) {};
  \node[store, minimum width=6mm, minimum height=4mm, right=4mm of L6, label={[lbl]right:Object/Vector Store}] (L7) {};
\end{tikzpicture}
};
