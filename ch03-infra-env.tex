...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[
    ...
    az/.style={rectangle, draw=green!60!black, fill=green!10, thick, rounded corners=5pt, minimum width=3.2cm, minimum height=4cm},
    k8s/.style={rectangle, draw=purple!60!black, fill=purple!10, thick, rounded corners=3pt, minimum width=2.5cm, minimum height=1.1cm, align=center, font=\footnotesize\bfseries},
    gpu/.style={rectangle, draw=orange!70!black, fill=orange!15, thick, rounded corners=3pt, minimum width=1.6cm, minimum height=0.75cm, align=center, font=\tiny},
    ...
    network/.style={rectangle, draw=cyan!60!black, fill=cyan!10, thick, rounded corners=3pt, minimum width=2.5cm, minimum height=0.8cm, align=center, font=\tiny\bfseries},
    ...
    \node[security, left=2cm of gateway] (iam) {IAM};
    \node[security, right=2cm of gateway, align=center] (sg) {Security\\Groups};
    
    % Availability Zones - properly aligned and sized
    \node[az, below=1cm of gateway, xshift=-3.5cm] (az1) {};
    \node[font=\small\bfseries, color=green!70!black, above=3pt of az1.north] {AZ-1};
    
    \node[az, below=1cm of gateway, xshift=3.5cm] (az2) {};
    \node[font=\small\bfseries, color=green!70!black, above=3pt of az2.north] {AZ-2};
    
    % Kubernetes clusters in each AZ - centered
    \node[k8s] (k8s1) at ($(az1.center) + (0, 0.5)$) {K8s Cluster};
    \node[k8s] (k8s2) at ($(az2.center) + (0, 0.5)$) {K8s Cluster};
    
    % GPU nodes - properly aligned in grid
    \node[gpu] (gpu1a) at ($(k8s1.south) + (-0.7, -0.4)$) {GPU\\Node};
    \node[gpu] (gpu1b) at ($(k8s1.south) + (0, -0.4)$) {GPU\\Node};
    \node[gpu] (gpu1c) at ($(k8s1.south) + (0.7, -0.4)$) {GPU\\Node};
    \node[gpu, opacity=0.4] (gpu1d) at ($(k8s1.south) + (-0.35, -0.8)$) {GPU\\Node};
    \node[gpu, opacity=0.4] (gpu1e) at ($(k8s1.south) + (0.35, -0.8)$) {GPU\\Node};
    
    \node[gpu] (gpu2a) at ($(k8s2.south) + (-0.5, -0.4)$) {GPU\\Node};
    \node[gpu] (gpu2b) at ($(k8s2.south) + (0.5, -0.4)$) {GPU\\Node};
    
    % Auto-scaling indicator - better positioned
    \draw[arrow, color=orange!70!black, line width=1.5pt, <->] ($(gpu1c.east) + (0.15, 0)$) -- ($(gpu1d.east) + (0.15, 0)$);
    \node[font=\tiny\bfseries, color=orange!70!black, right=0.2cm of gpu1c, align=center] {Auto\\Scale};
    
    % Managed services - properly aligned vertically
    \node[service, right=2cm of az2.east, yshift=1.2cm] (sqs) {SQS\\Pub/Sub};
    ...
    % Private subnet indicator - properly sized
    ...
    \node[font=\tiny\bfseries, color=cyan!70!black, below=0.15cm of az1.south] {Private Subnets};
    
    % Connections - cleaner paths
    ...
    \draw[arrow, dashed, color=teal!60] (k8s1.east) .. controls (3, 3.5) .. (sqs.west);
    \draw[arrow, dashed, color=teal!60] (k8s2.east) -- (sqs.west);
    \draw[arrow, dashed, color=teal!60] (k8s1.east) .. controls (3, 2.5) .. (vector.west);
    \draw[arrow, dashed, color=teal!60] (k8s2.east) -- (vector.west);
    \draw[arrow, dashed, color=teal!60] (k8s1.east) .. controls (3, 1.5) .. (storage.west);
    \draw[arrow, dashed, color=teal!60] (k8s2.east) -- (storage.west);
    
    % Disaster Recovery indicator - better positioned
    ...
    \node[font=\footnotesize\bfseries, color=black!80, below=0.5cm of az1.south west, anchor=north west, xshift=-0.5cm] (legend) {Key Features:};
    \node[gpu, below=0.15cm of legend.south west, anchor=north west, minimum width=1.2cm] (leg1) {};
    \node[font=\tiny, right=0.15cm of leg1.east, anchor=west] {GPU Nodes (Auto-scaling)};
    \node[service, below=0.15cm of leg1.south west, anchor=north west, minimum width=1.2cm] (leg2) {};
    \node[font=\tiny, right=0.15cm of leg2.east, anchor=west] {Managed Services};
    \node[security, below=0.15cm of leg2.south west, anchor=north west, minimum width=1.2cm] (leg3) {};
