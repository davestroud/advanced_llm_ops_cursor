...
\begin{tikzpicture}[
  ...
  res/.style={rectangle, rounded corners=3pt, draw=black!70, thick, minimum width=14mm, minimum height=9mm, font=\footnotesize, align=center},
  arrow/.style={-{Latex}, line width=0.9pt},
  cloud/.style={box, fill=blue!20, draw=blue!70!black, minimum width=45mm, minimum height=55mm, inner sep=6pt},
  vpc/.style={box, fill=cyan!25, draw=cyan!70!black, inner sep=5pt},
  onprem/.style={box, fill=gray!18, draw=gray!70!black, minimum width=45mm, minimum height=55mm, inner sep=6pt},
  k8s/.style={box, fill=green!20, draw=green!70!black, inner sep=5pt},
...
% ================== (A) CLOUD-NATIVE ==================
\node[cloud, label={[lbl, above=6mm]Cloud-Native}] (cloudA) at (col1) {};
\node[vpc, minimum width=38mm, minimum height=42mm, label={[font=\footnotesize\bfseries, above=4mm]VPC}] (vpcA) at (cloudA.center) {};
\node[lb, minimum width=18mm, minimum height=10mm, align=center] (apigwA) at ($(vpcA.north) + (0, -8mm)$) {API\\GW};
\node[k8s, minimum width=32mm, minimum height=12mm, label={[font=\footnotesize\bfseries, above=3mm]K8s}] (k8sA) at ($(vpcA.center) + (0, -2mm)$) {};
\node[gpu, minimum width=14mm] (gpuA1) at ($(k8sA.south) + (-10mm, -10mm)$) {GPU};
\node[gpu, minimum width=14mm] (gpuA2) at ($(k8sA.south) + (10mm, -10mm)$) {GPU};
\node[store, minimum width=20mm, align=center] (objA) at ($(vpcA.south) + (0, -12mm)$) {Object\\Store};

\draw[arrow, orange!80!black] (apigwA) -- (k8sA);
\draw[arrow, purple!80!black] (k8sA) -- (objA);

% ================== (B) HYBRID ==================
\node[onprem, label={[lbl, above=6mm]Hybrid}] (premB) at ($(col2) + (-22mm, 0)$) {};
\node[k8s, minimum width=32mm, minimum height=12mm, label={[font=\footnotesize\bfseries, above=3mm]On-Prem K8s}] (k8sBprem) at ($(premB.center) + (0, 4mm)$) {};
\node[gpu, minimum width=14mm] (gpuB1) at ($(k8sBprem.south) + (-10mm, -10mm)$) {GPU};
\node[gpu, minimum width=14mm] (gpuB2) at ($(k8sBprem.south) + (10mm, -10mm)$) {GPU};
\node[store, minimum width=20mm, align=center] (vecB) at ($(premB.south) + (0, -12mm)$) {Vector\\DB};

\node[cloud] (cloudB) at ($(col2) + (22mm, 0)$) {};
\node[vpc, minimum width=38mm, minimum height=42mm, label={[font=\footnotesize\bfseries, above=4mm]Cloud VPC}] (vpcB) at (cloudB.center) {};
\node[lb, minimum width=18mm, minimum height=10mm, align=center] (apigwB) at ($(vpcB.north) + (0, -8mm)$) {API\\GW};
\node[k8s, minimum width=32mm, minimum height=12mm, label={[font=\footnotesize\bfseries, above=3mm]Cloud K8s}] (k8sBcloud) at ($(vpcB.center) + (0, -2mm)$) {};
\node[gpu, minimum width=14mm] (gpuB3) at ($(k8sBcloud.south) + (-10mm, -10mm)$) {GPU};
\node[gpu, minimum width=14mm] (gpuB4) at ($(k8sBcloud.south) + (10mm, -10mm)$) {GPU};

\node[gw, minimum width=16mm, minimum height=10mm] (vpnL) at ($(premB.east) + (6mm, 0)$) {VPN};
\node[gw, minimum width=16mm, minimum height=10mm] (vpnR) at ($(cloudB.west) + (-6mm, 0)$) {VPN};
...
% ================== (C) MULTI-CLUSTER ==================
\node[cloud, label={[lbl, above=6mm]Multi-Cluster / Multi-Region}] (cloudC) at (col3) {};
\node[lb, minimum width=20mm, minimum height=10mm, align=center] (glbC) at ($(cloudC.north) + (0, -6mm)$) {Global\\LB/DNS};

\node[vpc, minimum width=34mm, minimum height=42mm, label={[font=\footnotesize\bfseries, above=4mm]Region A}] (vpcC1) at ($(cloudC.center) + (-14mm, -4mm)$) {};
\node[k8s, minimum width=28mm, minimum height=12mm, label={[font=\footnotesize\bfseries, above=3mm]K8s A}] (k8sC1) at ($(vpcC1.center) + (0, 4mm)$) {};
\node[gpu, minimum width=14mm] (gpuC1) at ($(k8sC1.south) + (-10mm, -10mm)$) {GPU};
\node[gpu, minimum width=14mm] (gpuC2) at ($(k8sC1.south) + (10mm, -10mm)$) {GPU};

\node[vpc, minimum width=34mm, minimum height=42mm, label={[font=\footnotesize\bfseries, above=4mm]Region B}] (vpcC2) at ($(cloudC.center) + (14mm, -4mm)$) {};
\node[k8s, minimum width=28mm, minimum height=12mm, label={[font=\footnotesize\bfseries, above=3mm]K8s B}] (k8sC2) at ($(vpcC2.center) + (0, 4mm)$) {};
\node[gpu, minimum width=14mm] (gpuC3) at ($(k8sC2.south) + (-10mm, -10mm)$) {GPU};
\node[gpu, minimum width=14mm] (gpuC4) at ($(k8sC2.south) + (10mm, -10mm)$) {GPU};
...
\node[box, fill=white!95, draw=black!50, rounded corners=4pt, minimum width=0.95\linewidth, minimum height=20mm, inner sep=7pt] (legend) at ($(cloudB.south) + (0, -35mm)$) {};

\node[cloud, minimum width=14mm, minimum height=9mm] (L1) at ($(legend.west) + (14mm, 6mm)$) {};
\node[font=\footnotesize, right=4mm of L1.east, anchor=west] {Cloud/Region};

\node[vpc, minimum width=14mm, minimum height=9mm] (L2) at ($(L1.east) + (20mm, 0)$) {};
\node[font=\footnotesize, right=4mm of L2.east, anchor=west] {VPC};

\node[onprem, minimum width=14mm, minimum height=9mm] (L8) at ($(L2.east) + (20mm, 0)$) {};
\node[font=\footnotesize, right=4mm of L8.east, anchor=west] {On-Prem};

\node[k8s, minimum width=14mm, minimum height=9mm] (L3) at ($(L8.east) + (20mm, 0)$) {};
\node[font=\footnotesize, right=4mm of L3.east, anchor=west] {K8s};

\node[gpu, minimum width=14mm, minimum height=9mm] (L4) at ($(L1) + (0, -10mm)$) {};
\node[font=\footnotesize, right=4mm of L4.east, anchor=west] {GPU Node};

\node[lb, minimum width=14mm, minimum height=9mm] (L5) at ($(L2) + (0, -10mm)$) {};
\node[font=\footnotesize, right=4mm of L5.east, anchor=west] {API GW/LB};

\node[gw, minimum width=14mm, minimum height=9mm] (L6) at ($(L8) + (0, -10mm)$) {};
\node[font=\footnotesize, right=4mm of L6.east, anchor=west] {VPN/Direct Connect};

\node[store, minimum width=14mm, minimum height=9mm] (L7) at ($(L3) + (0, -10mm)$) {};
\node[font=\footnotesize, right=4mm of L7.east, anchor=west] {Object/Vector Store};
