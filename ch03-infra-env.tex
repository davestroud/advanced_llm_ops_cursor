...
\section*{Abstract}
\addcontentsline{toc}{section}{Abstract} % optional: include in ToC
...
\begin{itemize}
    \item \textbf{Select and configure compute resources:} Understand GPU/TPU architectures and pick the right accelerators for training versus inference workloads.
    \item \textbf{Design resilient architectures:} Employ high-availability and multi-zone clusters to avoid single points of failure.
    \item \textbf{Manage containerized deployments:} Use Kubernetes and container orchestration to schedule LLM services efficiently on GPU nodes.
    \item \textbf{Implement Infrastructure-as-Code (IaC):} Automate environment provisioning with tools like Terraform or Pulumi to ensure reproducibility and low mean-time-to-recovery (MTTR).
    \item \textbf{Build secure, compliant systems:} Integrate policy-as-code, secrets management, and auditing to meet enterprise security and compliance needs for LLM-centric applications.
\end{itemize}
...
In practice, cost modeling often involves simulating different deployment scenarios. For example, if you expect 100 requests per second each generating 200 tokens, that is 20k tokens/sec throughput needed. How many GPUs of type X are required to sustain that? If each A100 can do $\sim$8k/sec, you would need 3 A100s (at \$3.80/hr each, so \$11.4/hr). Maybe 2 H100s could do it (2 $\times$ \$5.00 = \$10/hr), slightly cheaper. Or perhaps 5 L4 GPUs (5 $\times$ \$0.35 = \$1.75/hr), but at $\sim$2k/sec each, 5 L4s only yield 10k/sec---not enough. This back-of-the-envelope calculation shows that while L4s are cheap, you would need too many of them in this scenario. On the other hand, if the workload was lighter (say 2k tokens/sec), a single A100 would be overkill and an L4 could handle it at one-eighth the hourly cost. 
...
\begin{figure}[htbp]
  \centering
  % \includegraphics[width=0.7\textwidth]{figures/batch_tradeoff.pdf}
  \caption{Throughput vs.\ batch size (schematic).}
