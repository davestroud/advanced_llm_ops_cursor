...
\begin{figure}[t]
  \centering
  \begin{tcolorbox}[
    title={\textbf{\textcolor{white}{Terraform: EKS Cluster with GPU Node Group}}},
    colback=blue!8,
    ...
    arc=5pt,
    left=5mm, right=5mm, top=4mm, bottom=4mm,
    drop shadow={shadow xshift=0.5mm, shadow yshift=0.5mm, shadow color=black!30},
    enhanced
  ]
  \footnotesize
  \ttfamily
...
\begin{verbatim}
# EKS Cluster with GPU-enabled node group
# This module provisions an AWS EKS cluster optimized for LLM workloads
...
# GPU Node Group (A100/H100 instances)
resource "aws_eks_node_group" "gpu_nodes" {
  ...
  instance_types = ["g5.2xlarge", "p4d.24xlarge"]  # NVIDIA A10G, A100
  ...
  # Labels and taints for GPU workload isolation
  labels = {
  ...
  taint {
    key    = "nvidia.com/gpu"
    value  = "true"
    effect = "NO_SCHEDULE"  # Only pods with toleration can schedule
  }

  # Launch template with NVIDIA drivers
  launch_template {
  ...
  depends_on = [
    aws_eks_cluster.llm_cluster,
    aws_iam_role_policy_attachment.node_group
  ]
}

# Output: Kubeconfig for downstream automation
output "kubeconfig" {
  value     = aws_eks_cluster.llm_cluster.kubeconfig
  sensitive = true
}
...
\end{verbatim}
  ...
  \normalsize
  \end{tcolorbox}
  \caption{Terraform configuration for an EKS cluster with GPU-enabled node groups. The configuration demonstrates key IaC patterns: declarative resource definitions, node labeling and taints for workload isolation, autoscaling configuration, and secure outputs. Labels and taints ensure only GPU workloads (with matching tolerations) can schedule on GPU nodes, preventing resource contention.}
  \label{fig:iac-example}
\end{figure}
