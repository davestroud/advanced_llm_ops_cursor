...
\begin{tikzpicture}[
  ...
  cloud/.style={box, fill=cloudblue!20, draw=cloudblue!85!black, line width=1.3pt, minimum width=50mm, minimum height=52mm, inner sep=8pt},
  vpc/.style={box, fill=vpccyan!25, draw=vpccyan!80!black, line width=1.1pt, inner sep=6pt},
  onprem/.style={box, fill=onpremgray!20, draw=onpremgray!80!black, line width=1.3pt, minimum width=50mm, minimum height=52mm, inner sep=8pt},
  k8s/.style={box, fill=k8sgreen!25, draw=k8sgreen!80!black, line width=1.1pt, inner sep=6pt},
  lb/.style={res, fill=apiorange!30, draw=apiorange!80!black, minimum width=28mm, minimum height=14mm},
  gw/.style={res, fill=vpnteal!30, draw=vpnteal!80!black, minimum width=24mm, minimum height=14mm},
  gpu/.style={res, fill=gpupurple!30, draw=gpupurple!80!black, minimum width=22mm, minimum height=12mm},
  store/.style={res, fill=storeyellow!30, draw=storeyellow!80!black, minimum width=30mm, minimum height=14mm}
]

% ================== SECTION HEADERS ==================
\node[header] (headerA) at (-60mm, 35mm) {Cloud-Native};
\node[header] (headerB) at (0mm, 35mm) {Hybrid};
\node[header] (headerC) at (60mm, 35mm) {Multi-Cluster / Multi-Region};

% ================== VISUAL SEPARATORS ==================
\draw[draw=black!20, line width=0.8pt, dashed] (-30mm, -45mm) -- (-30mm, 30mm);
\draw[draw=black!20, line width=0.8pt, dashed] (30mm, -45mm) -- (30mm, 30mm);

% ================== (A) CLOUD-NATIVE (Left) ==================
\node[cloud] (cloudA) at (-60mm, 0mm) {};
\node[vpc, minimum width=46mm, minimum height=48mm] (vpcA) at (cloudA.center) {VPC};
\node[lb, align=center] (apigwA) at ($(vpcA.north) + (0, -12mm)$) {API\\Gateway};
\node[k8s, minimum width=42mm, minimum height=18mm] (k8sA) at ($(vpcA.center) + (0, 2mm)$) {K8s Cluster};
\node[gpu] (gpuA1) at ($(k8sA.south) + (-15mm, -15mm)$) {GPU};
\node[gpu] (gpuA2) at ($(k8sA.south) + (15mm, -15mm)$) {GPU};
\node[store, align=center] (objA) at ($(vpcA.south) + (0, -18mm)$) {Object\\Store};

\draw[arrow, color=apiorange!90!black] (apigwA) -- (k8sA);
\draw[arrow, color=gpupurple!90!black] (k8sA) -- (objA);

% ================== (B) HYBRID (Center) ==================
\node[onprem] (premB) at (-30mm, 0mm) {};
\node[k8s, minimum width=42mm, minimum height=18mm] (k8sBprem) at ($(premB.center) + (0, 10mm)$) {On-Prem\\K8s};
\node[gpu] (gpuB1) at ($(k8sBprem.south) + (-15mm, -15mm)$) {GPU};
\node[gpu] (gpuB2) at ($(k8sBprem.south) + (15mm, -15mm)$) {GPU};
\node[store, align=center] (vecB) at ($(premB.south) + (0, -18mm)$) {Vector\\DB};

\node[cloud] (cloudB) at (30mm, 0mm) {};
\node[vpc, minimum width=46mm, minimum height=48mm] (vpcB) at (cloudB.center) {Cloud VPC};
\node[lb, align=center] (apigwB) at ($(vpcB.north) + (0, -12mm)$) {API\\Gateway};
\node[k8s, minimum width=42mm, minimum height=18mm] (k8sBcloud) at ($(vpcB.center) + (0, 2mm)$) {Cloud K8s};
\node[gpu] (gpuB3) at ($(k8sBcloud.south) + (-15mm, -15mm)$) {GPU};
\node[gpu] (gpuB4) at ($(k8sBcloud.south) + (15mm, -15mm)$) {GPU};

\node[gw, align=center] (vpnL) at ($(premB.east) + (12mm, 0)$) {VPN};
\node[gw, align=center] (vpnR) at ($(cloudB.west) + (-12mm, 0)$) {VPN};
...
% ================== (C) MULTI-CLUSTER (Right) ==================
\node[cloud] (cloudC) at (60mm, 0mm) {};
\node[lb, align=center] (glbC) at ($(cloudC.north) + (0, -12mm)$) {Global\\LB/DNS};

\node[vpc, minimum width=42mm, minimum height=48mm] (vpcC1) at ($(cloudC.center) + (-20mm, -5mm)$) {Region A};
\node[k8s, minimum width=38mm, minimum height=18mm] (k8sC1) at ($(vpcC1.center) + (0, 10mm)$) {K8s A};
\node[gpu] (gpuC1) at ($(k8sC1.south) + (-15mm, -15mm)$) {GPU};
\node[gpu] (gpuC2) at ($(k8sC1.south) + (15mm, -15mm)$) {GPU};

\node[vpc, minimum width=42mm, minimum height=48mm] (vpcC2) at ($(cloudC.center) + (20mm, -5mm)$) {Region B};
\node[k8s, minimum width=38mm, minimum height=18mm] (k8sC2) at ($(vpcC2.center) + (0, 10mm)$) {K8s B};
\node[gpu] (gpuC3) at ($(k8sC2.south) + (-15mm, -15mm)$) {GPU};
\node[gpu] (gpuC4) at ($(k8sC2.south) + (15mm, -15mm)$) {GPU};

\draw[arrow, color=apiorange!90!black] (glbC) -- ($(glbC) + (-20mm, -18mm)$) -- (k8sC1);
\draw[arrow, color=apiorange!90!black] (glbC) -- ($(glbC) + (20mm, -18mm)$) -- (k8sC2);
...
\node[box, fill=white!99, draw=black!50, rounded corners=6pt, line width=1.2pt, minimum width=180mm, minimum height=24mm, inner sep=8pt] (legend) at (0mm, -50mm) {};
...
\caption{Deployment pattern selection determines operational complexity and cost structure. \textbf{Cloud-Native} (left) maximizes elasticity and reduces operational overhead but may increase costs at scale. \textbf{Hybrid} (center) balances on-premise control with cloud flexibility, optimizing for cost and compliance. \textbf{Multi-Cluster} (right) enables geographic distribution and failover but increases management complexity. Choose based on scale, compliance requirements, and operational capabilities.}
