...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[
    ...
    cloud/.style={ellipse, draw=blue!70!black, fill=blue!20, line width=1pt,
      minimum width=50mm, minimum height=26mm, align=center, font=\small\bfseries, inner sep=7pt},
    az/.style={rectangle, draw=cyan!60!black, fill=cyan!12, line width=1pt,
      rounded corners=5pt, minimum width=55mm, minimum height=70mm},
    k8s/.style={rectangle, draw=purple!70!black, fill=purple!20, line width=1pt,
      rounded corners=3pt, minimum width=42mm, minimum height=18mm, align=center, font=\small\bfseries, inner sep=7pt},
    gpu/.style={rectangle, draw=orange!70!black, fill=orange!25, line width=1pt,
      rounded corners=3pt, minimum width=26mm, minimum height=13mm, align=center, font=\small, inner sep=5pt},
    service/.style={rectangle, draw=teal!70!black, fill=teal!20, line width=1pt,
      rounded corners=3pt, minimum width=30mm, minimum height=13mm, align=center, font=\small, inner sep=6pt},
    security/.style={rectangle, draw=red!70!black, fill=red!18, line width=1pt,
      rounded corners=3pt, minimum width=28mm, minimum height=13mm, align=center, font=\small\bfseries, inner sep=6pt},
    network/.style={rectangle, draw=cyan!70!black, fill=cyan!20, line width=1pt,
      rounded corners=3pt, minimum width=42mm, minimum height=15mm, align=center, font=\small\bfseries, inner sep=6pt},
...
\begin{tikzpicture}[
  ...
  lbl/.style={font=\small\bfseries, inner sep=4pt},
  res/.style={rectangle, rounded corners=3pt, draw=black!70, thick, minimum width=18mm, minimum height=11mm, font=\small, align=center},
  arrow/.style={-{Latex}, line width=1pt},
  cloud/.style={box, fill=blue!20, draw=blue!70!black, minimum width=48mm, minimum height=52mm, inner sep=6pt},
  vpc/.style={box, fill=cyan!25, draw=cyan!70!black, inner sep=5pt},
  onprem/.style={box, fill=gray!18, draw=gray!70!black, minimum width=48mm, minimum height=52mm, inner sep=6pt},
  k8s/.style={box, fill=green!20, draw=green!70!black, inner sep=6pt},
...
\node (row1) at (0, 42mm) {};  % Top: Cloud-Native
\node (row2) at (0, 0) {};     % Middle: Hybrid
\node (row3) at (0, -42mm) {}; % Bottom: Multi-Cluster

% ================== (A) CLOUD-NATIVE (Top) ==================
\node[cloud, label={[lbl, above=7mm]Cloud-Native}] (cloudA) at (row1) {};
\node[vpc, minimum width=40mm, minimum height=42mm, label={[font=\small\bfseries, above=4mm]VPC}] (vpcA) at (cloudA.center) {};
\node[lb, minimum width=22mm, minimum height=12mm, align=center] (apigwA) at ($(vpcA.north) + (0, -9mm)$) {API\\GW};
\node[k8s, minimum width=36mm, minimum height=14mm, label={[font=\small\bfseries, above=4mm]K8s}] (k8sA) at ($(vpcA.center) + (0, -2mm)$) {};
\node[gpu, minimum width=18mm] (gpuA1) at ($(k8sA.south) + (-12mm, -12mm)$) {GPU};
\node[gpu, minimum width=18mm] (gpuA2) at ($(k8sA.south) + (12mm, -12mm)$) {GPU};
\node[store, minimum width=24mm, align=center] (objA) at ($(vpcA.south) + (0, -14mm)$) {Object\\Store};

\draw[arrow, orange!80!black] (apigwA) -- (k8sA);
\draw[arrow, purple!80!black] (k8sA) -- (objA);

% ================== (B) HYBRID (Middle) ==================
\node[onprem, label={[lbl, above=7mm]Hybrid}] (premB) at ($(row2) + (-34mm, 0)$) {};
\node[k8s, minimum width=36mm, minimum height=14mm, label={[font=\small\bfseries, above=4mm]On-Prem K8s}] (k8sBprem) at ($(premB.center) + (0, 5mm)$) {};
\node[gpu, minimum width=18mm] (gpuB1) at ($(k8sBprem.south) + (-12mm, -12mm)$) {GPU};
\node[gpu, minimum width=18mm] (gpuB2) at ($(k8sBprem.south) + (12mm, -12mm)$) {GPU};
\node[store, minimum width=24mm, align=center] (vecB) at ($(premB.south) + (0, -14mm)$) {Vector\\DB};

\node[cloud] (cloudB) at ($(row2) + (34mm, 0)$) {};
\node[vpc, minimum width=40mm, minimum height=42mm, label={[font=\small\bfseries, above=4mm]Cloud VPC}] (vpcB) at (cloudB.center) {};
\node[lb, minimum width=22mm, minimum height=12mm, align=center] (apigwB) at ($(vpcB.north) + (0, -9mm)$) {API\\GW};
\node[k8s, minimum width=36mm, minimum height=14mm, label={[font=\small\bfseries, above=4mm]Cloud K8s}] (k8sBcloud) at ($(vpcB.center) + (0, -2mm)$) {};
\node[gpu, minimum width=18mm] (gpuB3) at ($(k8sBcloud.south) + (-12mm, -12mm)$) {GPU};
\node[gpu, minimum width=18mm] (gpuB4) at ($(k8sBcloud.south) + (12mm, -12mm)$) {GPU};

\node[gw, minimum width=20mm, minimum height=12mm] (vpnL) at ($(premB.east) + (7mm, 0)$) {VPN};
\node[gw, minimum width=20mm, minimum height=12mm] (vpnR) at ($(cloudB.west) + (-7mm, 0)$) {VPN};

\draw[arrow, teal!80!black] (k8sBprem) -- (vpnL);
\draw[arrow, teal!80!black] (vpnR) -- (k8sBcloud);
\draw[arrow, gray!70, dashed, line width=1.3pt] (vpnL) -- node[font=\small, align=center, midway, above] {Direct\\Connect} (vpnR);
\draw[arrow, purple!80!black] (k8sBprem) -- (vecB);
\draw[arrow, orange!80!black] (apigwB) -- (k8sBcloud);

% ================== (C) MULTI-CLUSTER (Bottom) ==================
\node[cloud, label={[lbl, above=7mm]Multi-Cluster / Multi-Region}] (cloudC) at (row3) {};
\node[lb, minimum width=24mm, minimum height=12mm, align=center] (glbC) at ($(cloudC.north) + (0, -7mm)$) {Global\\LB/DNS};
...
\node[k8s, minimum width=32mm, minimum height=14mm, label={[font=\small\bfseries, above=4mm]K8s A}] (k8sC1) at ($(vpcC1.center) + (0, 5mm)$) {};
\node[gpu, minimum width=18mm] (gpuC1) at ($(k8sC1.south) + (-12mm, -12mm)$) {GPU};
\node[gpu, minimum width=18mm] (gpuC2) at ($(k8sC1.south) + (12mm, -12mm)$) {GPU};
...
\node[k8s, minimum width=32mm, minimum height=14mm, label={[font=\small\bfseries, above=4mm]K8s B}] (k8sC2) at ($(vpcC2.center) + (0, 5mm)$) {};
\node[gpu, minimum width=18mm] (gpuC3) at ($(k8sC2.south) + (-12mm, -12mm)$) {GPU};
\node[gpu, minimum width=18mm] (gpuC4) at ($(k8sC2.south) + (12mm, -12mm)$) {GPU};

\draw[arrow, orange!80!black] (glbC) -- ($(glbC) + (-16mm, -14mm)$) -- (k8sC1);
\draw[arrow, orange!80!black] (glbC) -- ($(glbC) + (16mm, -14mm)$) -- (k8sC2);
\draw[arrow, gray!70, dashed, line width=1.3pt] (k8sC1) -- node[font=\small, align=center, midway, above, sloped] {spillover/\\failover} (k8sC2);

% ================== Clean legend (below all patterns) ==================
\node[box, fill=white!95, draw=black!50, rounded corners=4pt, minimum width=0.95\linewidth, minimum height=20mm, inner sep=7pt] (legend) at ($(row3) + (0, -36mm)$) {};

\node[cloud, minimum width=16mm, minimum height=11mm] (L1) at ($(legend.west) + (16mm, 7mm)$) {};
\node[font=\small, right=5mm of L1.east, anchor=west] {Cloud/Region};

\node[vpc, minimum width=16mm, minimum height=11mm] (L2) at ($(L1.east) + (22mm, 0)$) {};
\node[font=\small, right=5mm of L2.east, anchor=west] {VPC};

\node[onprem, minimum width=16mm, minimum height=11mm] (L8) at ($(L2.east) + (22mm, 0)$) {};
\node[font=\small, right=5mm of L8.east, anchor=west] {On-Prem};

\node[k8s, minimum width=16mm, minimum height=11mm] (L3) at ($(L8.east) + (22mm, 0)$) {};
\node[font=\small, right=5mm of L3.east, anchor=west] {K8s};

\node[gpu, minimum width=16mm, minimum height=11mm] (L4) at ($(L1) + (0, -12mm)$) {};
\node[font=\small, right=5mm of L4.east, anchor=west] {GPU Node};

\node[lb, minimum width=16mm, minimum height=11mm] (L5) at ($(L2) + (0, -12mm)$) {};
\node[font=\small, right=5mm of L5.east, anchor=west] {API GW/LB};

\node[gw, minimum width=16mm, minimum height=11mm] (L6) at ($(L8) + (0, -12mm)$) {};
\node[font=\small, right=5mm of L6.east, anchor=west] {VPN/Direct Connect};

\node[store, minimum width=16mm, minimum height=11mm] (L7) at ($(L3) + (0, -12mm)$) {};
\node[font=\small, right=5mm of L7.east, anchor=west] {Object/Vector Store};
...
\begin{tikzpicture}[
  ...
  gpu/.style={rectangle, rounded corners=3pt, draw=black!70, thick,
              minimum width=18mm, minimum height=10mm, font=\small, align=center},
  arrow/.style={-{Latex}, line width=1pt},
  lbl/.style={font=\small\bfseries}
]

% ============= On-Prem =============
\node[box, fill=gray!18, minimum width=48mm, minimum height=36mm,
      label={[lbl]above:{\textbf{On-Prem (L4 Cluster)}}}] (onprem) {};
\node[gpu, fill=purple!25] (l4a) at ([yshift=7mm]onprem.center) {L4 GPU};
\node[gpu, fill=purple!25, right=5mm of l4a] (l4b) {L4 GPU};
\node[box, fill=yellow!25, below=7mm of onprem.center, align=center, minimum width=34mm, minimum height=12mm] (embed) {Embeddings /\\NER Tagging};
\draw[arrow, gray!70] (l4a) -- (embed);
\draw[arrow, gray!70] (l4b) -- (embed);

% VPN to cloud
\node[box, fill=teal!25, minimum width=20mm, minimum height=12mm,
      right=12mm of onprem.east] (vpn) {VPN};

\draw[arrow, teal!80] (onprem.east) -- (vpn.west);

% ============= Cloud Baseline A100 =============
\node[box, fill=blue!20, minimum width=58mm, minimum height=56mm,
      right=16mm of vpn, label={[lbl]above:{\textbf{Cloud (AWS EKS)}}}] (cloud) {};

\node[box, fill=green!20, minimum width=52mm, minimum height=18mm,
      above=5mm of cloud.south, anchor=south, label={[font=\small\bfseries]above:{Baseline Node Group}}] (a100group) {};
\node[gpu, fill=purple!25] (a100a) at ([xshift=-14mm]a100group.center) {A100};
\node[gpu, fill=purple!25] (a100b) at ([xshift=14mm]a100group.center) {A100};

\node[box, fill=green!20, minimum width=52mm, minimum height=18mm,
      above=5mm of a100group.north, anchor=south, label={[font=\small\bfseries]above:{Autoscaling Node Group}}] (h100group) {};
\node[gpu, fill=purple!40] (h100a) at ([xshift=-14mm]h100group.center) {H100};
\node[gpu, fill=purple!40] (h100b) at ([xshift=14mm]h100group.center) {H100};

% Serving pods
\node[box, fill=orange!25, above=5mm of h100group.north, anchor=south,
      minimum width=52mm, minimum height=14mm] (serving) {Model Serving Pods (TGI / vLLM)};

% Terraform + K8s control
\node[box, fill=blue!25, minimum width=36mm, minimum height=12mm,
...
% ============= Legend =============
\node[box, fill=white!95, draw=black!50, very thick, inner sep=5pt,
      below=14mm of cloud.south, minimum width=0.9\linewidth, minimum height=16mm] (legend) {};
