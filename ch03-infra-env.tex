...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[
    ...
    % Top layers
    \node[cloud] (cloud) {Cloud Provider\\(AWS/Azure/GCP)};
    \node[network, below=0.85cm of cloud] (gateway) {API Gateway\\Load Balancer};

    \node[security, left=2.3cm of gateway]  (iam) {IAM};
    \node[security, right=2.3cm of gateway] (sg)  {Security\\Groups};
    ...
    % GPU nodes (AZ-1)
    \node[gpu, below=0.5cm of k8s1.south, xshift=-0.68cm] (gpu1a) {GPU\\Node};
    \node[gpu, below=0.5cm of k8s1.south]                 (gpu1b) {GPU\\Node};
    \node[gpu, below=0.5cm of k8s1.south, xshift= 0.68cm] (gpu1c) {GPU\\Node};

    \node[gpu, opacity=0.35, below=0.95cm of k8s1.south, xshift=-0.34cm] (gpu1d) {GPU\\Node};
    \node[gpu, opacity=0.35, below=0.95cm of k8s1.south, xshift= 0.34cm] (gpu1e) {GPU\\Node};

    % GPU nodes (AZ-2)
    \node[gpu, below=0.5cm of k8s2.south, xshift=-0.52cm] (gpu2a) {GPU\\Node};
    \node[gpu, below=0.5cm of k8s2.south, xshift= 0.52cm] (gpu2b) {GPU\\Node};
    ...
    \node[service, right=2.35cm of az2.east, yshift=1.05cm] (sqs)     {SQS\\Pub/Sub};
    \node[service, below=0.52cm of sqs]     (vector)  {Vector\\DB};
    \node[service, below=0.52cm of vector]  (storage) {Object\\Store};
    \node[service, below=0.52cm of storage] (bedrock) {Bedrock\\OpenAI};
...
\node[lb, minimum width=1cm, minimum height=0.5cm] (apigwA) at ($(vpcA.north) + (0, -0.3cm)$) {API\\GW};
...
\node[store, minimum width=1cm] (objA) at ($(gpuA1.south) + (0.4cm, -0.3cm)$) {Object\\Store};
...
\node[store, minimum width=1cm] (vecB) at ($(gpuB1.south) + (0.4cm, -0.3cm)$) {Vector\\DB};
...
\node[lb, minimum width=1cm, minimum height=0.5cm] (apigwB) at ($(vpcB.north) + (0, -0.3cm)$) {API\\GW};
...
\node[lb, minimum width=1.2cm, minimum height=0.5cm] (glbC) at ($(cloudC.north) + (0, -0.2cm)$) {Global\\LB/DNS};
...
\node[box, fill=yellow!25, below=4mm of onprem.center] (embed) {Embeddings /\\NER Tagging};
...
\begin{tikzpicture}[baseline=(current bounding box.center), node distance=2mm and 4mm]
  \node[gpu, fill=violet!25, minimum width=8mm, minimum height=5mm, label={[lbl]right:L4 / A100 GPU}] (L1) {};
  \node[gpu, fill=violet!40, minimum width=8mm, minimum height=5mm, right=20mm of L1, label={[lbl]right:H100 GPU}] (L2) {};
  \node[box, fill=orange!25, minimum width=8mm, minimum height=5mm, right=20mm of L2, label={[lbl]right:Serving Pods}] (L3) {};
  \node[box, fill=blue!25, minimum width=8mm, minimum height=5mm, right=20mm of L3, label={[lbl]right:IaC / Control Plane}] (L4) {};
\end{tikzpicture}
};
